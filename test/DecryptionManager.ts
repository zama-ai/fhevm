...
// Constants for the DecryptionManager contract
...
describe("DecryptionManager", function () {
  ...
  // Define other valid ctHandles (they will not be added in the ciphertext manager and allowed for
  ...
  async function prepareAddCiphertextFixture() {
    ...
    const { keyManager, ciphertextManager, owner, kmsTxSenders, coprocessorTxSenders, fheParamsName } = fixtureData;
  ...
  describe("Public Decryption", function () {
    ...
    async function preparePublicDecryptEIP712Fixture() {
      ...
      const { aclManager, decryptionManager, kmsSigners, coprocessorTxSenders } = fixtureData;
    ...
    it("Should request a public decryption with multiple ctHandles", async function () {
      ...
      const requestTx = await decryptionManager.publicDecryptionRequest(ctHandles);
    ...
    it("Should request a public decryption with a single ctHandle", async function () {
      ...
      const requestTx = await decryptionManager.publicDecryptionRequest([ctHandles[0]]);
    ...
    it("Should revert because ctHandles list is empty", async function () {
      ...
      await expect(decryptionManager.publicDecryptionRequest([])).to.be.revertedWithCustomError(
        decryptionManager,
        "EmptyCtHandles",
      );
    });
    ...
    it("Should revert because handle represents an invalid FHE type", async function () {
      ...
      await expect(decryptionManager.publicDecryptionRequest([invalidFHETypeCtHandle]))
        .to.be.revertedWithCustomError(decryptionManager, "InvalidFHEType")
    ...
    it("Should revert because handle represents an unsupported FHE type", async function () {
      ...
      await expect(decryptionManager.publicDecryptionRequest([unsupportedFHETypeCtHandle]))
        .to.be.revertedWithCustomError(decryptionManager, "UnsupportedFHEType")
    ...
    it("Should revert because total bit size exceeds the maximum allowed", async function () {
      ...
      await expect(decryptionManager.publicDecryptionRequest(largeBitSizeCtHandles))
        .to.be.revertedWithCustomError(decryptionManager, "MaxDecryptionRequestBitSizeExceeded")
    ...
    it("Should revert because the signer is not a KMS signer", async function () {
      ...
      await decryptionManager.publicDecryptionRequest(ctHandles);
    ...
    it("Should revert because of ctMaterials tied to different key IDs", async function () {
      ...
      const requestTx = decryptionManager.publicDecryptionRequest([...ctHandles, newCtHandle]);
  ...
  describe("User Decryption", function () {
    ...
    async function prepareUserDecryptEIP712Fixture() {
      const fixtureData = await loadFixture(prepareAddCiphertextFixture);
      const { decryptionManager, aclManager, kmsSigners, coprocessorTxSenders } = fixtureData;
    ...
    it("Should revert because ctHandleContractPairs is empty", async function () {
      ...
      ).to.be.revertedWithCustomError(decryptionManager, "EmptyCtHandleContractPairs");
    });
    ...
    describe("Checks", function () {
      ...
      it("Should revert because ciphertext material has not been added", async function () {
        ...
        await expect(decryptionManager.checkUserDecryptionReady(user.address, [newCtHandleContractPair]))
          .to.be.revertedWithCustomError(ciphertextManager, "CiphertextMaterialNotFound")
          .withArgs(newCtHandle);
      });
  ...
  describe("Delegated User Decryption", function () {
    ...
    async function prepareDelegatedUserDecryptEIP712Fixture() {
      ...
      for (const ctHandle of ctHandles) {
        for (let i = 0; i < coprocessorTxSenders.length; i++) {
          await aclManager.connect(coprocessorTxSenders[i]).allowAccount(ctHandle, delegatorAddress);
          await aclManager.connect(coprocessorTxSenders[i]).allowAccount(ctHandle, contractAddress);
        }
      }
      ...
      for (const txSender of coprocessorTxSenders) {
        await aclManager.connect(txSender).delegateAccount(hostChainId, delegationAccounts, contractAddresses);
      }
    ...
    it("Should revert because of ctMaterials tied to different key IDs", async function () {
      ...
      for (let i = 0; i < coprocessorTxSenders.length; i++) {
        await ciphertextManager
          .connect(coprocessorTxSenders[i])
          .addCiphertextMaterial(newCtHandle, keyId2, ciphertextDigest, snsCiphertextDigest);
        await aclManager.connect(coprocessorTxSenders[i]).allowAccount(newCtHandle, delegatorAddress);
        await aclManager.connect(coprocessorTxSenders[i]).allowAccount(newCtHandle, contractAddress);
      }
      ...
      // Check that different key IDs are not allowed for batched user decryption
      await expect(requestTx)
        .to.be.revertedWithCustomError(decryptionManager, "DifferentKeyIdsNotAllowed")
        .withArgs(keyId2);
    });
