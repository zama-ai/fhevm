...
async function initTestingWallets(nKmsNodes: number, nCoprocessors: number) {
  ...
  for (let idx = 0; idx < nKmsNodes; idx++) {
    const kmsNode = await hre.ethers.getSigner(getRequiredEnvVar(`KMS_NODE_ADDRESS_${idx}`));
    await checkIsHardhatSigner(kmsNode);
    kmsSigners.push(kmsNode);
  }

  const coprocessorSigners = [];
  for (let idx = 0; idx < nCoprocessors; idx++) {
    const coprocessor = await hre.ethers.getSigner(getRequiredEnvVar(`COPROCESSOR_ADDRESS_${idx}`));
    await checkIsHardhatSigner(coprocessor);
