...
| `preview-env-e2e-tests-lv2` | Same as `preview-env-e2e-tests`, but the host side runs the **listener v2 stack only** (`listener-<i>` → Redis → host-listener-consumer): the host-listener pollers (`coprocessor-poller-<i>`, `coprocessor-poller-polygon-<i>`) are not deployed. Cannot be combined with `preview-env-blue-green`. |
