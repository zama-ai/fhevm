...
set_kms_host_chain() {
  ...
  NAME="${name}" URL="${url}" CHAIN_ID="${chain_id}" yq -i '
    (.commonConfig.hostChains[] | select(.name == strenv(NAME))) |= (
      .url = strenv(URL) | .chainId = strenv(CHAIN_ID)
    )
  ' "${file}"
}
...
if [[ "${DEPLOY_POLYGON:-false}" == "true" ]]; then
  ...
  KMS_BASE="${kms}" yq -i '
    (.commonConfig.hostChains[] | select(.name == "ethereum")) =
      (load(strenv(KMS_BASE)).commonConfig.hostChains[] | select(.name == "ethereum"))
  ' "${pkms}"
