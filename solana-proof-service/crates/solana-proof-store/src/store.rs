...
use crate::reduce::{
    reduce_completed_block, LeafKind, PriorEncryptedValueAccountState, StagedBlockReduction,
};
use crate::replay::EncryptedValueAccountReplayState;
...
impl SqlProofStore {
    ...
    pub async fn proof_snapshot_for_leaf(
        ...
        // (a created-public `fhe_eval` output seals one, and a later `make_handle_public` re-release
    ...
    /// Reads an encrypted_value_account's metadata + ordered leaf commitments into a [`ProofSnapshot`] within a
    ...
    async fn load_prior_encrypted_value_accounts(
        ...
        tx: &mut Transaction<'_, Postgres>,
        block: &CompletedBlock,
    ) -> Result<BTreeMap<[u8; 32], PriorEncryptedValueAccountState>, StoreError> {
        ...
        for encrypted_value_account in encrypted_value_accounts.keys() {
            ...
            existing.insert(
                *encrypted_value_account,
                PriorEncryptedValueAccountState {
                    replay: EncryptedValueAccountReplayState {
                        current_handle,
