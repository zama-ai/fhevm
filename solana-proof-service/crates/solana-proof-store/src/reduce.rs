...
use crate::replay::{apply_instruction, EncryptedValueAccountReplayState, ReplayError};

#[derive(Clone, Debug, PartialEq, Eq)]
pub struct PriorEncryptedValueAccountState {
    pub replay: EncryptedValueAccountReplayState,
    pub leaf_count: u64,
    pub peaks: Vec<[u8; 32]>,
}
...
pub enum ReduceError {
    ...
    UnknownPreBootstrapEncryptedValueAccount { encrypted_value_account: String },
...
pub fn reduce_completed_block(
    ...
    existing: &BTreeMap<[u8; 32], PriorEncryptedValueAccountState>,
) -> Result<StagedBlockReduction, ReduceError> {
    let mut working_replay: BTreeMap<[u8; 32], EncryptedValueAccountReplayState> = existing
        .iter()
    ...
    for tx in &block.transactions {
        ...
        for instruction in &decoded {
            ...
            let events = match apply_instruction(&mut state, instruction) {
                Ok(events) => events,
                Err(ReplayError::UnknownEncryptedValueAccount(encrypted_value_account)) => {
                    return Err(ReduceError::UnknownPreBootstrapEncryptedValueAccount {
                        encrypted_value_account: hex32(&encrypted_value_account),
                    });
                }
...
mod tests {
    ...
    fn unknown_pre_bootstrap_encrypted_value_account_is_detected_without_existing_state() {
        ...
        let err = ReduceError::UnknownPreBootstrapEncryptedValueAccount {
