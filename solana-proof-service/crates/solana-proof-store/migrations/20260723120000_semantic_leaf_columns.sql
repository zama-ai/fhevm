...
--
-- The proof service resolves (lineage, handle[, subject], kind) -> leaf_index for the
...
-- still count in solana_proof_lineages.leaf_count, so at parity with chain a semantic query for a
...
-- (lineage, leaf_kind, handle, subject); public-decrypt leaves match on
-- (lineage, leaf_kind, handle) with subject NULL.
CREATE INDEX solana_proof_leaves_semantic_idx
    ON solana_proof_leaves (lineage, leaf_kind, handle, subject);
