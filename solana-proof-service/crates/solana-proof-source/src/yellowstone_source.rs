...
#[derive(Clone, Debug)]
pub struct YellowstoneBlockSource {
    config: YellowstoneSourceConfig,
}

impl YellowstoneBlockSource {
    pub fn new(config: YellowstoneSourceConfig) -> Result<Self, YellowstoneSourceError> {
...
pub struct YellowstoneSubscription {
...
impl YellowstoneSubscription {
    ...
    pub async fn next_block(&mut self) -> Result<CompletedBlock, YellowstoneSourceError> {
        next_block(
            &mut self.stream,
...
mod tests {
    ...
    fn resolves_successful_instruction_payload_in_execution_order() {
        ...
        assert_eq!(transaction.instructions[0].top_level_index, 0);
        assert_eq!(transaction.instructions[1].top_level_index, 0);
        let digest = Sha256::digest(b"global:make_handle_public");
        assert_eq!(&transaction.instructions[0].data[..8], &digest[..8]);
        assert_eq!(&transaction.instructions[0].data[8..], &[9; 32]);
    }
