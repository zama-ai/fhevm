...
import {KmsNode} from "../host-contracts/contracts/shared/Structs.sol";
...
abstract contract FhevmTest is Test {
    ...
    /// @dev v0.13: the KMS signer set lives in ProtocolConfig (KMSVerifier reads it via
    ///      `PROTOCOL_CONFIG.getKmsSigners()`). Register a single mock node whose signer is
    ///      `mockKmsSigner`, with all thresholds set to 1. (v0.14 widened the init to KmsNodeParams[]
    ///      plus a software version and PCR values; v0.13 takes just (KmsNode[], KmsThresholds).)
    function _deployProtocolConfig() internal {
        ...
        EmptyUUPSProxy(protocolConfigAdd).upgradeToAndCall(
            protocolConfigImpl, abi.encodeCall(ProtocolConfig.initializeFromEmptyProxy, (nodes, thresholds))
        );
    }
