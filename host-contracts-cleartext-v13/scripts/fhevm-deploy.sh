...
# CLI options
#
# This package is single-version (v0.14). There is one Foundry profile
# (`default`); the historical `--profile` flag is accepted but ignored.
#
# Usage:
#   ./fhevm-deploy.sh
#   ./fhevm-deploy.sh --dry-run
#   ./fhevm-deploy.sh --chain localcleartext
# ==============================================================================
...
Options:
  ...
  --profile <name>      Accepted for backward compatibility; ignored (single-version package).
...
if [[ -n "$profile_cli" && "$profile_cli" != "v14" && "$profile_cli" != "latest" && "$profile_cli" != "default" ]]; then
    echo "ℹ️  ignoring --profile '$profile_cli' (single-version package; using the default profile)"
fi
...
if [[ "$dry_run" == "true" ]]; then
    echo
    echo "🧪 Dry run: printing precomputed FHEVM host addresses (default profile)"
    env "${FORGE_ENV[@]}" forge script \
        scripts/DeployCleartextFHEVMHost.s.sol:PrintFHEVMHostAddressesDotSol \
        --non-interactive
    exit 0
fi
...
env "${FORGE_ENV[@]}" forge script scripts/DeployCleartextFHEVMHost.s.sol:WriteFHEVMHostAddressesDotSol
...
signers_json="$(forge_json scripts/DeployCleartextFHEVMHost.s.sol:PrintFhevmSigners)"
...
env "${FORGE_ENV[@]}" forge script \
    scripts/DeployCleartextFHEVMHost.s.sol:Deploy \
    --non-interactive \
    --rpc-url "${rpc_url}" \
    --broadcast \
    --slow
...
env "${FORGE_ENV[@]}" forge script \
    scripts/DeployCleartextFHEVMHost.s.sol:Verify \
    --non-interactive \
    --rpc-url "${rpc_url}"
