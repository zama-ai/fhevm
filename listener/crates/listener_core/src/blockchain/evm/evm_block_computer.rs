...
impl EvmBlockComputer {
    ...
    pub fn verify_block(
        ...
        if allow_skipping {
            match Self::verify_transaction_root(block) {
                Ok(()) => {}
                Err(e) => {
                    metrics::counter!(
            ...
            match Self::verify_receipt_root(block, receipts) {
                Ok(()) => {}
                Err(e) => {
                    metrics::counter!(
            ...
            match Self::verify_block_hash(block) {
                Ok(()) => {}
                Err(e) => {
                    metrics::counter!(
            ...
            if let Err(e) = Self::verify_transaction_root(block) {
                metrics::counter!(
                    "listener_compute_transaction_failure_total",
                    "chain_id" => chain_id_str.clone(),
                    "stalling" => "true"
        ...
        match tx {
            ...
            AnyTxEnvelope::Unknown(unknown) => {
                ...
                match tx_type {
                    ...
                    // TODO: Arbitrum specific encoding for transactions.
                    // 100 => encode_arbitrum_deposit_transaction(unknown, index),
                    // 101 => encode_arbitrum_unsigned_transaction(unknown, from, index),
                    // 102 => encode_arbitrum_contract_transaction(unknown, index),
                    // 104 => encode_arbitrum_retry_transaction(unknown, from, index),
                    // 105 => encode_arbitrum_submit_retryable_transaction(unknown, index),
