...
pub struct SemEvmRpcProvider {
    ...
    rpc_url: String,
    /// Sanitized host extracted from `rpc_url` (via `Url::host_str()`) used as
    /// the `endpoint` label on Prometheus metrics. Only the host is emitted —
    /// the URL path (which may contain API keys) is deliberately NOT exposed.
    endpoint: Arc<String>,
...
impl SemEvmRpcProvider {
    ...
    pub fn new(rpc_url: String, max_concurrent_requests: usize) -> Result<Self, RpcProviderError> {
        ...
        let endpoint = Arc::new(url.host_str().unwrap_or("unknown").to_owned());
        ...
        let result: T = client
            ...
            .map_err(|e| {
                    ...
                    || err_msg.contains("data did not match")
                {
                    ...
                    record_rpc_error(&method_owned, endpoint.as_str(), "deserialization", elapsed);
                    ...
                    || err_msg.contains("429")
                {
                    ...
                    record_rpc_error(&method_owned, endpoint.as_str(), "rate_limited", elapsed);

                    return RpcProviderError::RateLimited(err_msg);
                }

                record_rpc_error(&method_owned, endpoint.as_str(), "transport", elapsed);
    ...
    pub async fn get_transaction_receipts_batch(
        ...
        const BATCH_METHOD: &str = "eth_getTransactionReceipt_batch";
        ...
        let response = self
            ...
            .map_err(|e| {
                ...
                RpcProviderError::BatchError(format!("HTTP request failed: {}", e))
            })?;
        ...
        let response_text = response.text().await.map_err(|e| {
            ...
            RpcProviderError::BatchError(format!("Failed to read response: {}", e))
        })?;
...
/// Record RPC error metrics (shared by `raw_request` and `get_transaction_receipts_batch`).
fn record_rpc_error(method: &str, endpoint: &str, error_kind: &'static str, elapsed_secs: f64) {
    metrics::counter!(
        "listener_rpc_requests_total",
        "method" => method.to_owned(),
