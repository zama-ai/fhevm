...
pub enum PublisherError {
    ...
    #[error("Broker error during publish: {message}")]
    BrokerError { message: String },

    #[error("Filter fetch failed: {message}")]
    FilterFetchError { message: String },
}
...
/// advance DB state. The handler framework retries the entire block, which
/// guarantees at-least-once delivery to all consumers.
///
/// Broker-level retries (3 for Redis, 10 for AMQP with exponential backoff)
/// are already exhausted before the error reaches this function. Retrying
/// at this layer would be redundant and masks infrastructure failures from
/// the circuit breaker.
pub async fn publish_block_events(
    ...
    let filters = repositories
        .filters
        .get_filters_by_chain_id()
        .await
        .map_err(|e| {
            error!(error = %e, "Failed to fetch filters");
