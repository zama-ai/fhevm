...
use crate::config::{BlockchainConfig, PublishConfig};
...
pub enum CursorResult {
    ...
    /// Skip task to avoid deduplication
    Skip,
}
...
impl EvmListener {
    ...
    pub async fn validate_strategy_and_init_block(&self, block_start: u64) {
    ...
    pub async fn fetch_blocks_and_run_cursor(&self) -> Result<CursorResult, EvmListenerError> {
        ...
        let task_lock = self
            .broker
            .is_empty_or_pending(&fetch_topic, routing::FETCH_NEW_BLOCKS)
            .await
            .map_err(|e| {
                error!(error = %e, "Failed to check message processing lock on cursor process.");
                EvmListenerError::MessageProcessingError {
                    message: "Couldn't check message before processing".to_string(),
                }
            })?;

        let backtrack_topic = Topic::new(routing::BACKTRACK_REORG)
            .with_namespace(chain_id_to_namespace(self.chain_id));
        let backtrack_task_lock = self
            .broker
            .is_empty(&backtrack_topic, routing::BACKTRACK_REORG)
            .await
            .map_err(|e| {
                error!(error = %e, "Failed to check message processing lock on cursor process.");
                EvmListenerError::MessageProcessingError {
        ...
        if !task_lock || !backtrack_task_lock {
            warn!("Cursor: Duplicate message in the queue, skipping");
            return Ok(CursorResult::Skip);
        }
        ...
        if chain_height <= db_block_number {
            ...
            // Publish wake-up signal to trigger next fetch iteration
            self.publisher
        ...
        match (cursor_outcome, fetcher_outcome) {
            ...
            (
                ...
                // Publish the backtrack event — no DB mutation here.
                // Handled in reorg_backtrack function
                ...
                info!(
                    ...
                    "Reorg detected — backtrack event published"
                );
    ...
    pub async fn reorg_backtrack(
        ...
        let task_lock = self
            .broker
            .is_empty_or_pending(&reorg_topic, routing::BACKTRACK_REORG)
            .await
            .map_err(|e| {
                error!(error = %e, "Failed to check message processing lock on reorg process.");
                EvmListenerError::MessageProcessingError {
                    message: "Couldn't check message before processing".to_string(),
                }
            })?;
        ...
        if !task_lock {
            warn!("Reorg: Duplicate message in the queue, skipping");
            return Ok(());
        }
        ...
        // ══════════════════════════════════════════════════════════════
        // Phase 3: Resume cursor via FETCH_NEW_BLOCKS.
        // ══════════════════════════════════════════════════════════════
        ...
        self.publisher
            ...
            .await
            .map_err(|e| {
                error!(error = %e, "Failed to publish fetch trigger after backtrack v2");
                EvmListenerError::BrokerPublishError {
                    message: format!("Broker publish failed: {}", e),
                }
            })?;
