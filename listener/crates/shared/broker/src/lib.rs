...
impl Broker {
    ...
    /// Lightweight connectivity probe for the broker connection.
    ///
    /// - **Redis**: sends a `PING` via the shared connection manager.
    /// - **AMQP**: opens a throwaway channel, which internally requires
    ///   `conn.status().connected()`. The channel is dropped immediately.
    ///
    /// Does not touch any queue/stream. Intended for `/readyz`-style probes
    /// that must fail fast when the broker is unreachable.
    pub async fn health_check(&self) -> Result<(), BrokerError> {
        match self {
            #[cfg(feature = "redis")]
            Broker::Redis { conn, .. } => Ok(conn.health_check().await?),
            #[cfg(feature = "amqp")]
            Broker::Amqp { conn, .. } => {
                let _ch = conn.create_channel().await?;
                Ok(())
            }
        }
    }
