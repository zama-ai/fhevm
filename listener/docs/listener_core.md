...
2. We spawn parallel task to fetch blocks (http polling), and register them in an in memory datastructure (slots for new blocks). And we fetch receipt for those blocks (strategy pattern could be required for different chains implems (`eth_getBlockReceipts` or `eth_getTransactionReceipt`for each transaction))
