...
// we could have error due to node inconsistant state during gas estimation which we want to retry (readiness check has passed on some state).
// It could nonetheless introduce some latency due to multiple retries if this never pass, and goes to a failure mode with terminaison.
