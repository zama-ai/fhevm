...
function handleSolidityBinaryOperatorForImpl(op: Operator): string {
  ...
  return (
    ...
    function ${op.name}(bytes32 lhs, bytes32 rhs${scalarArg}) internal returns (bytes32 result) {
        ${scalarSection}
        CoprocessorConfigStruct storage $ = getCoprocessorConfig();
 ...
 */
export function generateSolidityImplLib(operators: Operator[], fheTypeBaseDir: string): string {
  const res: string[] = [];
...
import {FheType} from "${fheTypeBaseDir}/FheType.sol";
...
library Impl {
   ...
   */
  function getCoprocessorConfig() internal pure returns (CoprocessorConfigStruct storage $) {
      assembly {
...
function generateImplCoprocessorInterface(operators: Operator[]): string {
  ...
  res.push(`
     ...
     * @notice  This struct contains the the internal counter for requestIDs generated by the dapp, 
...
export function generateSolidityFHELib(operators: Operator[], fheTypes: FheType[], fheTypeBaseDir: string): string {
  const res: string[] = [];
