...
contract ProtocolConfig is IProtocolConfig, UUPSUpgradeableEmptyProxy, ACLOwnable {
    ...
    struct ProtocolConfigStorage {
        ...
        /// @notice Decryption threshold per context.
        mapping(uint256 contextId => uint256) contextDecryptionThreshold;
    ...
    /// @inheritdoc IProtocolConfig
    function getDecryptionThreshold() external view virtual returns (uint256) {
        ProtocolConfigStorage storage $ = _getProtocolConfigStorage();
        return $.contextDecryptionThreshold[$.currentKmsContextId];
    }
        ...
        returns (uint256 newContextId)
    {
        ...
        $.contextDecryptionThreshold[newContextId] = thresholds.decryptionThreshold;
        $.contextKmsGenThreshold[newContextId] = thresholds.kmsGenThreshold;
    ...
    function _validateThresholds(KmsThresholds calldata thresholds, uint256 nodeCount) internal pure virtual {
        if (thresholds.decryptionThreshold == 0) {
            revert InvalidNullDecryptionThreshold();
        }
        if (thresholds.decryptionThreshold > nodeCount) {
            revert InvalidHighDecryptionThreshold(thresholds.decryptionThreshold, nodeCount);
        }
        if (thresholds.kmsGenThreshold == 0) {
            revert InvalidNullKmsGenThreshold();
        }
        if (thresholds.kmsGenThreshold > nodeCount) {
            revert InvalidHighKmsGenThreshold(thresholds.kmsGenThreshold, nodeCount);
        }
    }

    /**
     * @dev Checks whether a context ID is within the allocated range.
     */
    ...
    function _isValidKmsContext(uint256 kmsContextId) internal view virtual returns (bool) {
        ProtocolConfigStorage storage $ = _getProtocolConfigStorage();
        // A valid context must have at least one stored node. This also keeps migration gap IDs
        // invalid when initializeFromMigration preserves a legacy context ID above BASE + 1.
        return _contextIdInRange(kmsContextId) && $.contextKmsNodes[kmsContextId].length != 0
