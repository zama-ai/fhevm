...
contract ProtocolConfig is IProtocolConfig, UUPSUpgradeableEmptyProxy, ACLOwnable {
    ...
    /// @dev The DAO must not open a switch while another is in flight: at most one context and one
    ///      epoch may be non-active (Pending/Created) at a time. Settle the in-flight one first, either
    ///      by completing it (confirmEpochActivation) or aborting it (abortPendingContext / abortPendingEpoch).
    ...
    function confirmEpochActivation(
        ...
        // leaving the epoch Pending until the signers converge on a single result, or governance calls
        // abortPendingEpoch / abortPendingContext.
     ...
     *      context-creation bookkeeping. Used by `abortPendingContext`; the caller owns
     *      the preconditions and the event emitted.
     */
