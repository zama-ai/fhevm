...
contract ProtocolConfig is IProtocolConfig, UUPSUpgradeableEmptyProxy, ACLOwnable {
     ...
     *      needed for the migration. Unlike `initializeFromEmptyProxy`, this path emits no
     *      `NewKmsContext` event and therefore records no `KmsContextAnchor`: an anchor only exists
    ...
    function initializeFromMigration(
        ...
        KmsNodeParams[] calldata existingKmsNodeParams,
    ...
    function defineNewKmsContextAndEpoch(
        ...
        _requireNoInFlightSwitch();

        // Register the new signer set and its first epoch as Pending.
    ...
    function defineNewEpochForCurrentKmsContext() external virtual onlyACLOwner {
        ...
        // Register the new epoch as Pending.
        uint256 contextId = $.activeKmsContextId;
        uint256 epochId = ++$.epochCounter;
        $.epochState[epochId] = EpochState.Pending;
        $.contextForEpoch[epochId] = contextId;
    ...
    function confirmKmsContextCreation(uint256 kmsContextId) external virtual {
        ...
        if (_hasContextCreationQuorum(kmsContextId)) {
            ...
            // The context-switch created this context and its epoch as a paired (Pending, Pending), and
            // _requireNoInFlightSwitch forbids issuing a new epoch mid-switch, so the latest-issued epoch
            // must still be this context's Pending epoch here.
            uint256 epochId = $.epochCounter;
    ...
    function confirmEpochActivation(
        ...
        // All signers agreed, promote epoch + context to Active.
    ...
    function _storeAndActivateKmsContext(
        ...
        uint256 epochId = ++$.epochCounter;
        $.epochState[epochId] = EpochState.Active;
        $.contextForEpoch[epochId] = newContextId;
        $.activeEpochId = epochId;
    }
    ...
    /**
     * @dev Reverts unless both the latest-issued context and epoch are settled, i.e. no switch is
     *      in flight. The context is unsettled while still `Pending`/`Created` and the epoch while
     *      still `Pending`. Relies on the invariant that at most one context and one epoch are
     *      non-active at a time, so the latest-issued of each is the only candidate to check.
     */
    function _requireNoInFlightSwitch() internal view virtual {
        ProtocolConfigStorage storage $ = _getProtocolConfigStorage();
        ContextState s = $.contextState[$.currentKmsContextId];
        if (s == ContextState.Pending || s == ContextState.Created) {
            revert PendingKmsContextAlreadyExists($.currentKmsContextId);
        }
        if ($.epochState[$.epochCounter] == EpochState.Pending) {
            revert PendingEpochAlreadyExists($.epochCounter);
        }
    }
     ...
     * check also keeps migration gap IDs invalid when `initializeFromMigration` preserves
     * a legacy context ID above `BASE + 1`.
     */
    ...
    function _clearContext(uint256 kmsContextId) internal virtual {
        ...
        if ($.epochState[latestEpochId] == EpochState.Pending && $.contextForEpoch[latestEpochId] == kmsContextId) {
            $.epochState[latestEpochId] = EpochState.None;
            delete $.contextForEpoch[latestEpochId];
        }
