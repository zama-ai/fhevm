 ...
 */
contract KMSGeneration is IKMSGeneration, IKMSGenerationMigration, EIP712Upgradeable, UUPSUpgradeableEmptyProxy, ACLOwnable {
    // ----------------------------------------------------------------------------------------------
    ...
    uint256 private constant PREP_KEYGEN_COUNTER_BASE = uint256(3) << REQUEST_TYPE_SHIFT;
    uint256 private constant KEY_COUNTER_BASE = uint256(4) << REQUEST_TYPE_SHIFT;
    uint256 private constant CRS_COUNTER_BASE = uint256(5) << REQUEST_TYPE_SHIFT;
    ...
    struct KMSGenerationStorage {
        ...
        /// @notice Whether a request was explicitly aborted and therefore has no retrievable materials.
        mapping(uint256 requestId => bool) isAborted;
    }
    ...
    function keygen(ParamsType paramsType) external virtual onlyACLOwner {
        ...
        $.requestParamsType[keyId] = paramsType;
    ...
    function keygenResponse(
        ...
        if (prepKeygenId != 0 && !$.isRequestDone[prepKeygenId]) {
            revert KeyManagementRequestPending();
        }

        bytes32 digest;
        {
            bytes memory extraData = $.requestExtraData[keyId];

            // Compute the digest of the KeygenVerification struct.
            digest = _hashKeygenVerification(prepKeygenId, keyId, keyDigests, extraData);
        }
        ...
        if (!$.isRequestDone[keyId] && _isKmsConsensusReached(consensusTxSendersLength)) {
            ...
            string[] memory consensusUrls =
                _buildConsensusStorageUrls(contextId, consensusTxSenders, consensusTxSendersLength);
    ...
    function crsgenResponse(
        ...
        bytes32 digest;
        {
            ...
            bytes memory extraData = $.requestExtraData[crsId];

            // Compute the digest of the CrsgenVerification struct.
            digest = _hashCrsgenVerification(crsId, maxBitLength, crsDigest, extraData);
        }
    ...
    function getKeyParamsType(uint256 keyId) external view virtual returns (ParamsType) {
        ...
        if (!_hasConsensusMaterials($, keyId)) {
            revert KeyNotGenerated(keyId);
        }
    ...
    function getCrsParamsType(uint256 crsId) external view virtual returns (ParamsType) {
        ...
        if (!_hasConsensusMaterials($, crsId)) {
            revert CrsNotGenerated(crsId);
        }
    ...
    function getKeyMaterials(uint256 keyId) external view virtual returns (string[] memory, KeyDigest[] memory) {
        ...
        uint256 consensusTxSendersLength = consensusTxSenders.length;
        if (consensusTxSendersLength == 0) {
            revert KeyNotGenerated(keyId);
        }

        // Resolve the context under which this key was generated so that storage URLs remain
        // correct even after a committee rotation.
        uint256 contextId = _extractContextIdFromExtraData($.requestExtraData[keyId]);
        string[] memory consensusUrls =
            _buildConsensusStorageUrls(contextId, consensusTxSenders, consensusTxSendersLength);
    ...
    function getCrsMaterials(uint256 crsId) external view virtual returns (string[] memory, bytes memory) {
        ...
        // Resolve the context under which this CRS was generated.
        uint256 contextId = _extractContextIdFromExtraData($.requestExtraData[crsId]);
        string[] memory consensusUrls =
            _buildConsensusStorageUrls(contextId, consensusTxSenders, consensusTxSendersLength);
     ...
     * @param consensusTxSendersLength The number of tx senders.
     */
    function _buildConsensusStorageUrls(
        uint256 contextId,
        address[] memory consensusTxSenders,
        uint256 consensusTxSendersLength
    ) internal view virtual returns (string[] memory) {
        string[] memory urls = new string[](consensusTxSendersLength);
    ...
    /**
     * @dev Checks whether a request has consensus materials recorded.
     */
    function _hasConsensusMaterials(KMSGenerationStorage storage $, uint256 requestId)
        internal
        view
        virtual
        returns (bool)
    {
        bytes32 digest = $.consensusDigest[requestId];
        return $.consensusTxSenderAddresses[requestId][digest].length != 0;
    }
    ...
    /// @custom:oz-upgrades-validate-as-initializer
    function initializeFromMigration(IKMSGenerationMigration.MigrationState calldata state)
        public
        virtual
        onlyACLOwner
        reinitializer(REINITIALIZER_VERSION)
    {
        ...
        require(state.prepKeygenCounter >= PREP_KEYGEN_COUNTER_BASE);
        require(state.keyCounter >= KEY_COUNTER_BASE);
        require(state.crsCounter >= CRS_COUNTER_BASE);
        ...
        if (state.activePrepKeygenId != 0 && state.activeKeyIdForPairing != 0) {
            $.keygenIdPairs[state.activePrepKeygenId] = state.activeKeyIdForPairing;
            $.keygenIdPairs[state.activeKeyIdForPairing] = state.activePrepKeygenId;
        }

        // Key digests
        if (state.activeKeyId != 0) {
        ...
        _setConsensusState(
            ...
            state.prepKeygenExtraData,
            state.isPrepKeygenDone
        );

        // isRequestDone flags
        if (state.activePrepKeygenId != 0) {
            $.isRequestDone[state.activePrepKeygenId] = state.isPrepKeygenDone;
        }
        if (state.activeKeyId != 0) {
            $.isRequestDone[state.activeKeyId] = state.isKeygenDone;
        }
        if (state.activeCrsId != 0) {
            $.isRequestDone[state.activeCrsId] = state.isCrsgenDone;
        }
        ...
        if (state.activeKeyId != 0) {
            // Keygen shares the same params type as its paired prep-keygen request.
            $.requestParamsType[state.activeKeyId] = state.prepKeygenParamsType;
        }
        if (state.activeCrsId != 0) {
        ...
        // Extra data
        if (state.activePrepKeygenId != 0 && state.prepKeygenExtraData.length > 0) {
            $.requestExtraData[state.activePrepKeygenId] = state.prepKeygenExtraData;
        }
        if (state.activeKeyId != 0 && state.keygenExtraData.length > 0) {
            $.requestExtraData[state.activeKeyId] = state.keygenExtraData;
        }
        if (state.activeCrsId != 0 && state.crsgenExtraData.length > 0) {
            $.requestExtraData[state.activeCrsId] = state.crsgenExtraData;
        }
    }

    function _setConsensusState(
        ...
        if (requestId == 0 || txSenders.length == 0) return;
        if (isRequestDone) {
            $.consensusDigest[requestId] = digest;
        }
        uint256 contextId = _extractContextIdFromExtraData(extraData);
