...
export async function buildEncryptedInput(args: BuildEncryptedInputArgs): Promise<EncryptedInputBundle> {
  const { contractAddress, userAddress, inputs, hostChainId, gatewayChainId, inputVerificationAddress, coprocessorSigners } = args;
...
export async function runMockEncryptCli(opts: CliOptions, db: MockDb): Promise<void> {
  ...
  if (bits === undefined) {
    throw new Error(
      `Unknown --type: ${opts.type}. Supported: ${Object.keys(TYPE_ALIASES).sort().join(', ')}`,
    );
  }
  ...
  console.error(
    `[mock-coprocessor:encrypt] inserted handle=${bundle.handles[0]} cleartext=${bundle.cleartexts[0]} ` +
      `into ${db ? 'mock DB' : '<no db>'} for { contract=${opts.contract}, user=${opts.user}, hostChainId=${opts.hostChainId} }`,
  );
}
...
function numericEnv(name: string): number {
  ...
  if (!Number.isInteger(n) || n < 0) throw new Error(`${name} must be a non-negative integer (got ${process.env[name]})`);
  return n;
}
