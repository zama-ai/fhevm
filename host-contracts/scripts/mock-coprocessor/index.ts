...
import { runMockEncryptCli, CliOptions } from './input';
...
async function queryHandle(handleRaw: string): Promise<void> {
  ...
  await db.close();
  if (value === null) {
    console.error(`Handle ${handle} not present in mock DB. The daemon may still be catching up, or the handle was never produced.`);
    process.exit(1);
  }
