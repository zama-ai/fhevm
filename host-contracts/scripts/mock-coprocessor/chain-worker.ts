...
import { applyExecutorEvent, ExecutorEvent } from './handlers/fhe-executor';
import { applyBridgeEvent, BridgeEvent, pendingBridgeCount, retryPendingBridges } from './handlers/bridge';
...
export async function runChainWorker(chain: ChainConfig, db: MockDb, abort: AbortSignal): Promise<void> {
  ...
  logger.info(
    `starting: rpc=${chain.rpcUrl} executor=${chain.fhevmExecutor} bridge=${chain.confidentialBridge}`,
  );
