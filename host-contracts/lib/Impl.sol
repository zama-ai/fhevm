...
interface IFHEVMExecutor {
    ...
    /**
     * @notice                  High-arity sample multi-output op (stress test).
     *                          Returns 100 handles derived from one base hash.
     * @param ct                Ciphertext input.
     * @return results          Output handles 0..99 (each = `ct + (i + 1)`).
     */
    function fheSampleMultiOutput100(bytes32 ct) external returns (bytes32[] memory results);
...
library Impl {
    ...
    function sampleMultiOutput100(bytes32 ct) internal returns (bytes32[] memory results) {
        CoprocessorConfig storage $ = getCoprocessorConfig();
        results = IFHEVMExecutor($.CoprocessorAddress).fheSampleMultiOutput100(ct);
    }
    ...
    function mulDiv(bytes32 lhs, bytes32 rhs, bytes32 divisor, bool scalar) internal returns (bytes32 result) {
        CoprocessorConfig storage $ = getCoprocessorConfig();
        bytes1 scalarByte = scalar ? bytes1(0x01) : bytes1(0x00);
        result = IFHEVMExecutor($.CoprocessorAddress).fheMulDiv(lhs, rhs, divisor, scalarByte);
    }
