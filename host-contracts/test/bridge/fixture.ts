...
export async function deployBridgeFixture() {
  ...
  const srcBridge = await _deployBridgeProxy(
    owner,
    await srcEndpoint.getAddress(),
    [DST_EID],
    [DST_CHAIN_ID],
  );
  const dstBridge = await _deployBridgeProxy(owner, await dstEndpoint.getAddress(), [], []);
...
async function _deployBridgeProxy(
  bridgeOwner: HardhatEthersSigner,
  lzEndpoint: string,
  dstEids: number[],
  dstChainIds: bigint[],
  ...
  const bridge = (await upgrades.upgradeProxy(await emptyProxy.getAddress(), bridgeFactory, {
    ...
    call: { fn: 'initializeFromEmptyProxy', args: [bridgeOwner.address, dstEids, dstChainIds] },
