...
contract KMSGenerationHarness is KMSGeneration {
    ...
    function decodeExtraData(bytes memory extraData) external view returns (uint256 contextId, uint256 epochId) {
        return _extractContextIdAndEpochIDFromExtraData(extraData);
    }
}
...
contract KMSGenerationTest is HostContractsDeployerTestUtils {
    ...
    function test_extractContextIdAndEpochIDFromExtraDataReturnsActiveContextAndEpochForV0() public view {
        (uint256 activeContextId, uint256 activeEpochId) = protocolConfig.getCurrentKmsContextAndEpoch();
        (uint256 decodedContextId, uint256 decodedEpochId) = kmsGenerationHarness.decodeExtraData("");

        assertEq(decodedContextId, activeContextId);
        assertEq(decodedEpochId, activeEpochId);
    }

    function test_extractContextIdAndEpochIDFromExtraDataReturnsActiveEpochForV1() public view {
        ...
        (uint256 decodedContextId, uint256 decodedEpochId) = kmsGenerationHarness.decodeExtraData(extraData);

        assertEq(decodedContextId, contextId);
        assertEq(decodedEpochId, activeEpochId);
    }
    ...
    function test_extractContextIdAndEpochIDFromExtraDataReturnsV2ContextAndEpoch() public view {
        (uint256 contextId, uint256 epochId) = protocolConfig.getCurrentKmsContextAndEpoch();
        bytes memory extraData = abi.encodePacked(uint8(0x02), contextId, epochId);
        ...
        assertEq(decodedContextId, contextId);
        assertEq(decodedEpochId, epochId);
    }
