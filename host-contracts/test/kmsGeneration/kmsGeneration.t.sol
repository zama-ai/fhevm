...
contract KMSGenerationTest is HostContractsDeployerTestUtils {
    ...
    function test_compressedKeyMigrationFullCycleKeepsActiveKey() public {
        ...
        bytes memory extraData = _buildExtraData();
        vm.expectEmit(true, true, true, true, address(kmsGeneration));
        emit IKMSGeneration.PrepKeygenRequest(migrationPrepId, IKMSGeneration.ParamsType.Default, keyId, extraData);
        vm.prank(owner);
        kmsGeneration.keygen(IKMSGeneration.ParamsType.Default, keyId);
    ...
    function test_abortMigrationAfterRequestEmissionAllowsRetry() public {
        (, uint256 keyId) = _runFullKeygenCycle();
        _switchToMultiSignerContext();
        ...
        uint256 retryRequestId = KEY_COUNTER_BASE + 3;
