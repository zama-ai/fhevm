...
const publicDecryptRequestMocked =
  ...
  async (_handles: (string | Uint8Array<ArrayBufferLike>)[]): Promise<PublicDecryptResults> => {
    ...
    if (process.env.DISABLE_TEST_KMS_THRESHOLD !== '1') {
...
////////////////////////////////////////////////////////////////////////////////
// Env Signers (using env variables)
////////////////////////////////////////////////////////////////////////////////
...
// No env
async function getHostKMSThreshold(kmsContractAddress: `0x${string}`): Promise<bigint> {
  const abiKmsVerifier = ['function getThreshold() view returns (uint256)'];
  const kmsContract = new ethers.Contract(kmsContractAddress, abiKmsVerifier, hre.ethers.provider);
  return BigInt(await kmsContract.getThreshold());
}

// env function for testing
