...
const abi = [
  ...
  'event ValidateInput(address indexed caller, bytes32 inputHandle, address userAddress, bytes inputProof, uint8 inputType, bytes32 result)',
...
export function getTxHCUFromTxReceipt(
  ...
  const relevantLogs = receipt.logs.filter((log: ethers.Log) => {
    ...
    try {
      ...
      return abi.some((item) => item.startsWith(`event ${parsedLog.name}`) && parsedLog.name !== 'ValidateInput');
    } catch {
      return false;
    }
  });
