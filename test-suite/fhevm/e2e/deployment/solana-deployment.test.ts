...
const deploy = (action = 'deploy', overrides: Record<string, string> = {}, target = 'host') => {
  ...
  if (image && process.platform === 'darwin') {
    for (const name of ['SOLANA_RPC_URL', 'GATEWAY_RPC_URL', 'DATABASE_URL']) {
      if (variables[name])
        variables[name] = variables[name]!.replace(/127\.0\.0\.1|localhost/g, 'host.docker.internal');
    }
  }
  return run(
    image
...
test('coprocessor register connects to PostgreSQL and preserves registration on retry', async () => {
  const sql = (input: string) =>
    run(['docker', 'exec', '-i', container, 'psql', '-U', 'postgres', '-At', '-v', 'ON_ERROR_STOP=1'], { input });
  // The real coprocessor schema: its unique index on keys decides what a retry may insert.
  const migrations = path.join(REPO_ROOT, 'coprocessor/fhevm-engine/db-migration/migrations');
  await run(['docker', 'cp', migrations, `${container}:/migrations`]);
  await run([
    'docker',
    'exec',
    container,
    'sh',
    '-ec',
    'for f in /migrations/*.sql; do psql -U postgres -q -v ON_ERROR_STOP=1 -f "$f"; done',
  ]);
  ...
  expect(missingKeys.stderr).toContain('Canonical host key material is not ready');
  expect((await sql('SELECT count(*) FROM host_chains')).stdout.trim()).toBe('0');
  // Legacy key rows keep a NULL block_hash (migration 20260421160000 backfills only chain_id).
  await sql(
    "INSERT INTO keys (chain_id,key_id,key_id_gw,pks_key,sks_key,block_hash) VALUES (12345,'\\x01','\\x02','\\x03','\\x04',NULL)",
  );
