...
try {
  ...
  if (env.network === 'devnet') {
    try {
      await recoverPreview(createHostDeployContext(env.rpcUrl), await loadKeypairSigner(env.roots.deployerKeypairPath), DEFAULT_SOLANA_ENVIRONMENT, recoveryDirectory(), false, env.roots.deployerKeypairPath);
    } catch {
