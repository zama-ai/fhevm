...
export const verticalSetup = async (): Promise<VerticalTestSetup> => {
  ...
  await context.fundSol(wallet.signer.address, env.funding.primarySol);
  ...
  // Connector validates each permit's signed pair against. The local stack runs the test FHE
  // parameter set.
