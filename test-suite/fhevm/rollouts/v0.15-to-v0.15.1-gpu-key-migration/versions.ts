...
export const migrationVersions = (env: Env = process.env): MigrationVersions => {
  ...
  const greenSha = required(env, "RFC029_GREEN_SHA");
  if (!/^[0-9a-f]{40}$/i.test(greenSha)) {
    throw new Error("RFC029_GREEN_SHA must be a full commit SHA");
  }
