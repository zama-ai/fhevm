...
| `rollouts/v0.13-to-v0.14/run.ts`                        | `two-of-three-multi-chain-threshold-kms` | v0.13.2 → v0.14 in the documented component order, holding the client on relayer-sdk 0.4.4 until the last phase so each backend component crosses on its own: the mandatory kms-core PRSS bridge stop, KMS nodes upgraded one at a time with their own connectors, coprocessor operators one at a time, and the canonical ProtocolConfig mirror across two host chains. |
...
to the phase's full checkpoint list.
