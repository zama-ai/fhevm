...
The PR e2e workflow boots `latest-main` with the checked-in `two-of-two` scenario and forces `--build`, then runs `test standard` followed by the operators-only `test heavy` lane against that stack.
...
- CI follows the same contract: direct PR e2e boots `latest-main --build` with the checked-in `two-of-two` scenario and runs `test standard` plus the operators-only `test heavy` lane, while orchestrated e2e boots the same scenario with `build=false` and overlays selected `*_VERSION` image refs.
