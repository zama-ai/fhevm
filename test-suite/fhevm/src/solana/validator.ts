 ...
 */
export const startGeyserValidator = async (options: ValidatorStartOptions): Promise<RunningValidator> => {
  ...
  return { pid: proc.pid, ledgerDir, logPath };
};
 ...
 * program IDs.
 */
export const seedProgramKeypairs = async (): Promise<void> => {
  const deployDir = path.join(SOLANA_DIR, "target", "deploy");
