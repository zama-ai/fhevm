...
const abortKeygenMidFlight = async (state: State, target: Target, owner: Owner, abi: AbiHashes, paramsType: string, baselineKeyId: bigint) => {
  ...
  const trigger = await castSend(target.rpcUrl, target.kmsGenerationAddress, owner, "keygen(uint8,uint256)", paramsType, "0");
  ...
  await callContractAndExpectRevert(
    target.rpcUrl, target.kmsGenerationAddress, owner,
