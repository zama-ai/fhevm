...
const TEST_PROFILE_DESCRIPTIONS: Partial<Record<(typeof TEST_PROFILE_NAMES)[number], string>> = {
  ...
  "kms-context-switch":
    "Drive the NewKmsContext + NewKmsEpoch lifecycle on the host ProtocolConfig and prove the KMS reshares, activates, and still decrypts under each, with the input-proof app smoke at baseline, while the switch is pending, and after each transition. On a same-committee cluster it then destroys the retired context and epoch (destroyKmsContext / destroyKmsEpoch) and proves every layer retires them — reverts for non-owner/current/unknown/already-destroyed ids, on-chain invalidation without moving the active pointer, per-party connector DestroyMpcContext/DestroyMpcEpoch forwarding and cache invalidation, and the current context/epoch still serving. On a cluster with a spare core (e.g. --scenario swap-threshold-kms) the NewKmsContext step is instead a genuine node swap — stop a committee node's tx-sender before the switch so it cannot confirm on-chain, promote the spare, and force it into the 2t+1 quorum; the destroy phase is skipped there (the retired context's committee no longer matches the party set) (threshold-mode KMS). Disruptive + single-run: it advances the context/epoch and destroys the retired ones, so it must start from a pristine stack — re-up between runs.",
};
