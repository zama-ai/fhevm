...
describe("compat", () => {
  ...
  test("does not add gateway URL compat args for modern gateway-facing coprocessor services", () => {
    const policy = compatPolicyForState({
      versions: {
        target: "latest-main",
        lockName: "latest-main.json",
    ...
    expect(policy.coprocessorArgs["transaction-sender"]).toBeUndefined();
  });
