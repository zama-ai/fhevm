/**
 * NOTE: vendored into `kms-qa/` on purpose.
 *
 * This file is a copy of `src/kms-onchain.ts` as it exists on `main`. The release branch this profile
 * targets predates that module, and this workstream's convention is to duplicate rather than modify
 * or back-port shared code: a vendored copy cannot collide with anything the release already has, so
 * the profile stays purely additive and merges without conflict.
 *
 * Keep it a faithful copy. If `main`'s version changes in a way this profile needs, re-vendor rather
 * than editing here, so the divergence stays visible.
 */
/**
...
/**
 * Renders a uint256 as 64 hex chars, big-endian, unprefixed.
 *
 * Inlined rather than imported: the release branch has this helper in `utils/fs.ts` but does not
 * export it, and this workstream does not modify shared modules. One line, and keeping it here means
 * the vendored copy has no new coupling to the host branch.
 */
