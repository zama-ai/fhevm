...
const discover = (value: unknown) => {
  ...
  try {
    ...
    return Bun.spawnSync(["bun", "-e", `import { defaultEndpoints } from ${JSON.stringify(path.join(import.meta.dir, "flow/discovery.ts"))}; console.log(JSON.stringify(await defaultEndpoints()));`], {
      env: { ...process.env, PATH: `${directory}:${process.env.PATH}`, MINIO_INSPECT_FIXTURE: fixture },
      timeout: 10_000,
    });
  } finally { rmSync(directory, { recursive: true, force: true }); }
};

test("MinIO discovery survives its container IP being reassigned to a restarted worker", () => {
  for (const address of ["172.18.0.2", "172.18.0.37"]) {
    const result = discover(inspect(address));
    expect(result.exitCode, result.stderr.toString()).toBe(0);
    expect(JSON.parse(result.stdout.toString()).minioExternal).toBe("http://172.18.0.1:9000");
  }
});

test("MinIO discovery rejects an absent or ambiguous bridge gateway", () => {
...
test("MinIO discovery refuses a host-loopback-only or unpublished S3 port", () => {
