...
export const preflightPorts = (state: Pick<State, "scenario">) =>
  [...new Set([...PORTS, ...(scenarioUsesForkAnvil(state.scenario) ? [DEFAULT_FORK_RPC_PORT] : []), ...state.scenario.hostChains.map((chain) => chain.rpcPort)])];
...
/**
 * Boots the KMS core at the scenario's `kms.bootstrap.coreVersion` so the key material is
 * generated by a release the pinned Blue can read; the bootstrap step upgrades it to the lock.
 */
export const applyKmsBootstrap = (
  bundle: VersionBundle,
  scenario: State["scenario"],
): Pick<State, "versions" | "kmsBootstrapPending"> => {
  const bootstrap = scenario.kms.bootstrap;
  if (!bootstrap) {
...
const logKmsBootstrap = (state: Pick<State, "kmsBootstrapPending" | "versions">) => {
  if (state.kmsBootstrapPending) {
    console.log(
      `[kms-bootstrap] core=${state.versions.env.CORE_VERSION}; upgraded to the lock once the operators ingest the key material`,
    );
  }
};
...
export const previewStateFromBundle = (
  ...
  assertSupportedBundleScenario({ versions: boot.versions, overrides: options.overrides, scenario });
  return {
...
const bootstrapState = async (options: UpOptions) => {
  ...
  const boot = applyKmsBootstrap(resolved.bundle, scenario);
  assertBlueGreenKmsCompatibility(scenario, boot.versions);
  assertSupportedBundleScenario({ versions: boot.versions, overrides: options.overrides, scenario });
...
/**
 * Ends the bootstrap step: regenerates the runtime and, when the KMS core booted at
 * `kms.bootstrap.coreVersion`, upgrades it to the lock now that the key material is ingested.
 * Mutates `state` so the remaining steps and the step marker see the upgraded version.
 */
const completeBootstrap = async (state: State) => {
  ...
  const { state: locked } = await buildLockedState("kms bootstrap", state, state.lockPath, ["CORE_VERSION"], state.overrides);
  const plan = resolveUpgradePlan(locked, "kms-core", { lockFile: true });
  console.log(`[kms-bootstrap] key material ingested; upgrading the KMS core to ${locked.versions.env.CORE_VERSION}`);
  Object.assign(state, locked);
  delete state.kmsBootstrapPending;
  await executeUpgradePlan(state, plan);
};
