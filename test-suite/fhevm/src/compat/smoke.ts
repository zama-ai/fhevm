...
/** Builds an actionable compat-smoke failure message with maintainer guidance. */
const compatFailure = (message: string) =>
  `${message}\nRead ${COMPAT_DOC}.\nEither add/update a shim in src/compat/compat.ts or intentionally raise the support floor in src/resolve/target.ts.`;
...
const loadResolvedCompose = async (component: (typeof COMPAT_COMPONENTS)[number]) => {
  ...
  const plan = stackSpecForState(runtimeState);
