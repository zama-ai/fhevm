...
export const renderEnvMaps = (
  state: Pick<State, "discovery">,
  plan: RuntimePlan,
  templateEnvs: Record<string, Record<string, string>>,
  deriveWallet: (mnemonic: string, index: number) => Effect.Effect<WalletMaterial>,
) =>
  Effect.gen(function* () {
    ...
    if (state.discovery) {
      ...
      updateContracts(envs["coprocessor"], {
        ...
        ...(requiresMultichainAclAddress(plan)
    ...
    if (plan.topology.count > 1) {
      if (plan.topology.count > COPROCESSOR_WALLET_INDICES.length) {
        return yield* Effect.die(
          new Error(
            `Multicopro topology exceeds supported count ${COPROCESSOR_WALLET_INDICES.length}`,
          ),
        );
      }
      ...
      for (let index = 0; index < plan.topology.count; index += 1) {
        const wallet = yield* deriveWallet(mnemonic, COPROCESSOR_WALLET_INDICES[index]);
