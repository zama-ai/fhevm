...
const VALID_PROJECT_NAME = /^[a-z0-9][a-z0-9_-]*$/;
const WORKTREE_ID = process.cwd().match(/[\\/]worktrees[\\/](.+?)([\\/]|$)/)?.[1];
const DEFAULT_PROJECT = WORKTREE_ID ? `fhevm-${WORKTREE_ID}` : "fhevm";

function sanitizeProjectName(value: string): string {
  const lowered = value.toLowerCase().replace(/[^a-z0-9_-]/g, "-");
  const squashed = lowered.replace(/-+/g, "-");
  const trimmed = squashed.replace(/^-+/, "");
  if (trimmed === "") {
    return "fhevm";
  }
  return trimmed;
...
const projectCandidate = PROJECT_OVERRIDE && PROJECT_OVERRIDE !== "" ? PROJECT_OVERRIDE : DEFAULT_PROJECT;
const sanitizedProject = sanitizeProjectName(projectCandidate);
export const PROJECT = VALID_PROJECT_NAME.test(sanitizedProject) ? sanitizedProject : "fhevm";
process.env.FHEVM_DOCKER_PROJECT = PROJECT;
...
// Account index 0 is used by deployer paths; keep coprocessor signers on dedicated slots.
export const COPROCESSOR_ACCOUNT_INDICES: number[] = [5, 8, 9, 10, 11];
export const MAX_LOCAL_COPROCESSORS = COPROCESSOR_ACCOUNT_INDICES.length;

export const INPUT_PROOF_COMPUTE_DECRYPT_RETRY_ATTEMPTS = 4;
export const INPUT_PROOF_COMPUTE_DECRYPT_RETRY_DELAY_SECONDS = 10;
...
export const TEST_TYPE_CONFIG: Record<string, TestTypeConfig> = {
  ...
  "input-proof-compute-decrypt": {
    ...
    retryAttempts: INPUT_PROOF_COMPUTE_DECRYPT_RETRY_ATTEMPTS,
    retryDelaySeconds: INPUT_PROOF_COMPUTE_DECRYPT_RETRY_DELAY_SECONDS,
  },
  "user-decryption": {
