...
COMPOSE_PROJECT="${FHEVM_DOCKER_PROJECT:-fhevm}"
MINIO_API_PORT="${MINIO_API_PORT:-9000}"

resolve_kms_core_container() {
    local preferred="${COMPOSE_PROJECT}-kms-core"
    local by_project=""

    if docker ps -a --format '{{.Names}}' | grep -qx "$preferred"; then
        echo "$preferred"
        return
    fi

    by_project="$(docker ps -a \
        --filter "label=com.docker.compose.project=${COMPOSE_PROJECT}" \
        --format '{{.Names}}' \
        | grep 'kms-core' \
        | head -n1 || true)"
    if [ -n "$by_project" ]; then
        echo "$by_project"
        return
    fi
...
KMS_CORE_CONTAINER="$(resolve_kms_core_container)"
if [ -z "$KMS_CORE_CONTAINER" ]; then
    log_error "Failed to locate a kms-core container for project '${COMPOSE_PROJECT}'."
    exit 1
fi

log_info "Using kms-core container: ${KMS_CORE_CONTAINER}"

extract_signing_key_id() {
    docker logs "$KMS_CORE_CONTAINER" 2>&1 \
...
if [ -z "$SIGNER_ADDRESS" ]; then
    ...
    for attempt in $(seq 1 "$MAX_SIGNER_ADDRESS_ATTEMPTS"); do
        for base_path in "PUB/PUB/VerfAddress" "PUB/VerfAddress"; do
            SIGNER_ADDRESS_URL="http://localhost:${MINIO_API_PORT}/kms-public/${base_path}/${KEY_SIGNER_ID}"
