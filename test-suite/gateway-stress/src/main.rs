...
async fn run() -> anyhow::Result<()> {
    let cli = Cli::parse();
    let mut config = Config::from_env_and_file(cli.config.as_ref())?;
    ...
    match cli.subcommand {
        Subcommands::Public => {
            ...
            test_manager.public_decryption_stress_test().await?
        }
        Subcommands::User => {
            let test_manager = GatewayTestManager::connect(config).await?;
            test_manager.user_decryption_stress_test().await?
        }
        Subcommands::Benchmark(args) => {
            let test_manager = GatewayTestManager::connect(config).await?;
