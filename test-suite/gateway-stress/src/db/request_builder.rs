use crate::decryption::types::{DecryptionRequest, DecryptionType};
...
pub struct RequestBuilder {
    id_counter: U256,
    user_ct_handles: Vec<FixedBytes<32>>,
    public_ct_handles: Vec<FixedBytes<32>>,
    ct_digest: FixedBytes<32>,
    key_id: U256,
...
impl RequestBuilder {
    ...
    fn build_public_request(&mut self) -> anyhow::Result<PublicDecryptionRequest> {
        ...
        let sns_ct_materials = self.generate_sns_materials(self.public_ct_handles.clone());
    ...
    fn build_user_request(&mut self) -> anyhow::Result<UserDecryptionRequest> {
        let decryption_id = self.generate_unique_id();
        let sns_ct_materials = self.generate_sns_materials(self.user_ct_handles.clone());
    ...
    fn generate_sns_materials(
        ...
        ct_handles
            .iter()
            .map(|h| SnsCiphertextMaterial {
                ctHandle: *h,
                keyId: self.key_id,
                snsCiphertextDigest: self.ct_digest,
