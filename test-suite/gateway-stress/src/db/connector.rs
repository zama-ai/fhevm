...
impl DbConnector {
    ...
    async fn insert_user_requests(
        ...
        ");
        query_builder.push_values(requests, |mut bind, req| {
            bind.push_bind(req.decryptionId.to_le_bytes_vec())
                .push_bind(
                    req.snsCtMaterials
                        .iter()
        ...
        let query = query_builder.build();
        let query_results = self.db_pool.fetch_all(query).await?;
        trace!("Inserted {} user decryption requests", query_results.len());

        let requests_metadata = query_results.into_iter().map(|r| r.into()).collect();
        Ok(requests_metadata)
    }
}
