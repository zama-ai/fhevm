...
import { createInstance, sleep, zip, timeout } from './shared';
...
`)
  ...
  .setAction(async (taskArgs: TaskArgs, hre: HardhatRuntimeEnvironment) => {
    ...
    if (taskArgs.benchmarkContractAddress == null) {
      ...
      const contractFactory = await ethers.getContractFactory('PublicDecryptBenchmark');
      contract = contractFactory.attach(taskArgs.benchmarkContractAddress).connect(signer) as PublicDecryptBenchmark;
    ...
    while (true) {
      ...
      for (let index = 0; index < taskArgs.decryptions; index++) {
        ...
        decrypted_values_promises.push(instance.publicDecrypt([handles[index]]));
