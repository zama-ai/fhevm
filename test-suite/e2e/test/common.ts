...
export function validateFHETypes(fheTypes: FheType[]): void {
  ...
  fheTypes.forEach((fheType) => {
    if (
      typeof fheType.type !== "string" ||
      typeof fheType.clearMatchingType !== "string"
    ) {
      throw new Error(`Invalid FHE type: ${JSON.stringify(fheType)}`);
    }

    if (
      !Array.isArray(fheType.supportedOperators) ||
      fheType.supportedOperators.some((op) => typeof op !== "string")
    ) {
      throw new Error(
        `Invalid supportedOperators for FHE type: ${fheType.type}`
      );
    }
    ...
    if (fheType.aliases) {
      fheType.aliases.forEach((alias) => {
        ...
        if (
          ...
          throw new Error(
            `Invalid supportedOperators for alias: ${alias.type}`
          );
        }
...
export function validateOperators(operators: Operator[]): void {
  ...
  operators.forEach((op) => {
    ...
    if (
      op.arguments === null ||
      !Object.values(OperatorArguments).includes(op.arguments)
    ) {
    ...
    if (
      op.returnType === null ||
      !Object.values(ReturnType).includes(op.returnType)
    ) {
    ...
    if (
      op.leftScalarEncrypt != null &&
      typeof op.leftScalarEncrypt !== "boolean"
    ) {
      throw new Error(
        `Invalid leftScalarEncrypt value for operator: ${op.name}`
      );
    }

    if (
      op.leftScalarDisable != null &&
      typeof op.leftScalarDisable !== "boolean"
    ) {
      throw new Error(
        `Invalid leftScalarDisable value for operator: ${op.name}`
      );
    }
