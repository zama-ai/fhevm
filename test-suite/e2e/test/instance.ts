...
// Optional because pre-v0.13 hosts have no ProtocolConfig at all. This is not a hole for
// modern stacks: discovery already refuses to generate this env without the address once
// host contracts reach v0.13 (requiresModernHostAddressArtifacts), and the tests that read
// ProtocolConfig on-chain assert it themselves.
...
// (see test/sdk/unified/unifiedUserDecrypt.ts for why the public SDK is not
// used there). Re-declared with explicit `string` types: the module-level
// throw guards above narrow them within this module, but the exported declared
// type would otherwise remain `string | undefined` and fail strict-mode
// consumers.
