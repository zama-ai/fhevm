...
describe('Service failure case', function () {
  ...
  it('arms the workload at the stage the fault is aimed at', async function () {
    ...
    if (['ingestion', 'ingestion-backlog', 'compute-backlog', 'submission', 'zkproof-input'].includes(WORKLOAD)) {
    ...
    switch (WORKLOAD) {
      ...
      case 'compute-backlog': {
        const { ethers } = await import('hardhat');
        for (let index = 0; index < 12; index++) {
          await ethers.provider.send('evm_mine', []);
          await send('combineFromStorage');
          armed.handles.push((await contract.combined()).toLowerCase());
        }
        expect(new Set(armed.handles).size).to.eq(12);
        const deadline = Date.now() + 240_000;
        for (;;) {
          const pending = await withPool(databaseUrls[VICTIM], async pool => Number((await pool.query(
            "SELECT count(*)::text AS count FROM computations WHERE output_handle=ANY($1::bytea[]) AND NOT is_completed AND NOT is_error",
            [armed.handles.map(handle => Buffer.from(handle.slice(2), 'hex'))])).rows[0].count));
          if (pending === 12) break;
      ...
      case 'ingestion': {
        ...
        const blocks: number[] = [];
        let blockHash = "";
        const targets = WORKLOAD === 'ingestion-backlog' ? 12 : 1;
        for (let index = 0; index < targets; index++) {
          if (targets > 1) await ethers.provider.send('evm_mine', []);
        ...
        console.info(
          `[failure-case/${CASE_ID}] armed ${armed.handles.join(",")} in block ${blockNumber} with operator ${VICTIM}'s ` +
  ...
  it('requires the armed workload itself to have recovered', async function () {
    ...
    if (record.workload.endsWith('-backlog')) {
      if (record.workload === 'ingestion-backlog' && !record.detail?.blocks) throw new Error('offline backlog block receipts missing');
      requireBacklogTargets(record.handles, record.transactionHashes, record.detail?.blocks);
    }
    ...
    switch (record.workload) {
      case 'compute-backlog': {
        expect(record.handles).to.have.length(12);
        expect(record.transactionHashes).to.have.length(12);
        for (let index = 0; index < record.handles.length; index++) for (const operator of operators) {
          const rows = await queryCanonicalOutputs(databaseUrls[operator], [record.handles[index]]);
          expect(rows).to.have.length(1);
          expect('0x' + rows[0].transactionId.toString('hex')).to.eq(record.transactionHashes[index]);
        }
        break;
      }
      case 'compute-chain': {
      ...
      case 'storage': {
        ...
        }
        {
          ...
          const expected = record.workload === 'storage' ? '12' : '16';
          expect(String(plaintext), 'recovered object plaintext').to.eq(expected);
          emitAssertions(CASE_ID, ['correctness'], `Recovered object material was retrievable and decrypted to the independent fixture value ${expected}.`);
    ...
    if (record.workload.endsWith('-backlog') || CASE_ID === 'FM-BROKER-REDELIVERY') {
      const retainedCount = CASE_ID === 'FM-BROKER-REDELIVERY' ? 1 : 12;
      if (CASE_ID === 'FM-BROKER-REDELIVERY') expect(record.workload).to.eq('ingestion');
      const { createInstances } = await import('../instance');
      const { getSigners } = await import('../signers');
      const signers = await getSigners();
      const instance = (await createInstances(signers)).alice;
      expect(record.handles).to.have.length(retainedCount);
      expect(new Set(record.handles).size).to.eq(retainedCount);
      for (const handle of record.handles) {
        expect(String(await withDeadline(instance.userDecryptSingleHandle({ handle, contractAddress: record.contractAddress,
          signer: signers.alice }), 300_000, 'retained recovery plaintext'))).to.eq('12');
      }
      const fresh = await contract.combineFromStorage({ gasLimit: GAS_LIMIT });
      await fresh.wait();
      const handle = (await contract.combined()).toLowerCase();
      expect(record.handles).to.not.include(handle);
      await assertOperatorsAgree(databaseUrls, operators, handle, { timeoutMs: 300_000 });
      expect(String(await withDeadline(instance.userDecryptSingleHandle({ handle, contractAddress: record.contractAddress,
        signer: signers.alice }), 300_000, 'fresh recovery plaintext'))).to.eq('12');
      emitAssertions(CASE_ID, ['correctness'], `All ${retainedCount} retained outputs and fresh post-recovery work decrypted to 7+5=12.`);
    }

    // Every non-smoke case ends in the quorum question too, judged against the
    // gateway's own threshold.
    if (record.workload !== 'smoke') {
      const membership = await assertGatewayTopology(GATEWAY_RPC_URL, GATEWAY_CONFIG_ADDRESS, COPROCESSOR_COUNT, Number(process.env.CONSENSUS_THRESHOLD));
      ...
      const quorumHandles = record.workload === 'detector-drift' ? [String(record.detail?.control ?? '')] : record.handles;
      expect(quorumHandles.length, 'nonempty original quorum target set').to.be.greaterThan(0);
      for (const quorumHandle of quorumHandles) {
        expect(quorumHandle).to.match(/^0x[0-9a-f]{64}$/);
        const outcome = await assertQuorumOutcome({ mode: 'required', gatewayRpcUrl: GATEWAY_RPC_URL,
          ciphertextCommitsAddress: CIPHERTEXT_COMMITS_ADDRESS, handle: quorumHandle,
          authorizedSenders: membership.txSenders, threshold: membership.threshold,
          timeoutMs: 8 * 60_000, label: CASE_ID });
        console.info(`[failure-case/${CASE_ID}] ${outcome.detail}`);
      }
      emitAssertions(CASE_ID, ['quorum', 'safety'], 'Every original target formed one consensus event from distinct authorized members.');
    }
