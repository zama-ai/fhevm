...
export async function injectDigestRow(
  ...
  try {
    ...
    if (columns.has('block_hash')) {
      await pool.query(
        `INSERT INTO ciphertext_digest_branch (tenant_id, handle, producer_block_hash, block_hash, block_number, ciphertext, ciphertext128, host_chain_id, key_id_gw)
         VALUES (0, $1, $2, $3, $4, $5, $6, 12345, $7)
...
export async function scrapeMetricOrZero(
  metricsUrl: string,
  metricName: string,
): Promise<number> {
  return (await scrapeMetric(metricsUrl, metricName)) ?? 0;
}
