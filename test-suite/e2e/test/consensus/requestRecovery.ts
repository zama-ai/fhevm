...
import { requireQuorumConfiguration, getCoprocessorDbUrls, readGatewayMembership, waitForDatabaseReadiness } from './helpers';
...
describe('Client request survives service replacement', function () {
  ...
  it('decrypts the original accepted request after recovery', async function () {
    ...
    const membership = await readGatewayMembership(process.env.GATEWAY_RPC_URL!, process.env.GATEWAY_CONFIG_ADDRESS!);
