...
(enabled ? describe : describe.skip)('manifest lifecycle fixture', function () {
  ...
  async function seed(withDescendant: boolean) {
    ...
    const fixture = {
      ...
      ...(withDescendant ? { child: await contract.child() } : {}),
  ...
  it('seed root', async function () { await seed(false); });
  it('seed graph', async function () { await seed(true); });
