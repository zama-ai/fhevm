...
import { rememberMiningState } from './abortRecovery';
...
export async function setIntervalMining(
  ...
  if (!enabled) {
    await rememberMiningState(provider, provider._getConnection().url);
 ...
 * working with an error that read as a malformed request. Forking fetches
 * state lazily rather than submitting a size-limited dump.
 *
 ...
 * fork still returned its own original parent hashes. Since the compute
...
export async function seedForkFromCanonical(
  ...
  await rememberMiningState(target, targetRpcUrl);
...
export async function evmBlockhash(
  ...
  // PUSH32 <n>; BLOCKHASH; PUSH1 0; MSTORE; PUSH1 32; PUSH1 0; RETURN.
  // Long-running local chains routinely exceed the PUSH2 range.
  const probe = `0x7f${ethers.toBeHex(blockNumber, 32).slice(2)}4060005260206000f3`;
...
/** Releasing a gate or acquiring a lock is progress, not terminal recovery. */
export const isTerminalForkChild = (state: { status: string } | null): boolean =>
  state === null || state.status === 'processed';
