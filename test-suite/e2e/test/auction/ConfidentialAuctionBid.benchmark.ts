  ...
  const targetCompletedExpression =
    completionTarget === 'tfhe-ciphertexts'
      ? `(
          ...
          JOIN ciphertexts ct ON ct.handle = p.handle
...
async function readCompletionState(
  ...
  const table = completionTarget === 'pbs-completed' ? 'pbs_computations' : 'computations';
  const result = await client.query<{
    total: string;
    completed: string;
    pending: string;
  ...
  it('benchmark confidential auction bids', async function () {
    ...
    const report = {
      ...
      timestampSources: {
        ...
        targetCompleted:
          completionTarget === 'tfhe-ciphertexts'
            ? 'max(ciphertexts.created_at) for ciphertexts joined from pbs_computations by transaction_id'
            : completionTarget === 'pbs-completed'
              ? 'max(pbs_computations.completed_at) grouped by transaction_id'
              : 'max(computations.completed_at) grouped by transaction_id',
      },
      pollIntervalMs,
    };
