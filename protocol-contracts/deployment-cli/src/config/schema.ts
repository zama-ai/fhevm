...
const EthereumNetworkSchema = z
  .object({
    ...
    host_pkg_name: z.string().min(1, 'Host package network name is required (e.g., "sepolia" for host-contracts Hardhat tasks)'),
    rpc_url_env: z.string().min(1, 'Environment variable name for RPC URL is required'),
...
const GatewayNetworkSchema = z
  .object({
    name: z.string().min(1, 'Network name is required'),
    gateway_pkg_name: z.string().min(1, 'Gateway package network name is required (e.g., "testnet" for gateway-contracts Hardhat tasks)'),
    rpc_url_env: z.string().min(1, 'Environment variable name for RPC URL is required'),
    blockscout_api_url: z.string().url('Blockscout API URL must be a valid URL'),
    chain_id: z.number().positive(),
    explorer_url: z.string().url().optional(),
    etherscan_api_key: z.string().optional()
  })
...
const WalletSchema = z
  .object({
    ...
    private_key_env: z.string().min(1, 'Environment variable name for private key is required')
  })
  .transform((value) => {
    const envPrivateKey = process.env[value.private_key_env] || ""
    if (!PRIVATE_KEY_REGEX.test(envPrivateKey)) {
      throw new ValidationError(`Private key of ${value.description} must be a 32-byte hex string`);
    }
...
const CustodianSchema = z.object({
  tx_sender_address: AddressSchema,
  signer_address: AddressSchema,
  encryption_key: z.string().regex(/^0x[a-fA-F0-9]{128}$/, { message: 'Encryption key must be a valid 64-byte hex string (0x + 128 hex chars)' })
});
...
export const DeploymentConfigSchema = z.object({
  ...
  networks: z
    .record(z.string().min(1, 'Environment name is required'), EnvironmentNetworksSchema)
    .refine(
...
export function loadDeploymentConfig(configPath: string): {
  ...
  if (!parsed.success) {
    const formatted = parsed.error.errors
      ...
      .join('\n');
    throw new ValidationError(`Invalid deployment configuration:\n${formatted}`);
  }
