...
export type PackageName = 'gateway-contracts' | 'host-contracts' | 'protocol-contracts/token' |
  'protocol-contracts/feesBurner' | 'protocol-contracts/pauserSetWrapper' | 'protocol-contracts/safe'
...
export class HardhatRunner {
  ...
  public async runTask(options: HardhatTaskOptions): Promise<HardhatCommandResult> {
    ...
    const { stdout = '', stderr = '', exitCode = 1 } = await execa('npx', args, execOptions);
    const result = {
      command: commandStr,
      stdout,
      stderr,
