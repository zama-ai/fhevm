...
export class Step04TokenDeployment extends BaseStep {
  ...
  protected async validate(ctx: DeploymentContext): Promise<void> {
    ...
    if (!recipients || recipients.length === 0) {
      throw new ValidationError('At least one token recipient is required before Step 4.');
    }
    ...
    if (!safeAddress) {
      throw new ValidationError('Safe address is required before Step 4.');
    }
  }
  ...
  protected async execute(ctx: DeploymentContext): Promise<StepExecutionResult> {
    ...
    if (!daoAddress || !safeAddress) {
      throw new ValidationError('Required prerequisite addresses missing before token deployment.');
    }
    ...
    const args = ['--preset', presetName, ...(ctx.config.options.auto_verify_contracts ? ['--verify', 'true'] : [])];
    ...
    const zamaToken = reader.readHardhatDeployment(this.pkgName, ethereum.name, 'ZamaERC20');
    const zamaOftAdapter = reader.readHardhatDeployment(this.pkgName, ethereum.name, 'ZamaOFTAdapter');
    const zamaOft = reader.readHardhatDeployment(this.pkgName, gateway.name, 'ZamaOFT');
