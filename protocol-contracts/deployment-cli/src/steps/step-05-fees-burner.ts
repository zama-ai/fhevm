...
export class Step05FeesBurner extends BaseStep {
  ...
  protected async validate(ctx: DeploymentContext): Promise<void> {
    if (!ctx.env.getAddress('ZAMA_TOKEN')) {
      throw new ValidationError('Zama token must be deployed before Step 5.');
    }
    if (!ctx.env.getAddress('ZAMA_OFT')) {
      throw new ValidationError('Zama OFT must be deployed before Step 5.');
    }
  }
  ...
  protected async execute(ctx: DeploymentContext): Promise<StepExecutionResult> {
    ...
    let protocolFeesBurner = ctx.env.getAddress('PROTOCOL_FEES_BURNER_ADDRESS');
    let feesSenderToBurner = ctx.env.getAddress('FEES_SENDER_TO_BURNER_ADDRESS');
    ...
    if (!protocolFeesBurner) {
      ...
      protocolFeesBurner = reader.readHardhatDeployment(this.pkgName, ethereum.name, 'ProtocolFeesBurner');
    }
    ...
    if (!feesSenderToBurner) {
      ...
      const projectRoot = resolveProjectRoot();
      const reader = new TaskOutputReader(projectRoot);
      feesSenderToBurner = reader.readHardhatDeployment(this.pkgName, gateway.name, 'FeesSenderToBurner');
    }

    if (ctx.config.options.auto_verify_contracts) {
      ctx.logger.info('Waiting 10 seconds for contract deployment to propagate before verification...');
      await new Promise(resolve => setTimeout(resolve, 10000));
      await ctx.hardhat.runTask({
        pkg: this.pkgName,
        task: 'task:verifyProtocolFeesBurner',
        args: ['--protocol-fees-burner', protocolFeesBurner!, '--network', ethereum.name],
        env: baseEnv
      });
      ...
      await ctx.hardhat.runTask({
        ...
        args: ['--fees-sender-to-burner', feesSenderToBurner!, '--network', gateway.name],
        env: {...baseEnv, PROTOCOL_FEES_BURNER_ADDRESS: protocolFeesBurner!}
      });
