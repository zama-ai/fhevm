...
export class Step10HostOwnership extends BaseStep {
  ...
  protected async execute(ctx: DeploymentContext): Promise<StepExecutionResult> {
    ...
    if (!aclAddress) {
      throw new ValidationError('ACL address missing. Complete Step 7 first.');
    }


    const deployerPk = ctx.env.resolveWalletPrivateKey('deployer');
    ctx.logger.info(`Starting ownership transfer of ACL ${aclAddress} to DAO ${daoAddress}`);
    ...
    await ctx.hardhat.runTask({
      ...
      args: ['--new-owner-address', daoAddress, '--network', ethereum.hostPkgName!],
      env: offerEnv
    });
    ...
    ctx.logger.info(`Function selector for acceptOwnership(): ${ethers.id('acceptOwnership()').slice(0, 10)}`);
    ...
    const verified = await this.isACLOwnedBy(ethereum.rpcUrl, aclAddress, daoAddress);
    if (!verified) {
   ...
   */
  private async isACLOwnedBy(rpcUrl: string, aclAddress: string, expectedOwner: string): Promise<boolean> {
    try {
      ...
      const contract = new ethers.Contract(aclAddress, ownerableABI, provider);
