 ...
 */
contract FeesSenderToBurnerMock {
    using OptionsBuilder for bytes;

    address public immutable ZAMA_OFT;
    address public immutable PROTOCOL_FEES_BURNER;
    /// @dev Restrictions on destination eid resolution are lifted in the mock version.
    uint32 public immutable DESTINATION_EID;

    /// @dev Made public for easy testing
    uint256 immutable public _DECIMAL_CONVERSION_RATE;
    ...
    constructor(address _oft, address _protocolFeesBurner, uint32 _dstEid) {
        ZAMA_OFT = _oft;
        PROTOCOL_FEES_BURNER = _protocolFeesBurner;
        DESTINATION_EID = _dstEid;
