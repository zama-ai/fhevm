...
task('task:getLZOptions')
    ...
    .addOptionalParam(
        'nativeValue',
        'Native value to send with the LZ message',
        0,
        types.int
    )
    .setAction(async function (taskArgs) {
        const options = Options.newOptions().addExecutorLzReceiveOption(taskArgs.gasLimit, taskArgs.nativeValue).toHex().toString()
