...
// Note: The assets are in the smallest unit of $ZAMA, using 10^18 as the decimals
// Example usage:
...
task('task:depositOperatorStakingFromDeployer')
  ...
  .setAction(async function ({ assets, receiver, operatorStakingAddress }, hre: HardhatRuntimeEnvironment) {
    ...
    console.log('Depositing assets into the operator staking contract...');
    ...
    const tx = await zamaTokenMock.connect(deployerSigner).approve(operatorStakingAddress, assets);
    ...
    await operatorStaking.deposit(assets, receiver);
