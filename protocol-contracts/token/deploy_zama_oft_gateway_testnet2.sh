...
note "Running lz:deploy in CI mode for ZamaERC20 on arbitrum-testnet."
npx hardhat lz:deploy --ci --networks arbitrum-testnet --tags ZamaERC20

info "Updating hardhat.config.ts with the deployed ZamaERC20 address"
deployments_file="deployments/arbitrum-testnet/ZamaERC20.json"
...
note "networks.arbitrum-testnet.oftAdapter.tokenAddress is now set to $current_token_address"

info "Deploy ZamaOFTAdapter on Ethereum Sepolia"
note "Running lz:deploy in CI mode for ZamaOFTAdapter on arbitrum-testnet."
npx hardhat lz:deploy --ci --networks arbitrum-testnet --tags ZamaOFTAdapter
...
note "Running lz:oapp:wire in CI mode using layerzero.config.gatewaytestnet2.ts."
npx hardhat lz:oapp:wire --ci --oapp-config layerzero.config.gatewaytestnet2.ts
