# Technical Specification

This folder holds the current technical specification of the ZWS Console.
A counterpart for the Coprocessor + HTTPZ protocol can be found at [https://github.com/zama-ai/tech-spec](https://github.com/zama-ai/tech-spec).

## Table of content

- [services and interactions](services.md)
- [users, teams and dapps](users-teams-dapps.md)
- [back pubsub](back-pub-sub.md)
- [ZWS Console deployment](deployment.md)
- [Dapp developer registration](registration.md)
- [Public decryption](public_decryption.md)
- [User decryption](user_decryption.md)
- [Metrics](metrics.md)
- [Architecture](architecture.md)
- [Get a local Development Environment](local_env.md)
- [Back Synchronization](back-sync.md)
