...
task('task:computeDecryptionOracleAddress')
  ...
  .setAction(async function (taskArguments: TaskArguments, { ethers }) {
    ...
    const envFilePath = path.join(__dirname, '../node_modules/fhevm-core-contracts/addresses/.env.decryptionoracle');
    const content = `DECRYPTION_ORACLE_ADDRESS=${decryptionoracleAddressPrecomputed}`;
    try {
      fs.writeFileSync(envFilePath, content, { flag: 'w' });
      console.log(
        'decryptionOracleAddress written to node_modules/fhevm-core-contracts/addresses/.env.decryptionoracle successfully!',
      );
    } catch (err) {
      console.error('Failed to write to node_modules/fhevm-core-contracts/addresses/.env.decryptionoracle:', err);
    }
...
`;
    ...
    try {
      fs.writeFileSync('node_modules/fhevm-core-contracts/addresses/DecryptionOracleAddress.sol', solidityTemplate, {
        encoding: 'utf8',
        flag: 'w',
      });
      console.log(
        'node_modules/fhevm-core-contracts/addresses/DecryptionOracleAddress.sol file has been generated successfully.',
      );
    } catch (error) {
      console.error('Failed to write node_modules/fhevm-core-contracts/addresses/DecryptionOracleAddress.sol', error);
    }
  });
