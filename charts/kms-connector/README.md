...
| `commonConfig.hostChains`                     | Host chains served by the connector, a map of `<name>: {url, chainId, aclAddress}`. Every chain deployed on `network` must have a `url` (may be `$(VAR)`); `chainId`/`aclAddress` override the preset when set. An `ethereum` entry is required. With `network` empty, the chain id's type byte names the kind: a Solana chain (type byte `0x01`) sets `solanaHostProgramId` and `solanaProofRoutes`, a list of `{url, apiKey}` with one entry per coprocessor, instead of `aclAddress`. Each `apiKey` is sent only to its `url`; pass it as `$(VAR)` from a Secret mapped through `kmsConnectorKmsWorker.env`. | `ethereum` and `polygon` with empty URLs |
