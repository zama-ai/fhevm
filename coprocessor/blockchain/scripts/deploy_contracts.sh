...
if [ "$MODE" = "threshold" ]; then
  ...
  ASC_INST_DEBUG_TX_HASH=$(echo $KEYRING_PASSWORD | NODE="$NODE" wasmd tx wasm instantiate "${ASC_CODE_ID}" '{"debug_proof": true, "verify_proof_contract_addr": "dummy", "kms_core_conf": { "parties":[{"party_id": "01", "address": ""}], "response_count_for_majority_vote": 1, "response_count_for_reconstruction": 1, "degree_for_reconstruction": 0, "param_choice": "default" }, "allowed_addresses": {"allowed_to_gen": ["*"], "allowed_to_response": ["*"]} }' --label "asc-debug" --from validator --output json --chain-id testing -y --admin "${VALIDATOR_ADDRESS}"  --gas-prices 0.25ucosm --gas auto --gas-adjustment 1.3 | jq -r '.txhash')

  sleep 6

  echo "Instantiating centralized ASC Ethermint"
  ASC_INST_ETHERMINT_TX_HASH=$(echo $KEYRING_PASSWORD | NODE="$NODE" wasmd tx wasm instantiate "${ASC_CODE_ID}" '{"debug_proof": false, "verify_proof_contract_addr": "'"${IPSC_ETHERMINT_ADDRESS}"'", "kms_core_conf": {"parties":[{"party_id": "01", "address": ""}], "response_count_for_majority_vote": 1, "response_count_for_reconstruction": 1, "degree_for_reconstruction": 0, "param_choice": "default"}, "allowed_addresses":{"allowed_to_gen": ["*"], "allowed_to_response": ["*"]} }' --label "tendermint-asc" --from validator --output json --chain-id testing -y --admin "${VALIDATOR_ADDRESS}"  --gas-prices 0.25ucosm --gas auto --gas-adjustment 1.3 | jq -r '.txhash')

  sleep 6

  echo "Instantiating centralized ASC Ethereum"
  ASC_INST_ETHEREUM_TX_HASH=$(echo $KEYRING_PASSWORD | NODE="$NODE" wasmd tx wasm instantiate "${ASC_CODE_ID}" '{"debug_proof": false, "verify_proof_contract_addr": "'"${IPSC_ETHEREUM_ADDRESS}"'", "kms_core_conf": { "parties":[{"party_id": "01", "address": ""}], "response_count_for_majority_vote": 1, "response_count_for_reconstruction": 1, "degree_for_reconstruction": 0, "param_choice": "default" }, "allowed_addresses":{"allowed_to_gen": ["*"], "allowed_to_response": ["*"]} }' --label "ethereum-asc" --from validator --output json --chain-id testing -y --admin "${VALIDATOR_ADDRESS}" --gas-prices 0.25ucosm --gas auto --gas-adjustment 1.3 | jq -r '.txhash')
