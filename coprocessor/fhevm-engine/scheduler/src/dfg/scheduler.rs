...
use super::{pattern, DFComponentGraph, DFGraph, OpNode};
            ...
            if self.is_ready_task(node) {
                ...
                for nidx in node.df_nodes.iter() {
                    ...
                    args.push(PartitionTask {
                        ...
                        transaction_pattern_id: tx.transaction_pattern_id.clone(),
                    });
                }
...
fn execute_partition(
    ...
    } in transactions
    {
        ...
        let _exec_guard = tracing::info_span!(
            "execute_transaction",
            transaction_id = %hex::encode(&tid),
            transaction_pattern_id = %transaction_pattern_id,
        )
        .entered();
...
fn try_execute_node(
    ...
    let operation_pattern_id = node.operation_pattern_id().to_vec();
    ...
    {
        let _guard = tracing::info_span!("rerandomise_op_inputs", operation_pattern_id = %pattern::pattern_to_base64url(&operation_pattern_id)).entered();
        let started_at = std::time::Instant::now();
    ...
    let result = std::panic::catch_unwind(|| {
        run_computation(opcode, cts, node_index, gpu_idx, &operation_pattern_id)
    });
    match result {
        Err(e) => {
            let msg = e
...
fn run_computation(
    ...
    gpu_idx: usize,
    operation_pattern_id: &[u8],
) -> (usize, OpResult) {
    let op = FheOperation::try_from(operation);
    let operation_pattern_id_hex = pattern::pattern_to_base64url(operation_pattern_id);
    match op {
