...
/// A consensus_epoch-scoped host block tracked by the manifest publication state
/// machine.
#[derive(Clone, Debug)]
...
/// Per-chain span of `block_manifest_state` already stored for this consensus_epoch.
///
...
///
/// Consensus epoch ownership is established by stack routing, not by a block-number
...
pub(crate) async fn discover_blocks(pool: &PgPool) -> Result<u64, ExecutionError> {
    let consensus_epoch =
        crate::manifest_consensus::storage::active::load_consensus_epoch(pool).await?;
    discover_blocks_for_consensus_epoch(pool, &consensus_epoch, &BTreeMap::new()).await
}

pub(crate) async fn discover_blocks_for_consensus_epoch(
    pool: &PgPool,
    consensus_epoch: &str,
    cadence_overrides: &BTreeMap<i64, i64>,
) -> Result<u64, ExecutionError> {
    let frontiers = load_existing_frontiers(pool, consensus_epoch).await?;
    let windows = load_consensus_epoch_windows(pool, consensus_epoch).await?;
...
async fn insert_bootstrap_blocks(
    ...
    let anchored = insert_consensus_epoch_anchor(pool, consensus_epoch, bound).await?;
    if anchored == 0 {
        return Ok(0);
    }
...
pub(crate) async fn pending_chain_ids(pool: &PgPool) -> Result<Vec<i64>, ExecutionError> {
    let consensus_epoch =
        crate::manifest_consensus::storage::active::load_consensus_epoch(pool).await?;
    pending_chain_ids_for_consensus_epoch(pool, &consensus_epoch).await
}

pub(crate) async fn pending_chain_ids_for_consensus_epoch(
    pool: &PgPool,
    consensus_epoch: &str,
...
pub(crate) async fn lock_next_block_to_progress(
    ...
    let consensus_epoch = sqlx::query_scalar::<_, String>(
        "SELECT consensus_epoch FROM blue_green_consensus_epoch WHERE singleton = TRUE",
    )
    .fetch_one(trx.as_mut())
    .await?;
    lock_next_block_to_progress_for_consensus_epoch(trx, host_chain_id, cursor, &consensus_epoch)
        .await
}

pub(crate) async fn lock_next_block_to_progress_for_consensus_epoch(
    trx: &mut Transaction<'_, Postgres>,
    host_chain_id: i64,
    cursor: &ManifestProgressCursor,
    consensus_epoch: &str,
    ...
    let result = sqlx::query!(
        ...
        SELECT candidate.consensus_epoch,
         ...
         WHERE candidate.host_chain_id = $1
           AND candidate.consensus_epoch = $4
           AND (
           ...
           AND NOT EXISTS (
                SELECT 1
                 ...
                 WHERE blocked.host_chain_id = candidate.host_chain_id
                   AND blocked.consensus_epoch = candidate.consensus_epoch
