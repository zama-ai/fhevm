...
#[test]
fn object_key_matches_the_consensus_epoch_scoped_v1_layout() {
...
async fn readiness_and_preparation_reject_incomplete_or_corrupted_block_content() {
    ...
    let err = load_manifest_descriptors(&mut trx, &block, false, None)
        .await
...
#[tokio::test]
#[serial(db)]
async fn first_manifest_of_a_consensus_epoch_starts_a_new_history_lineage() {
    const CHAIN_ID: i64 = 137;
    let instance = setup_test_db(ImportMode::None)
        .await
        .expect("create consensus_epoch-boundary database");
    let pool = PgPool::connect(instance.db_url())
        .await
        .expect("connect consensus_epoch-boundary database");
    ...
    .await;
    sqlx::query(
        "UPDATE block_manifest_state SET consensus_epoch = '1' WHERE block_hash IN ($1, $2)",
    )
    .bind(current_hash.as_slice())
    .bind(next_hash.as_slice())
    .execute(&pool)
    .await
    .expect("assign current-consensus_epoch blocks");
    ...
    sqlx::query(
        "UPDATE block_manifest_state
          ...
          WHERE consensus_epoch = '1' AND block_hash = $1",
    )
    .bind(current_hash.as_slice())
    ...
    .execute(&pool)
    .await
    .expect("mark the first current-consensus_epoch manifest published");
...
#[tokio::test]
#[serial(db)]
async fn missing_in_consensus_epoch_parent_is_materialized_as_empty() {
    const CHAIN_ID: i64 = 137;
    ...
    .execute(&pool)
    .await
    .expect("store consensus_epoch window");
    sqlx::query(
        "UPDATE blue_green_consensus_epoch SET consensus_epoch = '1', updated_at = NOW()
    ...
    sqlx::query(
        "INSERT INTO block_manifest_state (
             consensus_epoch, host_chain_id, block_number, block_hash, parent_block_hash,
             publication_cadence
         ) VALUES ('1', $1, 60, $2, $3, 1)",
    )
    .bind(CHAIN_ID)
    ...
    .execute(&pool)
    .await
    .expect("track consensus_epoch start");
    seed_revision_publication_block(
        &pool,
        CHAIN_ID,
        62,
    ...
    .await;
    sqlx::query("UPDATE block_manifest_state SET consensus_epoch = '1' WHERE block_hash = $1")
        .bind(target_hash.as_slice())
        .execute(&pool)
        .await
        .expect("assign target consensus_epoch");
    ...
    seal_block_content(&mut trx, &start, U256::ONE, &[])
        .await
        .expect("seal empty consensus_epoch start");
...
#[tokio::test]
#[serial(db)]
async fn missing_in_consensus_epoch_parent_with_producers_is_inserted_unsealed() {
    const CHAIN_ID: i64 = 137;
    let start_hash = B256::repeat_byte(0x60);
    let missing_hash = B256::repeat_byte(0x61);
    let target_hash = B256::repeat_byte(0x62);
...
async fn drift_injection_seals_and_signs_consistently_without_changing_stored_digests() {
    ...
    std::fs::write(&path, format!(r#"{{"enabled":true,"chain_id":{chain},"handle":"{handle}","fault":"ct64_digest_bit_flip"}}"#)).unwrap();
    ...
    injected.0[0] ^= 1;
    assert_eq!(descriptors[0].ct64_digest(), Some(injected));
    seal_block_content(&mut trx, &block, U256::ONE, &descriptors)
        .await
        .unwrap();
    ...
    for _ in 0..2 {
        ...
        .unwrap();
        assert_eq!(
            prepared.payload.detailed_range.blocks[0].ciphertexts[0].ct64_digest(),
            Some(injected)
        );
