...
use block_manifest::{
    block_content_digest, detailed_range_digest, dyadic_range_digest, CiphertextFormat,
...
///
/// Short local check:
/// `SNS_MANIFEST_SIMULATED_BLOCKS=7 SQLX_OFFLINE=true cargo test -p consensus-detector canonical_manifest_generation_simulation_fast_chain --lib -- --nocapture`
#[tokio::test]
#[serial(db)]
...
/// `SQLX_OFFLINE=true cargo test -p consensus-detector canonical_manifest_simulation_publishes_with_randomly_missing_previous_rows --lib -- --nocapture`
///
/// Short local check:
/// `SNS_MANIFEST_MISSING_PREVIOUS_SIMULATED_BLOCKS=13 SQLX_OFFLINE=true cargo test -p consensus-detector canonical_manifest_simulation_publishes_with_randomly_missing_previous_rows --lib -- --nocapture`
#[tokio::test]
#[serial(db)]
...
/// publishers race through production work-row locking throughout the reorg.
#[tokio::test]
#[serial(db)]
...
async fn run_concurrent_manifest_simulation(block_count_env: &str, inject_missing_previous: bool) {
            ...
            && fault_schedule.should_delete()
        {
            ...
            let predecessor =
                wait_for_published_manifest_reference(&pool, predecessor_number, predecessor_hash)
                    .await;
            let deleted_digest = delete_last_published_manifest(&pool, &predecessor).await;
    ...
    assert!(
        ...
        "concurrent workers never observed a locked production work row"
    );
    if inject_missing_previous {
        assert!(
...
async fn concurrent_manifest_worker(
    ...
    loop {
        ...
        for host_chain_id in chains {
            ...
            // Real publication holds this production row lock while it
...
fn assert_manifest_is_canonical(
    ...
    expected_blocks: &[ExpectedBlock],
    last_published_manifest: Option<&ManifestReference>,
) {
    ...
    let first = i64::try_from(payload.detailed_range.first_block_number)
        .expect("detailed range start fits BIGINT");
    let expected_first = last_published_manifest
    ...
    for (actual, (start, end, scale)) in payload.historical_ranges.iter().zip(expected_history) {
        assert_eq!(actual.end_block_generation, 0);
...
fn assert_reorg_manifests_are_canonical(
    ...
    for ((publication_number, publication_hash), signed) in manifests {
        ...
        assert_eq!(signed.payload.generation, 0);
        assert_eq!(signed.payload.publisher, publisher);
        assert_eq!(
        ...
        let previous =
            expected_last_publication(publication, manifests, &expected_by_hash, cadence);
...
fn expected_last_publication<'a>(
...
async fn assert_persisted_reorg_state(
    ...
    for row in range_rows {
        ...
        let (_, scale) = dyadic_range_size_and_scale(start, end);
...
async fn assert_persisted_canonical_state(
    ...
    for row in range_rows {
        ...
        let digest: Vec<u8> = row.get("range_digest");
        let (size, scale) = dyadic_range_size_and_scale(start, end);
...
fn dyadic_range_size_and_scale(start: i64, end: i64) -> (i64, u32) {
    let size = end
        .checked_sub(start)
        .and_then(|width| width.checked_add(1))
        .expect("range bounds do not fit BIGINT");
    let unsigned_size = u64::try_from(size).expect("range bounds are inverted");
    assert!(
        unsigned_size.is_power_of_two(),
        "range [{start}, {end}] is not dyadic"
    );
    (size, unsigned_size.ilog2())
}
