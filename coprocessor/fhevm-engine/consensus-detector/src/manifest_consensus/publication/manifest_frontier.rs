...
pub(super) fn rebuild_frontier_from_manifest(
    ...
    {
        if range.start_block_number
            != i64_from_u256("historical range start", historical.start_block_number)?
...
pub(super) fn append_leaf(
    ...
    let consensus_epoch_start = frontier
        .as_slice()
        .first()
        .map_or(leaf.start_block_number, |oldest| oldest.start_block_number);
    ...
    if let Some(existing) = available.get(&key) {
        if existing != &range {
            return Err(internal(format!(
                ...
                range.start_block_number, range.end, range.scale,
            )));
        }
        return Ok(());
    }
...
fn materialize_range(
    ...
    let (left, right) = match (left, right) {
        ...
        (MaterializedRange::Empty, MaterializedRange::Available(right)) => {
            let promoted = RangeNode {
                start_block_number: right.start_block_number,
                end: right.end,
                scale,
    ...
    if !left.is_left_sibling_of(&right) {
        return Err(internal(format!(
            ...
            left.start_block_number, left.end, right.start_block_number, right.end,
        )));
    }

    let parent = RangeNode {
        start_block_number: left.start_block_number,
        end: right.end,
        scale,
        ...
        digest: dyadic_range_digest(
            ...
            non_negative_u256("range start", left.start_block_number)?,
...
pub(super) fn validate_canonical_frontier(frontier: &[RangeNode]) -> Result<(), ExecutionError> {
    for pair in frontier.windows(2) {
        if pair[0].end.checked_add(1) != Some(pair[1].start_block_number)
    ...
    for (index, range) in frontier.iter().rev().enumerate() {
        ...
        if range.scale != expected_scale
            ...
            || (!is_oldest && range.start_block_number != expected_virtual_start)
            || (is_oldest
                && (range.start_block_number < expected_virtual_start
                    || range.start_block_number > range.end))
        {
            return Err(internal(format!(
...
mod tests {
    ...
    fn creates_frontier_from_scratch() {
        ...
        assert_eq!(
            frontier
                .as_slice()
                .iter()
                .map(|range| (range.start_block_number, range.end, range.scale))
                .collect::<Vec<_>>(),
            vec![(0, 1, 1), (2, 2, 0)],
        );
    }
    ...
    #[test]
    fn retains_a_consensus_epoch_truncated_oldest_virtual_range() {
        let mut frontier = RangeFrontier::default();
        for number in 1..=7 {
            append_leaf(HOST_CHAIN_ID, CONTEXT_ID, &mut frontier, leaf(number)).unwrap();
        }
