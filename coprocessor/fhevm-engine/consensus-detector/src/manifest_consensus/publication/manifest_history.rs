...
pub(crate) async fn load_detailed_lineage(
    ...
    // A consensus_epoch is an independent verification epoch. Its first manifest
    // starts a new lineage instead of inheriting detailed blocks or compact
    // history from the preceding consensus_epoch.
    let floor = lineage_floor(trx, &target.consensus_epoch, target.host_chain_id).await?;
    let mut lineage = vec![target.clone()];
    let mut parent_hash = target.parent_block_hash.clone();
    let mut consensus_epoch = target.consensus_epoch.clone();
    let last_published_manifest = loop {
        if let Some(parent) =
            load_block_by_hash(trx, target.host_chain_id, &consensus_epoch, &parent_hash).await?
        {
            ...
            consensus_epoch.clone_from(&parent.consensus_epoch);
            lineage.push(parent);
            continue;
        }
        // Not in publication state. If the host block is still in-consensus_epoch
        ...
        // consensus_epoch floor this is the origin.
...
async fn lineage_floor(
    ...
    .await?
    .ok_or_else(|| internal("consensus_epoch has no publication state to bound lineage"))
}
...
fn manifest_reference(block: &PendingBlock) -> Result<ManifestReference, ExecutionError> {
    Ok(ManifestReference {
        consensus_epoch: block.consensus_epoch.clone(),
...
pub(crate) async fn load_frontier(
    ...
    let consensus_epoch = archived.signed.payload.consensus_epoch.as_str();
    for range in reconstructed_ranges {
        persist_range(trx, host_chain_id, consensus_epoch, &range).await?;
    }
...
async fn reconstruct_frontier_from_lineage(
    ...
    let manifest_consensus_epoch = manifest.consensus_epoch.clone();
    let mut consensus_epoch = manifest_consensus_epoch.clone();
    ...
    loop {
        ...
        let Some(block) =
            load_block_by_hash(trx, host_chain_id, &consensus_epoch, &next_hash).await?
        else {
            break;
        };
        next_hash.clone_from(&block.parent_block_hash);
        consensus_epoch.clone_from(&block.consensus_epoch);
...
async fn persist_frontier_leaf(
    ...
    persist_range(trx, host_chain_id, consensus_epoch, &leaf).await?;
    for parent in append_leaf(host_chain_id, coprocessor_context_id, frontier, leaf)? {
        persist_range(trx, host_chain_id, consensus_epoch, &parent).await?;
    }
    Ok(())
}
