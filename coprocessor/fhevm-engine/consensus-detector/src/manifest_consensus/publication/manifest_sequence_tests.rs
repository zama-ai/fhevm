...
///
/// `SQLX_OFFLINE=true cargo test -p consensus-detector canonical_manifest_production_simulation_fast_chain --lib -- --nocapture`
#[tokio::test]
#[serial(db)]
async fn canonical_manifest_production_simulation_fast_chain() {
...
fn manifest_reference(signed: &SignedManifest) -> ManifestReference {
    ManifestReference {
        consensus_epoch: signed.payload.consensus_epoch.clone(),
