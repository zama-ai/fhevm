...
pub(crate) struct ManifestReference {
    pub(crate) consensus_epoch: String,
...
pub(crate) struct StoreResult {
    /// Database identity of the stored `block_manifest` row.
    #[allow(dead_code)]
    pub id: i64,
...
impl ManifestIdentity {
    fn from_manifest(manifest: &SignedManifest) -> Result<Self, ExecutionError> {
        Ok(Self {
            consensus_epoch: manifest.payload.consensus_epoch.clone(),
...
/// Loads the closest earlier local publication points on the same consensus_epoch
...
pub(crate) async fn load_previous_local_manifests(
    ...
    let consensus_epoch = payload.consensus_epoch.clone();
...
fn matches_archive_scope(
    ...
    payload.publisher == publisher
        ...
        && payload.consensus_epoch == consensus_epoch
