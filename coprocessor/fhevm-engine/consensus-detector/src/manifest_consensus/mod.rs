...
pub(crate) enum ExecutionError {
    ...
    /// A missing in-consensus_epoch parent has producer inventory and was inserted
...
pub(crate) struct ManifestWorkGate {
    mode: Arc<StackMode>,
    active_consensus_epoch: Arc<RwLock<Option<String>>>,
}

impl ManifestWorkGate {
    fn new(gcs_mode: bool, active_consensus_epoch: Option<String>) -> Arc<Self> {
        Arc::new(Self {
            mode: StackMode::new(gcs_mode),
            active_consensus_epoch: Arc::new(RwLock::new(active_consensus_epoch)),
        })
    }

    /// Returns whether work pinned to `consensus_epoch` may still make progress.
    ///
    /// Comparing the consensus_epoch, rather than merely checking that some
    /// consensus_epoch is active, fences an in-flight loop when Green is rapidly
    /// reactivated for a later upgrade window.
    pub(crate) fn work_enabled_for(&self, consensus_epoch: &str) -> bool {
        !self.mode.is_paused() && self.pinned_consensus_epoch().as_deref() == Some(consensus_epoch)
    }

    pub(crate) fn pinned_consensus_epoch(&self) -> Option<String> {
        if self.mode.is_paused() {
            return None;
        }
        self.active_consensus_epoch
            .read()
            .unwrap_or_else(|poisoned| poisoned.into_inner())
            .clone()
    }

    pub(crate) fn pin_consensus_epoch(&self, consensus_epoch: Option<String>) {
        *self
            .active_consensus_epoch
            .write()
            .unwrap_or_else(|poisoned| poisoned.into_inner()) = consensus_epoch;
    }
...
pub(crate) async fn start(
    ...
    if !config.gcs_mode && !work_gate.mode.is_paused() {
        let consensus_epoch = storage::active::load_validated_consensus_epoch(&pool).await?;
        work_gate.pin_consensus_epoch(Some(consensus_epoch.clone()));
        info!(consensus_epoch, "Pinned active manifest consensus_epoch at startup");
    }
    {
    ...
    if config.gcs_mode {
        ...
        let watcher_state = Arc::clone(&work_gate.active_consensus_epoch);
        let watcher_cancel = cancel.child_token();
        tokio::spawn(async move {
            loop {
                tokio::select! {
                    _ = watcher_cancel.cancelled() => return,
                    result = storage::active::run_gcs_active_consensus_epoch_watcher(
                        &watcher_pool,
                        &watcher_state,
                    ) => {
...
mod tests {
    ...
    fn green_manifest_work_waits_for_activation_and_repauses_on_rollback() {
        ...
        gate.pin_consensus_epoch(Some("7".to_owned()));
        assert!(gate.work_enabled_for("7"));

        gate.pin_consensus_epoch(None);
        assert!(!gate.work_enabled_for("7"));
    }

    #[test]
    fn rapid_green_reactivation_cannot_reuse_the_previous_consensus_epoch() {
        let gate = ManifestWorkGate::new(true, None);

        gate.pin_consensus_epoch(Some("7".to_owned()));
        gate.pin_consensus_epoch(None);
        // No manifest poll observes the parked state before the next window.
        gate.pin_consensus_epoch(Some("8".to_owned()));

        assert_eq!(gate.pinned_consensus_epoch().as_deref(), Some("8"));
        assert!(!gate.work_enabled_for("7"));
