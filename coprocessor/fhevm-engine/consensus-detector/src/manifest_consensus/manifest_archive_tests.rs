...
#[tokio::test]
async fn v1_manifest_object_key_is_consensus_epoch_namespaced() {
    let signer = PrivateKeySigner::random();
...
async fn concurrent_peer_archive_supports_db_only_multi_coprocessor_quorum() {
    ...
    let manifests = [
        sign_payload(&signers[0], payload(signers[0].address(), 1, 0)).await,
        sign_payload(&signers[1], payload(signers[1].address(), 1, 0)).await,
        sign_payload(&signers[2], payload(signers[2].address(), 9, 0)).await,
    ];
    ...
    for signer in &signers {
        let mut trx = pool
            .begin()
            .await
            .expect("begin highest archived revision read");
        ...
        .await
        .expect("load highest archived peer revision")
        .expect("peer revision zero is archived");
        trx.commit()
            .await
            .expect("commit highest archived revision read");
        commitment_groups
            .entry(manifest.signed.payload.detailed_range.digest)
            .or_default()
            .push(signer.address());
    }
...
#[tokio::test(flavor = "multi_thread")]
#[serial]
async fn concurrent_peer_archive_selects_highest_independent_revision() {
    let (_instance, pool) = setup_archive_db().await;
    let signer = PrivateKeySigner::random();
    let revision_zero = sign_payload(&signer, payload(signer.address(), 1, 0)).await;
    let revision_one = sign_payload(&signer, payload(signer.address(), 2, 1)).await;
    let revision_two = sign_payload(&signer, payload(signer.address(), 3, 2)).await;
    ...
    assert_eq!(
        load_highest_revision(&pool, signer.address()).await,
        Some(2),
        "the highest authenticated revision is independently usable",
    );
...
#[tokio::test(flavor = "multi_thread")]
#[serial]
async fn highest_revision_is_scoped_to_authenticated_publisher() {
    let (_instance, pool) = setup_archive_db().await;
    let old_signer = PrivateKeySigner::random();
    let current_signer = PrivateKeySigner::random();
    let revision_zero = sign_payload(&old_signer, payload(old_signer.address(), 1, 0)).await;
    let revision_one = sign_payload(&current_signer, payload(current_signer.address(), 1, 1)).await;

    assert_single_insert(concurrent_store(&pool, &revision_zero, TEST_WORKERS).await);
    assert_single_insert(concurrent_store(&pool, &revision_one, TEST_WORKERS).await);
    assert_eq!(
        load_highest_revision(&pool, current_signer.address()).await,
        Some(1),
    );
}
...
async fn concurrent_peer_archive_rejects_equivocation_at_one_numbered_key() {
    let (_instance, pool) = setup_archive_db().await;
    let signer = PrivateKeySigner::random();
    let first = sign_payload(&signer, payload(signer.address(), 1, 0)).await;
    let conflicting = sign_payload(&signer, payload(signer.address(), 9, 0)).await;
...
async fn load_highest_revision(pool: &PgPool, publisher: Address) -> Option<u64> {
    let mut trx = pool.begin().await.expect("begin highest revision read");
    let revision = load_highest_archived_revision(
        &mut trx,
        publisher,
        ManifestVersion::V1,
        TEST_CONTEXT_ID,
    ...
    .await
    .expect("load highest archived revision")
    .map(|manifest| manifest.signed.payload.revision);
    trx.commit().await.expect("commit highest revision read");
    revision
}
