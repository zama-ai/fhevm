...
async fn peer_manifest_authentication_binds_publisher_and_numbered_key() {
    ...
    let wrong_numbered_key = key.strip_suffix("revision_0").unwrap().to_owned() + "revision_1";
    assert!(
...
async fn v1_manifest_object_key_is_generation_namespaced() {
    ...
    payload.detailed_range.blocks[0].generation = 9;
    ...
    assert_eq!(
        ...
        format!(
            "manifests/v_1/context_1/chain_7/generation_9/block_42/hash_{}/revision_0",
...
#[tokio::test(flavor = "multi_thread")]
#[serial]
async fn tip_selection_is_scoped_to_authenticated_publisher() {
    let (_instance, pool) = setup_archive_db().await;
