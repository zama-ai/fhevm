...
async fn explicit_calls_retry_failed_propagation_and_skip_completed_work() {
    ...
    sqlx::query("INSERT INTO computations (output_handle, dependencies, fhe_operation, is_scalar, transaction_id, host_chain_id, block_number, is_completed, is_allowed) VALUES ($1, ARRAY[$2, $3]::bytea[], 0, TRUE, $1, 1, 1, TRUE, TRUE)")
        .bind(bytes(2)).bind(bytes(1)).bind(vec![0u8; 32])
        .execute(&pool)
        .await
        .unwrap();
...
async fn epoch_change_between_passes_leaves_old_findings_uncontained() {
    ...
    let result = tokio::time::timeout(Duration::from_secs(5), containment)
        ...
        .unwrap()
        .unwrap();
    // Epoch is captured at pass start; handle identity does not include a
    // second row per commitment, so the pass still finishes.
    assert_eq!(
        result,
        PropagationResult {
            inferred_handles: 2,
    ...
    assert_eq!(recovered, PropagationResult::default());
