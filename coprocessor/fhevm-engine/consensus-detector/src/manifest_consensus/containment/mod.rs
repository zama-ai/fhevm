...
/// Inferred row to persist; identity matches `idx_drifted_handle_inferred_identity`.
...
async fn apply_event(
    ...
    if event.already_computed() {
        ...
        let (block_number, block_hash) =
            producer_identity(trx, chain, &event.output_handle).await?;
        ...
        let grown =
            drifted.add_inferred(chain, &event.output_handle, &stack.consensus_epoch, source);
        return Ok(grown);
    }
...
async fn producer_identity(
    trx: &mut Transaction<'_, Postgres>,
    chain: i64,
    handle: &[u8],
) -> Result<(i64, BlockHash)> {
    let rows = sqlx::query!(
    ...
    .await?;
        ...
        rows.len() == 1,
        "missing or ambiguous producer identity for handle {} on chain {chain}",
        hex::encode(handle)
    );
    let row = rows.into_iter().next().expect("one producer checked");
    Ok((row.producer_block_number, row.producer_block_hash))
}
