            ...
            SELECT EXISTS (
                SELECT 1
                  ...
                  JOIN block_manifest_verification_attempt attempt
                    USING (consensus_epoch, task_id, attempt)
                  JOIN block_manifest_verification_task task
                    ON task.id = drift.task_id AND task.consensus_epoch = drift.consensus_epoch
                  JOIN block_manifest manifest
                    ON manifest.id = task.local_manifest_id AND manifest.consensus_epoch = task.consensus_epoch
                 WHERE drift.consensus_epoch = $1
