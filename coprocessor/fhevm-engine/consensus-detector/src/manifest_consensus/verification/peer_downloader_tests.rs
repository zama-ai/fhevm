...
async fn healing_state_is_independent_of_later_manifest_agreement() {
    ...
    let inferred: i64 = sqlx::query_scalar("INSERT INTO drifted_handle (consensus_epoch, version, coprocessor_context_id, host_chain_id, block_number, block_hash, handle, detection_kind, reason, local_present, observed_present)
        SELECT consensus_epoch, version, coprocessor_context_id, host_chain_id, block_number, block_hash, $2, 'inferred', 'ct64_mismatch', TRUE, FALSE FROM drifted_handle WHERE id = $1 RETURNING id")
