...
#[derive(Clone, Debug)]
pub(crate) struct HistoricalCommitmentEvidence {
    pub(crate) publisher: Address,
    pub(crate) scope: CommitmentScope,
...
pub(crate) enum DriftExplanation {
    ...
    ComputationErrorMismatch {
        block_number: U256,
        handle: B256,
        local_is_error: bool,
        observed_is_error: bool,
    },
    UncomputedMismatch {
        block_number: U256,
        handle: B256,
...
pub(crate) fn evaluate_quorum(
    ...
    evaluate_quorum_with_history(manifests, &[], local_publisher, required_quorum)
}

pub(crate) fn evaluate_quorum_with_history(
    manifests: &[AuthenticatedManifest],
    derived_history: &[HistoricalCommitmentEvidence],
    local_publisher: Address,
    required_quorum: usize,
) -> QuorumEvaluation {
    let collected = collect_commitments(manifests, derived_history, local_publisher);
...
fn collect_commitments(
    ...
    for evidence in derived_history {
        collect_commitment(
            &mut collected,
...
fn record_descriptor_differences(
    ...
    if local.is_error() != observed.is_error() {
        explanations.push(DriftExplanation::ComputationErrorMismatch {
            block_number,
            handle,
            local_is_error: local.is_error(),
            observed_is_error: observed.is_error(),
        });
        return;
    }
    if local.is_error() {
    ...
    if local.is_uncomputed() != observed.is_uncomputed() {
        explanations.push(DriftExplanation::UncomputedMismatch {
            block_number,
            handle,
            local_is_uncomputed: local.is_uncomputed(),
            observed_is_uncomputed: observed.is_uncomputed(),
        });
        return;
    }
    ...
    let (
        CiphertextStatus::Computed {
            keyset_id: local_keyset,
            ct64_digest: local_ct64,
            ct128_digest: local_ct128,
            ct128_format: local_format,
            ..
        },
        CiphertextStatus::Computed {
            keyset_id: observed_keyset,
            ct64_digest: observed_ct64,
            ct128_digest: observed_ct128,
            ct128_format: observed_format,
            ..
        },
    ) = (&local.status, &observed.status)
    else {
        return;
    };
    if local_keyset != observed_keyset {
        explanations.push(DriftExplanation::KeysetMismatch {
            block_number,
            handle,
            local: *local_keyset,
            observed: *observed_keyset,
        });
    }
    ...
    if local_format != observed_format {
