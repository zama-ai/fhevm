...
/// mix consensus_epochs, publication blocks, or registry snapshots. Every processing
...
pub(super) async fn bind_task_to_current_registry(
    ...
    for (publisher, s3_bucket_url) in peers {
        sqlx::query!(
            ...
            VALUES ($4, $1, $2, $3)
            ON CONFLICT (consensus_epoch, task_id, publisher) DO NOTHING
            "#,
            task_id,
            publisher.as_slice(),
            s3_bucket_url,
    ...
    sqlx::query!(
        ...
        UPDATE block_manifest_verification_task
           SET next_attempt_at = GREATEST(
                   eligible_at,
                   NOW() + retry_delay_secs * INTERVAL '1 second'
               ),
               last_error = $2,
               updated_at = NOW()
         WHERE id = $1
