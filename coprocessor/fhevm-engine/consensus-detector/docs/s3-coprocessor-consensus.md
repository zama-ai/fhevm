...
```text
(consensus_epoch, host_chain_id, block_hash)
```
...
An empty
block is still sealed with a block-specific digest. A missing in-consensus epoch
...
delays those steps by up to one period. Host-chain block rows are kept at least
10,000 finalized blocks and one week after ingestion. If discovery stops for
longer than both, for example a consensus-detector outage of more than a week,
undiscovered host rows may be pruned before they can be copied into
...
1. loads every sealed block after the last successful local publication through the publication
   block. An unsealed in-consensus epoch predecessor aborts this prepare with a wait;
...
```text
<s3BucketUrl>/manifests/v_1/context_<coprocessor_context_id>/chain_<host_chain_id>/block_<block_number>/hash_<block_hash>/consensus_epoch/<consensus_epoch>/revision/<revision>
```

The immutable identity is:

```text
(consensus_epoch, publisher, version, coprocessor_context_id, host_chain_id,
...
One local row per `(consensus_epoch, host_chain_id, block_hash)`. It stores block
...
and polls on `--manifest-healing-poll-interval` (default **30s**), picks up to
`--manifest-healing-batch-size` (default **8**) due `can_be_healed` rows, and
downloads matching ct64 from those sources concurrently. A pass that
installs at least one handle NOTIFYs `work_available` once.
Verification fills `peer_sources` from the quorum group's
pinned registry S3 URLs (the same URLs used for manifest download). It fills
the quorum descriptor (`quorum_ct64_digest`, and computed `quorum_keyset_id` /
`quorum_ct128_*` when the threshold group has ciphertext material) and
...
metadata. They share the handle unique key with verified rows. Observation
...
`consensus_epoch_history` is the one shared history for Blue and Green. When the
host listener accepts a genuinely new upgrade proposal, it appends a `pending`
row for that string and one `consensus_epoch_block_window` per host-chain window.
...
the first upgrade. During an upgrade Green selects the new consensus epoch in its
GCS singleton while Blue continues selecting the public consensus epoch.

### Consensus epoch-scoped ciphertext discovery

Manifest discovery does not infer ciphertext consensus epoch from block numbers.
Consensus epoch ownership comes from the B/G schema boundary:
...
`blue_green_consensus_epoch` value in memory. Discovery labels completed rows from
that stack-routed schema with the pinned consensus epoch, and publication and
verification continue selecting work with that pinned value. Thus the
consensus epoch field is routing metadata for already-isolated computation results;
...
| Table | Consensus epoch columns and keys |
| --- | --- |
| `block_manifest_state` | `consensus_epoch` is part of the primary key so Blue and Green can retain state for the same block hash. |
| `block_range_commitment` | `consensus_epoch` is part of the identity because the same range and digest can exist in two consensus epochs. |
| `block_manifest` | `consensus_epoch` is part of the immutable object identity and every highest-revision or covering lookup. |
| `block_manifest_verification_task` | `consensus_epoch` is copied from its local manifest; a composite foreign key prevents disagreement with that manifest. |
| `block_manifest_peer_download` | The task consensus epoch is retained in its key and indexes because it selects a consensus epoch-qualified peer prefix. |
| `block_manifest_verification_attempt` | The task consensus epoch is part of its task foreign key. |
| `block_manifest_verification_attempt_drift` | The task consensus epoch is recorded for every range so an audit can reproduce its covering-manifest lookup. |
| `drifted_handle` | `consensus_epoch` scopes the local inventory and its verification-task references; it references the task’s `consensus_epoch` column. |

The task descendants intentionally copy the consensus epoch even though it is
derivable through joins. Consensus epoch-scoped queues, audit retention, and
rollback inspection then cannot depend on an accidental cross-consensus epoch join.

All manifest-derived tables live only in `public`. Blue and Green therefore
share one durable archive and work-state store, while consensus epoch-qualified keys,
foreign keys, selectors, and claims keep their mutable work independent. Green
can publish immediately under its new consensus epoch without copying or linking to
Blue's manifest lineage. Failed Green consensus epochs remain available for audit,
but are never selected as accepted lineage.

`blue_green_consensus_epoch` remains per-stack during overlap: Blue resolves the
...
are labeled by `consensus_epoch`. Handles, hashes, digests, URLs, and error strings
