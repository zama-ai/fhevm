...
pub const DEFAULT_GATEWAY_HTTP_MAX_RETRIES: u32 = 15;
...
pub fn is_gateway_retryable_transport_error(inner: &TransportErrorKind) -> bool {
    matches!(inner, TransportErrorKind::Custom(_))
        || matches!(
            inner,
...
pub fn is_gateway_transient_transport_error(inner: &TransportErrorKind) -> bool {
    inner.is_retry_err()
        || is_gateway_retryable_transport_error(inner)
        || is_gateway_provider_exhausted_transport_error(inner)
}
