...
enum MainBlockOneShotStaging {
    OneLogicalBlock,
    /// One logical block whose chains are deliberately NON-uniform: a single
    /// long chain against many single-transfer ones, all ready at once.
    ///
    /// This is the shape the adaptive work window is judged on. That window
    /// gives each acquired chain `ceil(work_items_batch_size / acquired)`
    /// transactions and does not redistribute what a short chain cannot use,
...
impl MainBlockOneShotScenario {
    ...
    /// Transfers in chain `index`. Uniform for every shape except the skewed
    /// one, which exists precisely to make the lengths differ.
    fn chain_len_at(self, index: usize) -> usize {
        match self.staging {
            MainBlockOneShotStaging::SkewedOneLogicalBlock { long_chain_len, .. } => {
                if index == 0 {
                    long_chain_len
                } else {
                    1
    ...
    fn cross_transaction_balance_dependency_edge_count(self) -> usize {
        if self.uses_distinct_transfer_transaction_ids() {
            // Summed per chain rather than multiplied out: a skewed shape has
            // no single chain length to multiply by, and a short chain of one
            // transfer contributes no links at all.
            (0..self.chain_count())
                .map(|index| {
                    self.chain_len_at(index).saturating_sub(1)
...
const MAIN_BLOCK_ONE_SHOT_SCENARIOS: &[MainBlockOneShotScenario] = &[
    ...
    // Canonical workload #7: deliberate chain-length skew, for the adaptive
    // work window. 200 transfers in one chain against 50 chains of one, all
    // ready in the same logical block.
    //
    // The adaptive window gives each acquired chain
    // ceil(work_items_batch_size / acquired_chains) transactions and does not
    // redistribute what a chain cannot use. Here 50 of the 51 chains can
    // contribute exactly one transaction each, so the window fills to roughly
    // `share + 50` instead of its configured size — at the bench defaults
    // (batch 1500, 51 chains) that is a share of 30 and a window of ~80
    // transactions where 1500 were budgeted. Every uniform scenario hides
    // this, because there every chain can fill its share.
    //
    // The same fixture measures what the adaptive window is FOR: with it off,
    // the 200-transfer chain owns the oldest schedule_order rows and fills the
    // whole window while the 50 short chains wait out the 90 s timeslice.
    MainBlockOneShotScenario {
        name: "skewed_chains_250_1x200_50x1",
        workload: MainBlockWorkload::Erc20,
        transfers: 250,
        chain_len: 200,
        // PerTransfer is load-bearing, not incidental. The adaptive share is
        // counted in TRANSACTIONS per chain, so with one transaction per chain
        // every chain would contribute its single transaction and the share
        // could never bind — the skew would be invisible. One transaction per
        // transfer gives the long chain 200 transactions to be capped at and
        // the short chains one apiece.
        erc20_transaction_identity: Erc20TransactionIdentity::PerTransfer,
        staging: MainBlockOneShotStaging::SkewedOneLogicalBlock {
            long_chain_len: 200,
            short_chains: 50,
        },
    },
...
async fn run_main_block_one_shot() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result: Result<MainBlockOneShotOutcome, Box<dyn std::error::Error>> = async {
        ...
        for chain_index in 0..scenario.chain_count() {
            let chain_len = scenario.chain_len_at(chain_index);
...
/// Auction counterpart of `encrypted_u64_inputs_from_xof`, differing only in
/// the values it encrypts.
///
/// Scopes the server key with `with_server_key_as_context` rather than
/// `set_server_key`: this runs on the harness thread, and a bare
/// `set_server_key` leaves the key installed in that thread's local for
/// everything that follows.
fn auction_encrypted_u64_inputs_from_xof() -> Result<[Vec<u8>; 4], Box<dyn std::error::Error>> {
    encrypted_u64_inputs_from_xof_values([400_000_u64, 2, 1, 0])
}

/// Encrypts `VALUES` under the xof keyset's compact public key and returns the
/// compressed ciphertexts, with the server key installed only for the duration
/// of the call.
fn encrypted_u64_inputs_from_xof_values<const N: usize>(
    values: [u64; N],
        ...
        for index in 0..N {
            ...
            out.push(
                SupportedFheCiphertexts::FheUint64(ciphertext)
                    .compress()
                    .map_err(|err| format!("compress benchmark input: {err:?}"))?,
            );
        }
        out.try_into()
            .map_err(|_| "encrypted input count mismatch".to_owned().into())
    })
}
