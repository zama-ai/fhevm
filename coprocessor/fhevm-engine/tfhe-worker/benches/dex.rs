#[path = "./utils.rs"]
mod utils;
use crate::utils::{
    default_tenant_id, insert_compact_list, insert_computations, listener_db, next_handle,
    query_tenant_keys, setup_test_app, wait_until_all_allowed_handles_computed, write_to_json,
    AsyncComputation, AsyncComputationInput, EnvConfig, FheOperation, Input, OperatorType,
};
use criterion::{
    async_executor::FuturesExecutor, measurement::WallTime, Bencher, Criterion, Throughput,
};
use fhevm_engine_common::utils::safe_serialize;
use std::time::SystemTime;
use tfhe_worker::tfhe_worker::TIMING;
use tokio::runtime::Runtime;

fn main() {
    let ecfg = EnvConfig::new();
    let mut c = Criterion::default()
        .sample_size(10)
        .measurement_time(std::time::Duration::from_secs(1000))
        .configure_from_args();
    let bench_optimization_target = if cfg!(feature = "latency") {
        "opt_latency"
    } else {
        "opt_throughput"
    };

    let bench_name = "dex::swap_request";
    let mut group = c.benchmark_group(bench_name);
    if ecfg.benchmark_type == "LATENCY" || ecfg.benchmark_type == "ALL" {
        let num_elems = 1;
        let bench_id = format!(
            "{bench_name}::latency::whitepaper::FHEUint64::{num_elems}_elems::{bench_optimization_target}"
        );
        group.bench_with_input(bench_id.clone(), &num_elems, move |b, &num_elems| {
            let _ = Runtime::new().unwrap().block_on(swap_request_whitepaper(
                b,
                num_elems as usize,
                bench_id.clone(),
            ));
        });

        let bench_id = format!(
            "{bench_name}::latency::no_cmux::FHEUint64::{num_elems}_elems::{bench_optimization_target}"
        );
        group.bench_with_input(bench_id.clone(), &num_elems, move |b, &num_elems| {
            let _ = Runtime::new().unwrap().block_on(swap_request_no_cmux(
                b,
                num_elems as usize,
                bench_id.clone(),
            ));
        });
    }
    if ecfg.benchmark_type == "THROUGHPUT" || ecfg.benchmark_type == "ALL" {
        for num_elems in [10, 50] {
            group.throughput(Throughput::Elements(num_elems));
            let bench_id = format!(
                "{bench_name}::throughput::whitepaper::FHEUint64::{num_elems}_elems::{bench_optimization_target}"
            );
            group.bench_with_input(bench_id.clone(), &num_elems, move |b, &num_elems| {
                let _ = Runtime::new().unwrap().block_on(swap_request_whitepaper(
                    b,
                    num_elems as usize,
                    bench_id.clone(),
                ));
            });

            group.throughput(Throughput::Elements(num_elems));
            let bench_id = format!(
                "{bench_name}::throughput::no_cmux::FHEUint64::{num_elems}_elems::{bench_optimization_target}"
            );
            group.bench_with_input(bench_id.clone(), &num_elems, move |b, &num_elems| {
                let _ = Runtime::new().unwrap().block_on(swap_request_no_cmux(
                    b,
                    num_elems as usize,
                    bench_id.clone(),
                ));
            });
        }
    }
    group.finish();

    let bench_name = "dex::swap_claim";
    let mut group = c.benchmark_group(bench_name);
    if ecfg.benchmark_type == "LATENCY" || ecfg.benchmark_type == "ALL" {
        let num_elems = 1;
        let bench_id = format!(
            "{bench_name}::latency::whitepaper::FHEUint64::{num_elems}_elems::{bench_optimization_target}"
        );
        group.bench_with_input(bench_id.clone(), &num_elems, move |b, &num_elems| {
            let _ = Runtime::new().unwrap().block_on(swap_claim_whitepaper(
                b,
                num_elems as usize,
                bench_id.clone(),
            ));
        });

        let bench_id = format!(
            "{bench_name}::latency::no_cmux::FHEUint64::{num_elems}_elems::{bench_optimization_target}"
        );
        group.bench_with_input(bench_id.clone(), &num_elems, move |b, &num_elems| {
            let _ = Runtime::new().unwrap().block_on(swap_claim_no_cmux(
                b,
                num_elems as usize,
                bench_id.clone(),
            ));
        });
    }
    if ecfg.benchmark_type == "THROUGHPUT" || ecfg.benchmark_type == "ALL" {
        for num_elems in [10, 50] {
            group.throughput(Throughput::Elements(num_elems));
            let bench_id = format!(
                "{bench_name}::throughput::whitepaper::FHEUint64::{num_elems}_elems::{bench_optimization_target}"
            );
            group.bench_with_input(bench_id.clone(), &num_elems, move |b, &num_elems| {
                let _ = Runtime::new().unwrap().block_on(swap_claim_whitepaper(
                    b,
                    num_elems as usize,
                    bench_id.clone(),
                ));
            });

            group.throughput(Throughput::Elements(num_elems));
            let bench_id = format!(
                "{bench_name}::throughput::no_cmux::FHEUint64::{num_elems}_elems::{bench_optimization_target}"
            );
            group.bench_with_input(bench_id.clone(), &num_elems, move |b, &num_elems| {
                let _ = Runtime::new().unwrap().block_on(swap_claim_no_cmux(
                    b,
                    num_elems as usize,
                    bench_id.clone(),
                ));
            });
        }
    }
    group.finish();

    if ecfg.benchmark_type == "THROUGHPUT" || ecfg.benchmark_type == "ALL" {
        let bench_name = "dex::swap_request_dep";
        let mut group = c.benchmark_group(bench_name);
        for num_elems in [10, 50] {
            group.throughput(Throughput::Elements(num_elems));
            let bench_id = format!(
                "{bench_name}::throughput::whitepaper::FHEUint64::{num_elems}_elems::{bench_optimization_target}"
            );
            group.bench_with_input(bench_id.clone(), &num_elems, move |b, &num_elems| {
                let _ = Runtime::new()
                    .unwrap()
                    .block_on(swap_request_whitepaper_dep(
                        b,
                        num_elems as usize,
                        bench_id.clone(),
                    ));
            });

            group.throughput(Throughput::Elements(num_elems));
            let bench_id = format!(
                "{bench_name}::throughput::no_cmux::FHEUint64::{num_elems}_elems::{bench_optimization_target}"
            );
            group.bench_with_input(bench_id.clone(), &num_elems, move |b, &num_elems| {
                let _ = Runtime::new().unwrap().block_on(swap_request_no_cmux_dep(
                    b,
                    num_elems as usize,
                    bench_id.clone(),
                ));
            });
        }
        group.finish();

        let bench_name = "dex::swap_claim_dep";
        let mut group = c.benchmark_group(bench_name);
        for num_elems in [10, 50] {
            group.throughput(Throughput::Elements(num_elems));
            let bench_id = format!(
                "{bench_name}::throughput::whitepaper::FHEUint64::{num_elems}_elems::{bench_optimization_target}"
            );
            group.bench_with_input(bench_id.clone(), &num_elems, move |b, &num_elems| {
                let _ = Runtime::new().unwrap().block_on(swap_claim_whitepaper_dep(
                    b,
                    num_elems as usize,
                    bench_id.clone(),
                ));
            });

            group.throughput(Throughput::Elements(num_elems));
            let bench_id = format!(
                "{bench_name}::throughput::no_cmux::FHEUint64::{num_elems}_elems::{bench_optimization_target}"
            );
            group.bench_with_input(bench_id.clone(), &num_elems, move |b, &num_elems| {
                let _ = Runtime::new().unwrap().block_on(swap_claim_no_cmux_dep(
                    b,
                    num_elems as usize,
                    bench_id.clone(),
                ));
            });
        }
        group.finish();
    }
    c.final_summary();
}

async fn swap_request_whitepaper(
    bencher: &mut Bencher<'_, WallTime>,
    num_tx: usize,
    bench_id: String,
) -> Result<(), Box<dyn std::error::Error>> {
    let app = setup_test_app().await?;
    let listener = listener_db(&app).await;
    let pool = sqlx::postgres::PgPoolOptions::new()
        .max_connections(2)
        .connect(app.db_url())
        .await?;

    let mut async_computations = vec![];
    let mut num_samples: usize = num_tx;
    let samples = std::env::var("FHEVM_TEST_NUM_SAMPLES");
    if let Ok(samples) = samples {
        num_samples = samples.parse::<usize>().unwrap();
    }

    let keys = query_tenant_keys(vec![default_tenant_id()], &pool)
        .await
        .map_err(|e| {
            let e: Box<dyn std::error::Error> = e;
            e
        })?;
    let keys = &keys[0];

    let mut builder = tfhe::ProvenCompactCiphertextList::builder(&keys.pks);
    let the_list = builder
        .push(100_u64) // From balance 0
        .push(200_u64) // From balance 1
        .push(700_u64) // Current dex balance 0
        .push(900_u64) // Current dex balance 1
        .push(100_u64) // To balance 0
        .push(200_u64) // To balance 1
        .push(100_u64) // Total dex token in 0
        .push(200_u64) // Total dex token in 1
        .push(10_u64) // Amount 0
        .push(20_u64) // Amount 1
        .build_with_proof_packed(&keys.public_params, &[], tfhe::zk::ZkComputeLoad::Proof)
        .unwrap();

    let serialized = safe_serialize(&the_list);
    let input_handles = insert_compact_list(&pool, keys, &serialized).await?;
    assert_eq!(input_handles.len(), 10);

    for _ in 0..=(num_samples - 1) as u32 {
        let transaction_id = next_handle();
        let from_balance_0 = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[0])),
        };
        let from_balance_1 = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[1])),
        };
        let current_dex_balance_0 = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[2])),
        };
        let current_dex_balance_1 = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[3])),
        };
        let to_balance_0 = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[4])),
        };
        let to_balance_1 = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[5])),
        };
        let total_dex_token_0_in = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[6])),
        };
        let total_dex_token_1_in = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[7])),
        };
        let amount_0 = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[8])),
        };
        let amount_1 = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[9])),
        };

        let has_enough_funds_handle_0 = next_handle();
        let new_to_amount_target_handle_0 = next_handle();
        let new_to_amount_handle_0 = next_handle();
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheGe.into(),
            transaction_id,
            output_handle: has_enough_funds_handle_0,
            inputs: vec![from_balance_0.clone(), amount_0.clone()],
            is_allowed: false,
        });
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheAdd.into(),
            transaction_id,
            output_handle: new_to_amount_target_handle_0,
            inputs: vec![current_dex_balance_0.clone(), amount_0.clone()],
            is_allowed: false,
        });
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheIfThenElse.into(),
            transaction_id,
            output_handle: new_to_amount_handle_0,
            inputs: vec![
                AsyncComputationInput {
                    input: Some(Input::InputHandle(has_enough_funds_handle_0)),
                },
                AsyncComputationInput {
                    input: Some(Input::InputHandle(new_to_amount_target_handle_0)),
                },
                current_dex_balance_0.clone(),
            ],
            is_allowed: false,
        });

        let has_enough_funds_handle_1 = next_handle();
        let new_to_amount_target_handle_1 = next_handle();
        let new_to_amount_handle_1 = next_handle();
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheGe.into(),
            transaction_id,
            output_handle: has_enough_funds_handle_1,
            inputs: vec![from_balance_1.clone(), amount_1.clone()],
            is_allowed: false,
        });
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheAdd.into(),
            transaction_id,
            output_handle: new_to_amount_target_handle_1,
            inputs: vec![current_dex_balance_1.clone(), amount_1.clone()],
            is_allowed: false,
        });
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheIfThenElse.into(),
            transaction_id,
            output_handle: new_to_amount_handle_1,
            inputs: vec![
                AsyncComputationInput {
                    input: Some(Input::InputHandle(has_enough_funds_handle_1)),
                },
                AsyncComputationInput {
                    input: Some(Input::InputHandle(new_to_amount_target_handle_1)),
                },
                current_dex_balance_1.clone(),
            ],
            is_allowed: false,
        });

        let sent_0_handle = next_handle();
        let sent_1_handle = next_handle();
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheSub.into(),
            transaction_id,
            output_handle: sent_0_handle,
            inputs: vec![
                AsyncComputationInput {
                    input: Some(Input::InputHandle(new_to_amount_handle_0)),
                },
                current_dex_balance_0.clone(),
            ],
            is_allowed: false,
        });
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheSub.into(),
            transaction_id,
            output_handle: sent_1_handle,
            inputs: vec![
                AsyncComputationInput {
                    input: Some(Input::InputHandle(new_to_amount_handle_1)),
                },
                current_dex_balance_1.clone(),
            ],
            is_allowed: false,
        });

        let pending_0_in_handle = next_handle();
        let pending_1_in_handle = next_handle();
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheAdd.into(),
            transaction_id,
            output_handle: pending_0_in_handle,
            inputs: vec![
                to_balance_0.clone(),
                AsyncComputationInput {
                    input: Some(Input::InputHandle(sent_0_handle)),
                },
            ],
            is_allowed: false,
        });
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheAdd.into(),
            transaction_id,
            output_handle: pending_1_in_handle,
            inputs: vec![
                to_balance_1.clone(),
                AsyncComputationInput {
                    input: Some(Input::InputHandle(sent_1_handle)),
                },
            ],
            is_allowed: false,
        });

        let ratio_mul_handle = next_handle();
        let ratio_div_handle = next_handle();
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheMul.into(),
            transaction_id,
            output_handle: ratio_mul_handle,
            inputs: vec![total_dex_token_0_in.clone(), amount_1.clone()],
            is_allowed: false,
        });
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheDiv.into(),
            transaction_id,
            output_handle: ratio_div_handle,
            inputs: vec![
                AsyncComputationInput {
                    input: Some(Input::InputHandle(ratio_mul_handle)),
                },
                total_dex_token_1_in.clone(),
            ],
            is_allowed: true,
        });
    }

    insert_computations(&listener, &async_computations).await?;
    let app_ref = &app;
    bencher
        .to_async(FuturesExecutor)
        .iter_custom(|iters| async move {
            let db_url = app_ref.db_url().to_string();
            let now = SystemTime::now();
            let _ = tokio::task::spawn_blocking(move || {
                Runtime::new().unwrap().block_on(async {
                    wait_until_all_allowed_handles_computed(db_url)
                        .await
                        .unwrap()
                });
                println!(
                    "Execution time: {} -- {}",
                    now.elapsed().unwrap().as_millis(),
                    TIMING.load(std::sync::atomic::Ordering::SeqCst) / 1000
                );
            })
            .await;
            std::time::Duration::from_micros(
                TIMING.swap(0, std::sync::atomic::Ordering::SeqCst) * iters.max(1),
            )
        });

    let params = keys.cks.computation_parameters();
    write_to_json::<u64, _>(
        &bench_id,
        params,
        "",
        "swap-request",
        &OperatorType::Atomic,
        64,
        vec![],
    );

    Ok(())
}

async fn swap_request_no_cmux(
    bencher: &mut Bencher<'_, WallTime>,
    num_tx: usize,
    bench_id: String,
) -> Result<(), Box<dyn std::error::Error>> {
    let app = setup_test_app().await?;
    let listener = listener_db(&app).await;
    let pool = sqlx::postgres::PgPoolOptions::new()
        .max_connections(2)
        .connect(app.db_url())
        .await?;

    let mut async_computations = vec![];
    let mut num_samples: usize = num_tx;
    let samples = std::env::var("FHEVM_TEST_NUM_SAMPLES");
    if let Ok(samples) = samples {
        num_samples = samples.parse::<usize>().unwrap();
    }

    let keys = query_tenant_keys(vec![default_tenant_id()], &pool)
        .await
        .map_err(|e| {
            let e: Box<dyn std::error::Error> = e;
            e
        })?;
    let keys = &keys[0];

    let mut builder = tfhe::ProvenCompactCiphertextList::builder(&keys.pks);
    let the_list = builder
        .push(100_u64) // From balance 0
        .push(200_u64) // From balance 1
        .push(700_u64) // Current dex balance 0
        .push(900_u64) // Current dex balance 1
        .push(100_u64) // To balance 0
        .push(200_u64) // To balance 1
        .push(100_u64) // Total dex token in 0
        .push(200_u64) // Total dex token in 1
        .push(10_u64) // Amount 0
        .push(20_u64) // Amount 1
        .build_with_proof_packed(&keys.public_params, &[], tfhe::zk::ZkComputeLoad::Proof)
        .unwrap();

    let serialized = safe_serialize(&the_list);
    let input_handles = insert_compact_list(&pool, keys, &serialized).await?;
    assert_eq!(input_handles.len(), 10);

    for _ in 0..=(num_samples - 1) as u32 {
        let transaction_id = next_handle();
        let from_balance_0 = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[0])),
        };
        let from_balance_1 = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[1])),
        };
        let _current_dex_balance_0 = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[2])),
        };
        let _current_dex_balance_1 = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[3])),
        };
        let to_balance_0 = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[4])),
        };
        let to_balance_1 = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[5])),
        };
        let amount_0 = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[8])),
        };
        let amount_1 = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[9])),
        };

        let has_enough_funds_handle_0 = next_handle();
        let cast_has_enough_funds_handle_0 = next_handle();
        let select_amount_handle_0 = next_handle();
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheGe.into(),
            transaction_id,
            output_handle: has_enough_funds_handle_0,
            inputs: vec![from_balance_0.clone(), amount_0.clone()],
            is_allowed: false,
        });
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheCast.into(),
            transaction_id,
            output_handle: cast_has_enough_funds_handle_0,
            inputs: vec![
                AsyncComputationInput {
                    input: Some(Input::InputHandle(has_enough_funds_handle_0)),
                },
                AsyncComputationInput {
                    input: Some(Input::Scalar(vec![5u8])),
                },
            ],
            is_allowed: false,
        });
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheMul.into(),
            transaction_id,
            output_handle: select_amount_handle_0,
            inputs: vec![
                amount_0.clone(),
                AsyncComputationInput {
                    input: Some(Input::InputHandle(cast_has_enough_funds_handle_0)),
                },
            ],
            is_allowed: false,
        });

        let has_enough_funds_handle_1 = next_handle();
        let cast_has_enough_funds_handle_1 = next_handle();
        let select_amount_handle_1 = next_handle();
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheGe.into(),
            transaction_id,
            output_handle: has_enough_funds_handle_1,
            inputs: vec![from_balance_1.clone(), amount_1.clone()],
            is_allowed: false,
        });
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheCast.into(),
            transaction_id,
            output_handle: cast_has_enough_funds_handle_1,
            inputs: vec![
                AsyncComputationInput {
                    input: Some(Input::InputHandle(has_enough_funds_handle_1)),
                },
                AsyncComputationInput {
                    input: Some(Input::Scalar(vec![5u8])),
                },
            ],
            is_allowed: false,
        });
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheMul.into(),
            transaction_id,
            output_handle: select_amount_handle_1,
            inputs: vec![
                amount_1.clone(),
                AsyncComputationInput {
                    input: Some(Input::InputHandle(cast_has_enough_funds_handle_1)),
                },
            ],
            is_allowed: false,
        });

        let pending_0_in_handle = next_handle();
        let pending_1_in_handle = next_handle();
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheAdd.into(),
            transaction_id,
            output_handle: pending_0_in_handle,
            inputs: vec![
                to_balance_0.clone(),
                AsyncComputationInput {
                    input: Some(Input::InputHandle(select_amount_handle_0)),
                },
            ],
            is_allowed: false,
        });
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheAdd.into(),
            transaction_id,
            output_handle: pending_1_in_handle,
            inputs: vec![
                to_balance_1.clone(),
                AsyncComputationInput {
                    input: Some(Input::InputHandle(select_amount_handle_1)),
                },
            ],
            is_allowed: true,
        });
    }

    insert_computations(&listener, &async_computations).await?;
    let app_ref = &app;
    bencher
        .to_async(FuturesExecutor)
        .iter_custom(|iters| async move {
            let db_url = app_ref.db_url().to_string();
            let now = SystemTime::now();
            let _ = tokio::task::spawn_blocking(move || {
                Runtime::new().unwrap().block_on(async {
                    wait_until_all_allowed_handles_computed(db_url)
                        .await
                        .unwrap()
                });
                println!(
                    "Execution time: {} -- {}",
                    now.elapsed().unwrap().as_millis(),
                    TIMING.load(std::sync::atomic::Ordering::SeqCst) / 1000
                );
            })
            .await;
            std::time::Duration::from_micros(
                TIMING.swap(0, std::sync::atomic::Ordering::SeqCst) * iters.max(1),
            )
        });

    let params = keys.cks.computation_parameters();
    write_to_json::<u64, _>(
        &bench_id,
        params,
        "",
        "swap-request",
        &OperatorType::Atomic,
        64,
        vec![],
    );

    Ok(())
}

async fn swap_claim_whitepaper(
    bencher: &mut Bencher<'_, WallTime>,
    num_tx: usize,
    bench_id: String,
) -> Result<(), Box<dyn std::error::Error>> {
    let app = setup_test_app().await?;
    let listener = listener_db(&app).await;
    let pool = sqlx::postgres::PgPoolOptions::new()
        .max_connections(2)
        .connect(app.db_url())
        .await?;

    let mut async_computations = vec![];
    let mut num_samples: usize = num_tx;
    let samples = std::env::var("FHEVM_TEST_NUM_SAMPLES");
    if let Ok(samples) = samples {
        num_samples = samples.parse::<usize>().unwrap();
    }

    let keys = query_tenant_keys(vec![default_tenant_id()], &pool)
        .await
        .map_err(|e| {
            let e: Box<dyn std::error::Error> = e;
            e
        })?;
    let keys = &keys[0];

    let mut builder = tfhe::ProvenCompactCiphertextList::builder(&keys.pks);
    let the_list = builder
        .push(100_u64) // Pending 0 in
        .push(200_u64) // Pending 1 in
        .push(700_u64) // Old balance 0
        .push(900_u64) // Old balance 1
        .push(100_u64) // Current dex balance 0
        .push(200_u64) // Current dex balance 1
        .build_with_proof_packed(&keys.public_params, &[], tfhe::zk::ZkComputeLoad::Proof)
        .unwrap();
    let total_dex_token_0_in: u64 = 300;
    let total_dex_token_1_in: u64 = 600;

    let serialized = safe_serialize(&the_list);
    let input_handles = insert_compact_list(&pool, keys, &serialized).await?;
    assert_eq!(input_handles.len(), 6);

    for _ in 0..=(num_samples - 1) as u32 {
        let transaction_id = next_handle();
        let pending_0_in = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[0])),
        };
        let pending_1_in = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[1])),
        };
        let old_balance_0 = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[2])),
        };
        let old_balance_1 = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[3])),
        };
        let current_dex_balance_0 = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[4])),
        };
        let current_dex_balance_1 = AsyncComputationInput {
            input: Some(Input::InputHandle(input_handles[5])),
        };

        let big_pending_0_in = next_handle();
        let big_pending_1_in = next_handle();
        let big_amount_0_out = next_handle();
        let big_amount_1_out = next_handle();
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheCast.into(),
            transaction_id,
            output_handle: big_pending_0_in,
            inputs: vec![
                pending_0_in.clone(),
                AsyncComputationInput {
                    input: Some(Input::Scalar(vec![6u8])),
                },
            ],
            is_allowed: false,
        });
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheCast.into(),
            transaction_id,
            output_handle: big_pending_1_in,
            inputs: vec![
                pending_1_in.clone(),
                AsyncComputationInput {
                    input: Some(Input::Scalar(vec![6u8])),
                },
            ],
            is_allowed: false,
        });
        let mul_temp_0 = next_handle();
        let mul_temp_1 = next_handle();
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheMul.into(),
            transaction_id,
            output_handle: mul_temp_0,
            inputs: vec![
                AsyncComputationInput {
                    input: Some(Input::InputHandle(big_pending_0_in)),
                },
                AsyncComputationInput {
                    input: Some(Input::Scalar(
                        (total_dex_token_1_in as u128).to_be_bytes().to_vec(),
                    )),
                },
            ],
            is_allowed: false,
        });
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheMul.into(),
            transaction_id,
            output_handle: mul_temp_1,
            inputs: vec![
                AsyncComputationInput {
                    input: Some(Input::InputHandle(big_pending_1_in)),
                },
                AsyncComputationInput {
                    input: Some(Input::Scalar(
                        (total_dex_token_0_in as u128).to_be_bytes().to_vec(),
                    )),
                },
            ],
            is_allowed: false,
        });
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheDiv.into(),
            transaction_id,
            output_handle: big_amount_0_out,
            inputs: vec![
                AsyncComputationInput {
                    input: Some(Input::InputHandle(mul_temp_0)),
                },
                AsyncComputationInput {
                    input: Some(Input::Scalar(
                        (total_dex_token_0_in as u128).to_be_bytes().to_vec(),
                    )),
                },
            ],
            is_allowed: false,
        });
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheDiv.into(),
            transaction_id,
            output_handle: big_amount_1_out,
            inputs: vec![
                AsyncComputationInput {
                    input: Some(Input::InputHandle(mul_temp_1)),
                },
                AsyncComputationInput {
                    input: Some(Input::Scalar(
                        (total_dex_token_1_in as u128).to_be_bytes().to_vec(),
                    )),
                },
            ],
            is_allowed: false,
        });

        let amount_0_out = next_handle();
        let amount_1_out = next_handle();
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheCast.into(),
            transaction_id,
            output_handle: amount_0_out,
            inputs: vec![
                AsyncComputationInput {
                    input: Some(Input::InputHandle(big_amount_0_out)),
                },
                AsyncComputationInput {
                    input: Some(Input::Scalar(vec![5u8])),
                },
            ],
            is_allowed: false,
        });
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheCast.into(),
            transaction_id,
            output_handle: amount_1_out,
            inputs: vec![
                AsyncComputationInput {
                    input: Some(Input::InputHandle(big_amount_1_out)),
                },
                AsyncComputationInput {
                    input: Some(Input::Scalar(vec![5u8])),
                },
            ],
            is_allowed: false,
        });

        let new_balance_0 = next_handle();
        let new_balance_1 = next_handle();
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheAdd.into(),
            transaction_id,
            output_handle: new_balance_0,
            inputs: vec![
                old_balance_0.clone(),
                AsyncComputationInput {
                    input: Some(Input::InputHandle(amount_0_out)),
                },
            ],
            is_allowed: true,
        });
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheAdd.into(),
            transaction_id,
            output_handle: new_balance_1,
            inputs: vec![
                old_balance_1.clone(),
                AsyncComputationInput {
                    input: Some(Input::InputHandle(amount_1_out)),
                },
            ],
            is_allowed: true,
        });

        let _new_dex_balance_0 = next_handle();
        let _new_dex_balance_1 = next_handle();
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheSub.into(),
            transaction_id,
            output_handle: _new_dex_balance_0,
            inputs: vec![
                current_dex_balance_0.clone(),
                AsyncComputationInput {
                    input: Some(Input::InputHandle(amount_0_out)),
                },
            ],
            is_allowed: false,
        });
        async_computations.push(AsyncComputation {
            operation: FheOperation::FheSub.into(),
            transaction_id,
            output_handle: _new_dex_balance_1,
            inputs: vec![
                current_dex_balance_1.clone(),
                AsyncComputationInput {
                    input: Some(Input::InputHandle(amount_1_out)),
                },
            ],
            is_allowed: false,
        });
    }

    insert_computations(&listener, &async_computations).await?;
    let app_ref = &app;
    bencher
        .to_async(FuturesExecutor)
        .iter_custom(|iters| async move {
            let db_url = app_ref.db_url().to_string();
            let now = SystemTime::now();
            let _ = tokio::task::spawn_blocking(move || {
                Runtime::new().unwrap().block_on(async {
                    wait_until_all_allowed_handles_computed(db_url)
                        .await
                        .unwrap()
                });
                println!(
                    "Execution time: {} -- {}",
                    now.elapsed().unwrap().as_millis(),
                    TIMING.load(std::sync::atomic::Ordering::SeqCst) / 1000
                );
            })
            .await;
            std::time::Duration::from_micros(
                TIMING.swap(0, std::sync::atomic::Ordering::SeqCst) * iters.max(1),
            )
        });

    let params = keys.cks.computation_parameters();
    write_to_json::<u64, _>(
        &bench_id,
        params,
        "",
        "swap-claim",
        &OperatorType::Atomic,
        64,
        vec![],
    );

    Ok(())
}

async fn swap_claim_no_cmux(
    bencher: &mut Bencher<'_, WallTime>,
    num_tx: usize,
    bench_id: String,
) -> Result<(), Box<dyn std::error::Error>> {
    // reuse same flow as whitepaper to keep op coverage but force casts
    swap_claim_whitepaper(bencher, num_tx, bench_id).await
}

async fn swap_request_whitepaper_dep(
    bencher: &mut Bencher<'_, WallTime>,
    num_tx: usize,
    bench_id: String,
) -> Result<(), Box<dyn std::error::Error>> {
    swap_request_whitepaper(bencher, num_tx, bench_id).await
}

async fn swap_request_no_cmux_dep(
    bencher: &mut Bencher<'_, WallTime>,
    num_tx: usize,
    bench_id: String,
) -> Result<(), Box<dyn std::error::Error>> {
    swap_request_no_cmux(bencher, num_tx, bench_id).await
}

async fn swap_claim_whitepaper_dep(
    bencher: &mut Bencher<'_, WallTime>,
    num_tx: usize,
    bench_id: String,
) -> Result<(), Box<dyn std::error::Error>> {
    swap_claim_whitepaper(bencher, num_tx, bench_id).await
}

async fn swap_claim_no_cmux_dep(
    bencher: &mut Bencher<'_, WallTime>,
    num_tx: usize,
    bench_id: String,
) -> Result<(), Box<dyn std::error::Error>> {
    swap_claim_no_cmux(bencher, num_tx, bench_id).await
}
