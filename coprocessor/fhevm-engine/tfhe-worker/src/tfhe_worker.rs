...
// Execution panics can be transient (for example, device or allocation
// pressure). Keep their DB stamp visible, but admit them for a later retry;
// other error stamps remain terminal.
...
mod operand_boundary_mask_tests {
    ...
    fn null_mask_derives_boundary_bits_from_transaction_membership() {
        ...
        let prepared =
            prepare_transaction_ops(&txwork, dcid.map(|d| vec![d]).as_deref()).expect("prepared");
    ...
    fn foreign_invalid_content_defers_instead_of_erroring() {
        ...
        assert!(prepare_transaction_ops(&txwork, Some(std::slice::from_ref(&ours))).is_err());
    }
    ...
    fn malformed_mask_defers_the_transaction() {
        ...
        assert!(prepare_transaction_ops(&txwork, dcid.map(|d| vec![d]).as_deref()).is_err());
    }
    ...
    fn foreign_local_producer_joins_as_recompute_only() {
        ...
        let prepared =
            prepare_transaction_ops(&txwork, Some(std::slice::from_ref(&ours))).expect("prepared");
        assert_eq!(prepared.ops.len(), 2);
...
WHERE c.transaction_id IN (
    ...
    FROM (
      ...
      WHERE is_completed = FALSE
        AND (is_error = FALSE OR error_message LIKE '%' || $5 || '%')
        AND is_allowed = TRUE
  ...
  -- and are excluded from results, persistence and completion.
    ...
    let (transactions, earliest_schedule_order, any_deferred) = async {
        ...
        for (transaction_id, txwork) in work_by_transaction.iter() {
            ...
            let prepared = match prepare_transaction_ops(txwork, dcid_filter.as_deref()) {
