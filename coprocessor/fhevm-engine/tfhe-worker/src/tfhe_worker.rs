...
FOR UPDATE SKIP LOCKED            ",
        ...
        // 1. Map allowed handles to transactions in this batch. Only
        // allowed handles can be used outside the transaction that
        // produces them.
        // let mut allowed_map: HashMap<&Handle, &Handle> = HashMap::new();
        // for (_, work) in work_by_tenant.iter() {
        //     for w in work.iter() {
        //         if w.is_allowed.unwrap_or(false) {
        //             allowed_map.insert(&w.output_handle, &w.transaction_id);
        //         }
        //     }
        // }
