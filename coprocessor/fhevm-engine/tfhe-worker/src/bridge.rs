...
pub async fn run_confidential_bridge(
    ...
    loop {
        ...
        match drain_associations(&pool, args.bridge_associate_batch_size).await {
...
pub(crate) async fn drain_associations(pool: &PgPool, batch_size: i64) -> Result<u64, sqlx::Error> {
    let mut total = 0;
    loop {
