...
pub(crate) async fn drain_associations(
    ...
    drain_associations_at_cutover(pool, batch_size, cancel_token, i64::MAX, gcs_mode).await
}
...
pub(crate) async fn drain_associations_at_cutover(
    ...
    loop {
        ...
        let associated = associate_batch(pool, batch_size, branch_cutover_block, gcs_mode).await?;
        total += associated;
