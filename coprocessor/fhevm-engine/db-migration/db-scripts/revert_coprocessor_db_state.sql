...
-- ===========================================================================
-- Collect affected output handles, derived from the `computations` table by
-- block_number. These drive ciphertext byte deletion only; PBS-only reverts
-- must not delete canonical ciphertext bytes.
-- ===========================================================================
...
-- SNS digest rows are affected both by reverted producers and by reverted
-- PBS/ACL work for a retained producer.
...
-- latest key, a key rotation may have occurred. This check is best-effort: if
-- the sns-worker hasn't processed all handles, some rows may be missing and
-- the check will pass (false negative).
-- ===========================================================================
...
-- `_affected_output_handles` are not in it yet. Add them here too so the
...
-- ===========================================================================
-- Delete ACL/PBS/digest state and ciphertext bytes produced by reverted rows,
-- preserving bytes still referenced by a retained computation.
-- ===========================================================================
