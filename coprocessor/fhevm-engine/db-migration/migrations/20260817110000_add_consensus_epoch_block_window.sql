-- Persist every host-chain ownership window for a manifest consensus_epoch.
-- `allocated_at` remains audit metadata and must not be used as a discovery
-- boundary. A proposal has one global consensus_epoch but may carry a distinct
-- block window for each host chain.
CREATE TABLE consensus_epoch_block_window
(
    consensus_epoch TEXT NOT NULL
        CHECK (consensus_epoch <> '' AND LENGTH(consensus_epoch) <= 256)
        REFERENCES consensus_epoch_history(consensus_epoch) ON DELETE CASCADE,
    host_chain_id BIGINT NOT NULL CHECK (host_chain_id >= 0),
    start_block BIGINT NOT NULL CHECK (start_block >= 0),
    consensus_deadline_block BIGINT NOT NULL
        CHECK (consensus_deadline_block >= start_block),

    PRIMARY KEY (consensus_epoch, host_chain_id)
);
