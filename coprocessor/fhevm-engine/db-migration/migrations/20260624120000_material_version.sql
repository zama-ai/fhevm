...
-- material. `compressed_key_state` only tells readers whether that column is
-- native material (NULL, read with today's COALESCE path) or migrated material
-- that must be hidden until a finalized cutover says to use it.
ALTER TABLE keys
ADD COLUMN IF NOT EXISTS compressed_key_state SMALLINT NULL
CHECK (compressed_key_state IN (1, 2));
...
-- there is no version column -- the table IS the legacy-key cutover.
CREATE TABLE IF NOT EXISTS legacy_key_host_cutover (
...
CREATE TABLE IF NOT EXISTS legacy_key_gateway_cutover (
