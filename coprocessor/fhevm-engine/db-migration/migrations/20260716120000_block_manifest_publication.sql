...
(
    ...
    -- Current-revision publication errors. A NULL retry time after an error
    -- means the configured publication retry limit is exhausted.
    publication_error_count BIGINT NOT NULL DEFAULT 0 CHECK (publication_error_count >= 0),
    publication_last_error TEXT NULL,
    publication_next_retry_at TIMESTAMPTZ NULL,

    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),

    PRIMARY KEY (generation, host_chain_id, block_hash),
    CHECK (
        (
            manifest_published
...
(
    end_block_generation BIGINT NOT NULL DEFAULT 0
        CHECK (end_block_generation >= 0),
    host_chain_id BIGINT NOT NULL CHECK (host_chain_id >= 0),
...
(
    ...
    UNIQUE (generation, publisher, object_key),
    UNIQUE (id, generation)
);

CREATE INDEX IF NOT EXISTS idx_block_manifest_state_parent
ON block_manifest_state (generation, host_chain_id, parent_block_hash);

-- Durable per-chain discovery frontier. Descending order also supports the
-- bounded overlap scan used after detector restarts.
CREATE INDEX IF NOT EXISTS idx_block_manifest_state_discovery_frontier
ON block_manifest_state (generation, host_chain_id, block_number DESC);

CREATE INDEX IF NOT EXISTS idx_block_manifest_state_pending_manifest
ON block_manifest_state (generation, host_chain_id, block_number, block_hash)
