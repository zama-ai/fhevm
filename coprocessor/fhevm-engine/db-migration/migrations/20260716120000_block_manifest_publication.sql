...
(
    ...
    PRIMARY KEY (consensus_epoch, host_chain_id, block_hash),
    CHECK (
        (
            manifest_published
...
(
    ...
    UNIQUE (consensus_epoch, publisher, object_key),
    UNIQUE (id, consensus_epoch)
);

CREATE INDEX IF NOT EXISTS idx_block_manifest_state_parent
ON block_manifest_state (consensus_epoch, host_chain_id, parent_block_hash);
...
ON block_manifest_state (consensus_epoch, host_chain_id, block_number DESC);

CREATE INDEX IF NOT EXISTS idx_block_manifest_state_pending_manifest
ON block_manifest_state (consensus_epoch, host_chain_id, block_number, block_hash)
