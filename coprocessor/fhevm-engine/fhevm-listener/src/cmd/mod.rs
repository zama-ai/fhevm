...
impl InfiniteLogIter {
        ...
        for _ in 0..REORG_RETRY_GET_LOGS {
            ...
            match logs {
                ...
                Err(err) => {
                    ...
                    tokio::time::sleep(Duration::from_millis(RETRY_GET_LOGS_DELAY_IN_MS)).await;
                    continue;
                }
            }
        }
    ...
    async fn check_reorgs(&mut self) -> bool {
        ...
        if cfg!(feature = "test-simulate-reorg") {
            eprintln!("Missing ancestors detected: {} blocks. Probably a simulated reorg.", missing_blocks.len());
        }
        warn!(
