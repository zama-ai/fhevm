...
use fhevm_engine_common::healthz_server::{
    HealthCheckService, HealthStatus, Version,
};
use fhevm_engine_common::types::BlockchainProvider;

const IS_ALIVE_TICK_FRESHNESS: u64 = 20; // Not aline if tick is older
...
impl HealthCheckService for HealthCheck {
    async fn health_check(&self) -> HealthStatus {
        ...
        status.set_custom_check("alive", check_alive);
        // blockchain
        if self
            .blockchain_tick
            ...
            .await
        {
            status.set_custom_check("blockchain_provider", true);
            ...
            (*self.blockchain_provider.read().await).clone()
        {
            ...
            status.set_custom_check("blockchain_provider", false);
        };
        // database
