    ...
    GatewayListener<P, A>
{
                    ...
                    for log in logs {
                        match log.address() {
                            a if a == self.input_verification_address => {
                                ...
                                } else if let Ok(event) = VerifyProofRequestNative::decode_log(&log.inner) {
                                    self.verify_proof_request_native(db_pool, event.data, log.clone()).await
                                        .inspect(|_| {
                                            verify_proof_success += 1;
                                        })
                                        .inspect_err(|e| {
                                            error!(error = %e, "VerifyProofRequestNative processing failed");
                                            VERIFY_PROOF_FAIL_COUNTER.inc();
                                        })?;
                                } else {
    ...
    async fn verify_proof_request_native(
        &self,
        db_pool: &Pool<Postgres>,
        request: VerifyProofRequestNative,
        log: Log,
    ) -> anyhow::Result<()> {
        let transaction_id = log.transaction_hash.map(|h| h.to_vec()).unwrap_or_default();
        info!(
            ...
            "Received native ZK proof request event"
        );
