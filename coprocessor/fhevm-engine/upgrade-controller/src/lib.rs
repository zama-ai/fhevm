...
/// Completes the generation recorded when the host listener accepted an upgrade.
        ...
        WITH claimed AS (
            UPDATE upgrade_state u
               SET state = 'PAUSED', status = 'failed',
                   ...
                   synthetic_txn_hashes = ''::bytea, updated_at = NOW()
             WHERE u.stack_role = 'GCS'
...
async fn select_green_generation(
    ...
    if updated.rows_affected() != 1 {
        return Err(Error::InvalidState(
            "active upgrade has no pending generation history row".to_owned(),
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    async fn execute_cutover_merges_match_live_unique_keys() {
        ...
        create_gcs_schema(&pool).await.expect("create gcs schema");
        select_green_generation(&pool, &[0x02; 32], 50)
            .await
        ...
        let (generation, outcome, completed): (String, String, bool) = sqlx::query_as(
    ...
    async fn rollback_serializes_guarded_writes_and_pruning() {
        ...
        let mut rollback =
            tokio::spawn(
                async move { rollback_dry_run(&rollback_pool, &[0x02; 32], 10, 200).await },
            );
