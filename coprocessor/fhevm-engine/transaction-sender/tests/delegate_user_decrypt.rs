...
async fn delegate_user_decrypt_terminal_on_gw_config_error(
    ...
    for _ in 0..80 {
        ...
        .fetch_one(&env.db_pool)
        .await?;
        let on_gateway: bool = row.try_get("on_gateway")?;
        let gateway_nb_attempts: i64 = row.try_get("gateway_nb_attempts")?;
        let gateway_last_error: Option<String> = row.try_get("gateway_last_error")?;
        if !on_gateway
