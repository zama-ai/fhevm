...
async fn verify_proof_terminal_on_gw_config_error(
    ...
    for _ in 0..60 {
        ...
        let retry_count = rows
            .first()
            .and_then(|row| row.try_get::<i32, _>("retry_count").ok());
        let last_error = rows
            .first()
    ...
    .await?;
    assert_eq!(rows.len(), 1);
    let retry_count: i32 = rows[0].try_get("retry_count")?;
    let last_error: Option<String> = rows[0].try_get("last_error")?;
    assert_eq!(retry_count, conf.verify_proof_resp_max_retries as i32);
    assert!(
        last_error
            .as_deref()
            .is_some_and(|err| err.starts_with("gw: ")),
