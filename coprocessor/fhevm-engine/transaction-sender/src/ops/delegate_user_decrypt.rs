...
impl<P: Provider<Ethereum> + Clone + 'static> DelegateUserDecryptOperation<P> {
    ...
    async fn send_transaction(
        ...
        let transaction = match transaction {
            ...
            Err(e) if self.idempotency_error(&e).is_some() => {
                warn!(
                    ...
                    ?delegation,
                    "DelegateUserDecryptOperation is already included in contract",
                );
            ...
            Err(error @ RpcError::ErrorResp(_)) if is_nonce_error(&error) => {
                ...
                warn!(
                    ...
                    %error,
                    ?delegation,
                    "DelegateUserDecryptOperation sending with transient nonce error. Will retry indefinitively"
                );
            ...
            Err(error) if is_transient_error(&error) => {
                ...
                warn!(
                    ...
                    %error,
                    ?delegation,
                    "DelegateUserDecryptOperation sending with transient error. Will retry indefinitively"
                );
                return TxResult::TransientError;
            }
            Err(error) => {
                ...
                warn!(
                    ...
                    %error,
                    ?delegation,
                    "Transaction sending failed with unexpected error"
                );
        ...
        if receipt.status() {
            info!(
                %transaction_hash,
                ?delegation,
                "DelegationForUserDecryption txn succeeded"
            );
            ...
            error!(
                ...
                ?delegation,
                "DelegationForUserDecryption txn failed"
            );
    ...
    pub async fn tx_check_ready_delegations(
        ...
        DELEGATE_USER_DECRYPT_ERROR_BACKLOG.set(past_error_backlog as f64);
