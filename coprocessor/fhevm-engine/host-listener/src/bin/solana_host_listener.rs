//! Live Solana host listener: reconstructs `zama-host` events from finalized
//! Yellowstone transaction and sysvar streams, then ingests them into the
...
async fn main() -> Result<()> {
    ...
    run_grpc(&db, &args, program_id, cancel).await
}
...
async fn run_grpc(
    ...
    let mut config = SolanaGrpcListenerConfig {
        ...
        // Auto-detected from the on-chain HostConfig below.
        chain_id: 0,
...
async fn run_grpc(
    ...
    anyhow::bail!(
        "solana_host_listener requires building host-listener with --features solana-grpc"
    )
}
