        ...
        if is_bridge_address {
                ...
                BridgeContract::BridgeContractEvents::decode_log(&log.inner)
            {
                    ...
                    &event.data
                {
                    ...
                    // the source of truth. A later event is skipped: the
                    // computation for the handle either already exists or is
                    // about to be created by an earlier event in this block.
                    let first_in_block =
                            ...
                            .await?
                    {
                        warn!(
                            dst_handle = ?dst_handle,
                            "Ignoring FallbackGrantedPlaintext: a computation for dstHandle already exists"
                        );
                        continue;
                    }
