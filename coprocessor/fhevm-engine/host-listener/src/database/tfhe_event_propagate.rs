...
impl Database {
    ...
    pub async fn handle_bridge_event(
        ...
        let inserted = match &event.data {
            BridgeContractEvents::BridgeHandle(e) => {
                ...
                // SnS is enqueued by ingest_block_logs (enqueue_bridge_source_pbs).
                sqlx::query!(
    ...
    /// Enqueues SnS for bridged handles: `send` accepts a transient allowance,
    /// which emits no ACL event. Call after this block's TFHE events are
    /// inserted so a same-transaction producer resolves.
