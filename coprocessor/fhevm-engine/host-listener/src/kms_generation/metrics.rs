...
pub(crate) static KMS_EVENT_INVALID_COUNTER: LazyLock<IntCounter> =
    LazyLock::new(|| {
        register_int_counter!(
            "coprocessor_host_listener_kms_event_invalid_counter",
            "Number of decoded KMSGeneration logs rejected during host-listener preparation"
        )
        .unwrap()
    });
