...
/// The decoder must read the consensus version out of the software version.
#[tokio::test]
#[serial(db)]
async fn test_upgrade_proposal_reads_consensus_version(
) -> Result<(), anyhow::Error> {
    let setup = setup(None).await?;
    let mut args = setup.args.clone();
    args.service_name.clear();

    // The proposal must cover every configured host chain.
    sqlx::query("INSERT INTO host_chains (chain_id, name, acl_contract_address) VALUES ($1, 'test', '0x0') ON CONFLICT DO NOTHING")
        .bind(setup.chain_id.as_i64())
        .execute(&setup.db_pool)
        .await?;
    ...
    let block = 10_u64;
    let windows = vec![ProtocolConfigTest::ChainUpgradeWindow {
        chainId: setup.chain_id.as_i64() as u64,
        startBlock: block,
        endBlock: block + 100,
    }];

    // An unreadable software version must be dropped, not stored as "unchanged".
    setup
        .protocol_config_contract
        .emitCoprocessorUpgradeProposed(
            U256::from(1_u64),
            "v0.15.0".to_string(),
            windows.clone(),
            block,
    ...
    let mut row = None;
    for _ in 0..60 {
        row = sqlx::query_as::<_, (Option<String>, Option<i64>)>(
            "SELECT version, consensus_version FROM upgrade_state WHERE stack_role = 'GCS'",
        )
        .fetch_optional(&setup.db_pool)
        .await?;
        if row.as_ref().is_some_and(|(_, c)| c.is_some()) {
            break;
        }
        tokio::time::sleep(tokio::time::Duration::from_secs(1)).await;
    }
    ...
    assert_eq!(
        row,
        Some((Some("1.0.0".to_string()), Some(1))),
        "the decoder must store the consensus version it read"
    );
    Ok(())
}
