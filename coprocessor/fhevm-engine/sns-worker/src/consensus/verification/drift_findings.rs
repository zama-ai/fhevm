...
use crate::consensus::manifest_archive::{load_manifest_by_reference, AuthenticatedManifest};
...
async fn upsert_finding(
    ...
    let observed_manifest_id = sqlx::query_scalar!(
        r#"
        SELECT id
          FROM block_manifest
         WHERE publisher = $1
    ...
    sqlx::query!(
        ...
        ON CONFLICT (
           ...
           SET block_number = EXCLUDED.block_number,
               ...
               observed_manifest_id = EXCLUDED.observed_manifest_id,
               ...
               resolved_at = NULL,
               updated_at = NOW()
         WHERE drifted_handle.last_observed_task_id
                   <= EXCLUDED.last_observed_task_id
        "#,
