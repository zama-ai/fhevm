use super::*;
use crate::consensus::manifest_archive::{
...
#[tokio::test(flavor = "multi_thread")]
#[ignore = "database-backed claim recovery after simulated downloader crash"]
#[serial]
async fn expired_claim_recovers_after_mid_download_crash_without_redownloading() {
    ...
    let claim = claim_due_task(&pool, "crashed-worker", Duration::from_secs(60))
        .await
...
async fn concurrent_workers_cover_five_copro_drift_populations_from_every_origin() {
    ...
    for scenario in &scenarios {
        for (origin, (signer, expected_outcome)) in
            signers.iter().zip(scenario.expected_outcomes).enumerate()
        {
            let row = sqlx::query(
                r#"
                SELECT task.id,
            ...
            if expected_outcome == VerificationOutcome::Drift {
                ...
                let persisted = sqlx::query(
                    ...
                    SELECT COUNT(*) AS finding_count,
                           BOOL_OR(observed_has_quorum) AS has_quorum_backed_difference
                      FROM drifted_handle
...
async fn concurrent_one_drifter_replay_resolves_exact_handle_findings() {
    ...
    let rows = sqlx::query(
        ...
        SELECT finding.local_publisher,
               ...
               observed_manifest.publisher AS observed_publisher,
               observed_manifest.manifest_digest AS observed_manifest_digest,
               finding.observed_commitment_digest,
               finding.observed_has_quorum,
               finding.first_detected_task_id,
          ...
          JOIN block_manifest observed_manifest
            ON observed_manifest.id = finding.observed_manifest_id
          JOIN block_manifest_verification_task detected
            ON detected.id = finding.first_detected_task_id
        ...
        .map(|((row, block), local_manifest)| (row, block, local_manifest))
    {
        ...
        assert_eq!(
            row.try_get::<i64, _>("first_detected_task_id").unwrap(),
            row.try_get::<i64, _>("last_observed_task_id").unwrap()
        );
    ...
    for row in &resolved_rows {
        ...
        let first_task_id = row.try_get::<i64, _>("first_detected_task_id").unwrap();
        let last_task_id = row.try_get::<i64, _>("last_observed_task_id").unwrap();
        assert_ne!(first_task_id, last_task_id);
        assert_eq!(
