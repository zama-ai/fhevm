...
static OLDEST_DUE_VERIFICATION_AGE_SECONDS: LazyLock<IntGauge> = LazyLock::new(|| {
    register_int_gauge!(
        ...
        "Age in seconds of the oldest due verification task"
    )
    .unwrap()
});
...
static VERIFICATION_TARGETS: LazyLock<IntGaugeVec> = LazyLock::new(|| {
    register_int_gauge_vec!(
        "coprocessor_sns_manifest_verification_tasks",
...
async fn update_consensus_gauges(pool: &PgPool) -> Result<(), sqlx::Error> {
    ...
    for row in sqlx::query!(
        "SELECT state, COUNT(*)::BIGINT AS \"count!\" FROM block_manifest_verification_task GROUP BY state"
    )
    .fetch_all(pool)
    .await?
    {
