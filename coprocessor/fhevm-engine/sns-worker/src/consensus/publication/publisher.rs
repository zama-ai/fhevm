...
use crate::consensus::{
    manifest::{
        ...
        pending_chain_ids, prepare_manifest, seal_block_content, ManifestProgressCursor,
        PendingBlock,
    },
...
async fn run_manifest_publisher_with_poll_interval(
    ...
    loop {
        ...
        loop {
            let chain_ids = pending_chain_ids(&pool).await?;
            ...
            for host_chain_id in chain_ids {
                match progress_chain(&pool, &client, &bucket, host_chain_id, &signer, &consensus)
                    .await
                {
