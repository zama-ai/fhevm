...
use super::manifest_frontier::{
    append_leaf, derive_range_scale, rebuild_frontier_from_manifest, validate_canonical_frontier,
};
...
async fn hydrate_historical_ranges(
    ...
    for historical in payload.historical_ranges.iter().rev() {
        ...
        let scale = derive_range_scale(start, end)?;
        if scale != manifest_scale {
            return Err(internal(format!(
                "historical range [{start}, {end}] declares scale {manifest_scale}, expected {scale}",
            )));
        }
