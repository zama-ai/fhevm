...
/// * `digest_signature` - Digital signature of the digest for verification purposes
/// * `digest_signer` - Address or identifier of the entity that signed the digest
...
impl CiphertextSideCar {
    ...
    pub async fn new(
        ...
        let signed_tuple = serde_json::to_vec(&signed_tuple)?;
        let digest_signer = signer.address().to_string();
        let signed = signer.sign_message(&signed_tuple).await?.as_bytes();
