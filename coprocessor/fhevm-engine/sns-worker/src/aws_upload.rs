...
async fn upload_ct(
    ...
    ct_bytes: Vec<u8>,
    result: UploadResult,
) -> Result<Option<UploadResult>, ExecutionError> {
    ...
    let upload = client
        ...
        .metadata("Digest-Signature", &sidecar.digest_signature)
        .metadata("Digest-Signer", &sidecar.digest_signer)
        .key(key)
    ...
    if let Err(err) = upload {
        ...
        return Err(ExecutionError::S3TransientError(err.to_string()));
    }
...
async fn upload_ciphertexts(
    ...
    if !task.ct128.is_empty() && task.ct128.format() != Ciphertext128Format::Unknown {
        ...
        if !exists {
            ...
            let digest_signature_ct128 = signer
                .sign_hash(&B256::from_slice(&ct128_digest))
                .await?
                .as_bytes();
            ...
            let upload_ct = tokio::spawn(upload_ct(
                span,
                client.clone(),
                bucket.clone(),
                key.clone(),
            ...
            let upload_sidecar = tokio::spawn(upload_sidecar(
                span,
                client.clone(),
                bucket.clone(),
                key.clone(),
    ...
    if !task.ct64_compressed.is_empty() {
        ...
        if !exists {
            let signed_ct64_digest = signer
                .sign_hash(&B256::from_slice(&ct64_digest))
                .await?
                .as_bytes();
            let sidecar = CiphertextSideCar::new(
                &tx_hash,
    ...
    for result in join_all(jobs).await {
        match result {
            Ok(Ok(None)) => (), // no upload result => sidecar 64 or 128
            Ok(Ok(Some(UploadResult::CtType128(digest)))) => {
                task.update_ct128_uploaded(&mut trx, digest).await?
            }
            Ok(Ok(Some(UploadResult::CtType64(digest)))) => {
            ...
            Err(err) => {
                error!(error = %err, handle = handle_as_hex, "Upload task failed");
