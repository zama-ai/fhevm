...
| Peer download | Delayed durable tasks, pinned registry data, per-peer rows, bounded retries, claims, per-object crash recovery, bounded bodies, recursive predecessor retrieval, and archival of every observed revision | Cross-height discovery when peers use different cadences; periodic reopening after exhaustion |
...
1. loads every sealed block after the previous manifest through the publication
   block;
2. rechecks each stored block handle count and digest;
...
After the configured verification delay, a worker binds one unbound pending task to
the current complete registry snapshot. It creates one durable row per peer, then
claims the task with an expiring claim.
...
The verifier selects each publisher's highest tip-eligible revision for the task
block identity. It compares only exact block ranges:
...
localization-completeness flag. On differences only,
...
descriptor values, the archived observed manifest, observed commitment and quorum
attribution, first/last task references, and resolution state. Mismatch kind and
per-field differences are derived from the two stored descriptors.
...
future protocol must also handle a peer accidentally using another cadence:
...
A report will contain commitment groups, quorum attribution, exact block range and
...
publication backlog, task states, oldest due-task age, latest publication time,
...
stalled publication, stale registry refresh, aging tasks, unresolved drift, and
...
- `coprocessor_sns_manifest_verification_tasks{state=...}`;
...
1. **Durable incident and job.** An immutable incident must pin the verification
   task, registry snapshot, observed groups, unique winner when one exists, local
   losing commitment, lineage, coverage, policy, and expected result. A separately
   claimed job must advance idempotently through replay, material completion,
...
8. **Continuous reconciliation and precise remission.** New local or peer
   ...
   evaluation after the original bounded task is `retry_exhausted`. Equal visible values
...
9. **Operational safety and audit.** Replay starts default-off with observation,
   dry-run, and manual-approval modes. Production requires a kill switch, bounded
