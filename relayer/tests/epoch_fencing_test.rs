...
//! epoch, and the specific terminal-write interleaving described below - an
...
/// Two advisory-lock keys for this test's two holders, pid-derived as in
/// `dispatch_gate_test.rs` so concurrent test processes never serialise on each other. They
/// differ in a high bit, not by an increment, so neighboring pids cannot alias.
fn test_lock_keys() -> (i32, i32) {
    let base = (std::process::id() as i32).saturating_abs() | 0x4000_0000;
    (base, base | 0x2000_0000)
}
...
/// The interleaving this guards against: a Postgres failover kills the old
