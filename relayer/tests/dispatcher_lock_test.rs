...
/// nothing on the client side ever gives the advisory lock back; a heartbeat loop stopped
/// dead over a still-open connection is that shape. `idle_session_timeout` on the dedicated
/// connection is the only thing that closes such a session, and the lock goes with it - so a
/// standby takes over within the bound instead of waiting out server-side TCP keepalive.
#[tokio::test]
async fn test_an_idle_holder_session_is_reaped_and_a_standby_takes_over() {
    let schema = TestSchema::new()
        .await
        .expect("Failed to create test schema");
    ...
    let lock_a = DispatcherLock::connect(&config, &schema.database_url())
        .await
        .expect("connect a");
    let (_shutdown_a, task_a) = spawn_run(&lock_a);
    wait_until_held(&lock_a, Duration::from_secs(5)).await;

    // Stop heartbeating without releasing the lock or closing the connection, and never call
    // `release_last` - a dead node calls nothing.
    task_a.abort();
    let _ = task_a.await;
    ...
    let lock_b = DispatcherLock::connect(&config, &schema.database_url())
        ...
        .expect("connect b");
    let (shutdown_b, task_b) = spawn_run(&lock_b);
    wait_until_held(&lock_b, Duration::from_secs(15)).await;
...
/// doing everything right. At the same order of ratio between the two settings as the
/// defaults carry, compressed onto a test timescale, the holder stays `Held` across many
/// multiples of the idle bound and the standby never gets in. A reaped session would fail its
/// next heartbeat, demote to `Unconfirmed`, and hand the key over.
#[tokio::test]
async fn test_a_heartbeating_holder_is_never_reaped() {
    let schema = TestSchema::new()
        .await
        .expect("Failed to create test schema");
    let config = DispatcherLockConfig {
    ...
    // Three times the idle bound, sixty heartbeats: a session reaped mid-heartbeat would be
    // long gone by the end of this.
    let deadline = tokio::time::Instant::now() + Duration::from_secs(3);
    while tokio::time::Instant::now() < deadline {
        assert_eq!(
            lock_a.state(),
            LockState::Held,
            "a heartbeating holder must never have its session reaped"
        );
        assert_eq!(
            lock_b.state(),
            LockState::NotHeld,
            "the standby must never acquire while the holder is healthy"
        );
    ...
    assert_eq!(
        ...
        Some(epoch_a),
        "no reacquisition happened, so the holder's epoch must be unchanged"
    );
