...
mod constants {
    ...
    // 3-3-3 block pattern, with the consensus event in the same block as the ninth
    // share (both done by 1.5s, as the contract emits them from one call), then one
    // straggler share per block from 2s. `user_decrypt_shares_threshold` is 9 in the
    // test config, so with `additional_shares = A` the relayer's target is `9 + A`.
...
mod helpers {
    ...
    pub async fn submit_against_mock_committee(setup: &TestSetup, share_count: usize) -> String {
        ...
        let delegator_address = random_address();
        let delegate_address = random_address();
        let payload = create_delegated_user_decrypt_payload(
            &setup.settings.gateway.blockchain_rpc.chain_id.to_string(),
            contract_address,
            delegator_address,
            delegate_address,
        );
        let handles = extract_ciphertext_handles_from_delegated_payload(&payload);
    ...
    /// Shared with the direct suite; both bodies live in `common::utils`.
    ...
    pub async fn poll_until_terminal_within(
        ...
        crate::common::utils::poll_user_decrypt_until_terminal_within(
            &v2_delegated_user_decrypt_get_url(setup, job_id),
            budget,
        )
        .await
    }
    ...
    pub async fn get_status(
        ...
        crate::common::utils::user_decrypt_get_status(&v2_delegated_user_decrypt_get_url(
            setup, job_id,
        ))
        .await
    }
...
async fn test_delegated_user_decrypt_contract_paused_returns_503() {
    ...
    assert_eq!(error.label(), "protocol_paused");
...
async fn test_insufficient_balance_returns_503() {
    ...
    let error = body.error.as_ref().expect("Error should be present");
    assert_eq!(error.label(), "insufficient_balance");
...
async fn test_insufficient_allowance_returns_503() {
    ...
    let error = body.error.as_ref().expect("Error should be present");
    assert_eq!(error.label(), "insufficient_allowance");
...
async fn test_not_allowed_on_host_acl_returns_400() {
    ...
    // Delegated: 1 handle → 1 isHandleDelegatedForUserDecryption call
...
/// 2 handle-contract pairs, deny index 1 → 400 with label "not_allowed_on_host_acl".
#[tokio::test]
async fn test_multi_pair_acl_partial_deny() {
    ...
    // Override default ACL with partial deny (index 1)
    let acl_address =
        Address::from_str(&setup.settings.host_chains[0].acl_address).expect("Invalid ACL address");
    setup.host_server.reset_state();
    register_host_acl_partial_deny(&setup.host_server, acl_address, vec![1]);
...
/// 2 handle-contract pairs, all denied → 400 with label "not_allowed_on_host_acl".
#[tokio::test]
...
// the quorum. The delegated GET shares its handler with the direct one, so these
// cases guard that the delegated surface is wired to the same behaviour.
//
// Metrics are asserted on the extra-share case only: both surfaces record under
// the same `req_type="user_decrypt"` label, so repeating the delta assertions in
// every case here would re-measure what the direct suite already covers.

/// `additional_shares = 0` disables the wait: no 202 hold, even with a long window.
#[tokio::test]
async fn test_delegated_wait_disabled_completes_at_threshold() {
    let temp_config_dir = TempDir::new().expect("Failed to create temp dir");
...
/// The wait ends as soon as the extra share arrives, and the spare is returned.
#[tokio::test]
async fn test_delegated_wait_window_returns_extra_share() {
    let temp_config_dir = TempDir::new().expect("Failed to create temp dir");
    let temp_config_path =
...
///
/// The negative case that matters most: the extra shares never arrive and the
/// request must still complete with what it has rather than erroring out.
#[tokio::test]
async fn test_delegated_wait_window_expiry_returns_quorum() {
    let temp_config_dir = TempDir::new().expect("Failed to create temp dir");
    let temp_config_path =
