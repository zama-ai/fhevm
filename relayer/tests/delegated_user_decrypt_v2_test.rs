...
async fn test_delegated_wait_window_returns_extra_share() {
    ...
    // Delegated shares the `user_decrypt` label with the direct endpoint: the delegated
    // handler forwards to the same insert and the same GET.
    ...
    assert_eq!(
        ...
        1.0,
        "A delegated request counts under the same req_type as the direct one"
    );
