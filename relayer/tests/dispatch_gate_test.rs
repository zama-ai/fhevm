//! The dispatch gate (build-order step 7).
//!
...
//!
//! What is deliberately *not* asserted here: anything about a second relayer process. Two
//! live relayers against one lock is build-order step 9.
...
/// The whole of step 7 in one flow: a relayer that is not the dispatcher still accepts an HTTP
...
async fn test_a_standby_accepts_without_driving_then_the_sweep_drives_it_after_handover() {
    ...
    let setup = TestSetup::new_with_settings(|settings| {
        ...
        // Poll and sweep fast enough that the handover below resolves inside the test budget.
    ...
    // unowned row and re-dispatches it through the same path startup recovery used to use.
