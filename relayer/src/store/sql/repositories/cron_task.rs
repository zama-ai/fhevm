...
async fn run_timeout_worker_logic(
    ...
    loop {
        // Only the confirmed dispatcher times rows out (build-order step 7). Two pods writing
...
async fn run_expiry_worker_logic(
    ...
    loop {
        // Gated with the rest (build-order step 7): deleting rows is not destructive to another
