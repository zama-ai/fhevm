    ...
    T: ReadinessItem + Send + Sync + 'static,
{
    ...
    ///
    /// Nothing is dequeued while `gate` is closed (build-order step 7), for the same reason as
    /// the transaction throttler: a pod that is not the confirmed dispatcher must not drive
    /// work, and a readiness check that passes would go on to mark the row `processing` and
    /// push it at the tx throttler.
