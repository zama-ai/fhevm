...
pub struct TokioEventDispatcher {
    ...
    /// Tracks the per-(event, handler) dispatch tasks spawned below. An event whose handlers
    /// have not returned never lets the block cursor past it, so the next start re-reads it
    /// from the chain.
...
impl TokioEventDispatcher {
    ...
    /// Dispatch, then wait until every subscribed handler has finished with the event. Returns
