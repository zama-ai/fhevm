...
pub enum TransactionStatus {
    Failed,
    Confirmed,
    /// A receipt came back from the chain, but recording it lost the epoch fence (build-order
    /// step 8): a successor already claimed the row. Distinct from `Failed` because the send
