...
///
/// Each fixed-size version rejects trailing bytes; the Solana `0x03` MMR-proof blob
/// is variable-length. New fields are introduced by bumping the version byte, not by
...
///   The canonical layout and proof parsing live in the kms-connector `solana_extra_data` module.
///
...
mod tests {
    ...
    fn extra_data_evm_paths_unchanged() {
        ...
        // v0x01 with a properly 0x07-tagged contextId is valid.
        let v1 = format!("0x01{CONTEXT_ID_HEX}");
        assert!(validate_extra_data_field_decryption(&v1).is_ok());
        // A correctly-sized 0x02 (132 chars) with tagged contextId + epochId is valid.
