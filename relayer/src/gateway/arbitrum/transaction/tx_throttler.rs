    ...
    T: MemoryThrottlerItem + Send + Sync + 'static,
{
    ...
    /// Runs the dequeue loop until `shutdown` fires. Driven from the token rather than
    /// channel closure: the sender half lives in an `Arc` held permanently by the
    /// dispatcher and the HTTP router state, so it never drops on its own.
    ///
    /// Nothing is dequeued while `gate` is closed (build-order step 7). A pod that is not the
    /// confirmed dispatcher should not have work queued here at all - intake does not dispatch
    /// on it, so nothing enqueues - but a pod that *loses* the lock mid-flight does, and
    /// sending those queued transactions would make it a second dispatcher for rows a successor
    /// may already have claimed. Items stay in the bounded channel: this pod either regains the
    /// lock and drains them, or exits on lock loss and the successor's sweep re-drives their
    /// rows from Postgres.
