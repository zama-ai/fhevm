...
impl TxLifecycleHooks for GatewayHandler {
    ...
    async fn on_receipt_received(
        ...
        if rows_affected == 0 {
            ...
            // row is no longer `tx_in_flight` by the time we get here - not "still tx_in_flight,
            // waiting for its own re-drive" as an earlier version of this comment assumed. Or,
            // within this pod's *own* epoch, its own later sweep tick reclaimed and reset this
            // same row (build-order step 8's `claim_after` staleness case) while this send was
            // still genuinely in flight - no epoch changed at all, just the status raced out
            // from under it. Either way `gw_reference_id` is NOT stored: this must not read as
