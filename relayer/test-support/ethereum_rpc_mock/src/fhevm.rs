...
impl FhevmMockWrapper {
    ...
    /// Coprocessor signer set, the tx-sender-to-bucket mapping and the majority threshold.
    ///
    ...
    pub fn set_coprocessor_registry(&self, attestation_mock: &CtAttestationMock) {
        ...
        let signers: Vec<Address> = coprocessors.iter().map(|c| c.signer).collect();
        self.on_gateway_config_view(
            GatewayConfig::getCoprocessorSignersCall::SELECTOR,
            signers.abi_encode(),
        );
