...
fn binary_op_to_event(op: &BinaryOperatorTestCase, lhs: &Handle, rhs: &Handle, r_scalar: &BigInt, result: &Handle) -> TfheContractEvents {
    ...
    let caller = "0x0000000000000000000000000000000000000000".parse().unwrap();
    ...
    match S::try_from(op.operand).unwrap() {
        S::FheAdd => E::FheAdd(C::FheAdd {caller, lhs, rhs, scalarByte, result}),
        S::FheSub => E::FheSub(C::FheSub {caller, lhs, rhs, scalarByte, result}),
        S::FheMul => E::FheMul(C::FheMul {caller, lhs, rhs, scalarByte, result}),
        S::FheDiv => E::FheDiv(C::FheDiv {caller, lhs, rhs, scalarByte, result}),
        S::FheRem => E::FheRem(C::FheRem {caller, lhs, rhs, scalarByte, result}),
        S::FheBitAnd => E::FheBitAnd(C::FheBitAnd {caller, lhs, rhs, scalarByte, result}),
        S::FheBitOr => E::FheBitOr(C::FheBitOr {caller, lhs, rhs, scalarByte, result}),
        S::FheBitXor => E::FheBitXor(C::FheBitXor {caller, lhs, rhs, scalarByte, result}),
        S::FheShl => E::FheShl(C::FheShl {caller, lhs, rhs, scalarByte, result}),
        S::FheShr => E::FheShr(C::FheShr {caller, lhs, rhs, scalarByte, result}),
        S::FheRotl => E::FheRotl(C::FheRotl {caller, lhs, rhs, scalarByte, result}),
        S::FheRotr => E::FheRotr(C::FheRotr {caller, lhs, rhs, scalarByte, result}),
        S::FheMax => E::FheMax(C::FheMax {caller, lhs, rhs, scalarByte, result}),
        S::FheMin => E::FheMin(C::FheMin {caller, lhs, rhs, scalarByte, result}),
        S::FheGe => E::FheGe(C::FheGe {caller, lhs, rhs, scalarByte, result}),
        S::FheGt => E::FheGt(C::FheGt {caller, lhs, rhs, scalarByte, result}),
        S::FheLe => E::FheLe(C::FheLe {caller, lhs, rhs, scalarByte, result}),
        S::FheLt => E::FheLt(C::FheLt {caller, lhs, rhs, scalarByte, result}),
        S::FheEq => if use_bytes_when_avail {
            E::FheEqBytes(C::FheEqBytes {caller, lhs, rhs:rhs_bytes, scalarByte, result})
        } else {
            E::FheEq(C::FheEq {caller, lhs, rhs, scalarByte, result})
        },
        S::FheNe => if use_bytes_when_avail {
            E::FheNeBytes(C::FheNeBytes {caller, lhs, rhs:rhs_bytes, scalarByte, result})
        } else {
            E::FheNe(C::FheNe {caller, lhs, rhs, scalarByte, result})
        },
...
async fn test_fhe_binary_operands_events() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for op in generate_binary_test_cases() {
        ...
        listener_event_to_db.insert_tfhe_event(&tfhe_event(
            TfheContractEvents::TrivialEncryptBytes(TfheContract::TrivialEncryptBytes {caller, pt:lhs_bytes, toType:to_ty(op.input_types), result:lhs_handle.clone()})
        )).await?;
        if !op.is_scalar {
            listener_event_to_db.insert_tfhe_event(&tfhe_event(
                TfheContractEvents::TrivialEncryptBytes(TfheContract::TrivialEncryptBytes {caller, pt:rhs_bytes, toType:to_ty(op.input_types), result:rhs_handle.clone()})
            )).await?;
        }
        ...
        println!("Checking computation for binary test bits:{} op:{} is_scalar:{} lhs:{} rhs:{} output:{}",
            op.bits, op.operand, op.is_scalar, op.lhs.to_string(), op.rhs.to_string(), decr_response.value);
        assert_eq!(
...
fn unary_op_to_event(op: &UnaryOperatorTestCase, input: &Handle, result: &Handle) -> TfheContractEvents {
    use fhevm_engine_common::types::SupportedFheOperations as S;
    use fhevm_listener::contracts::TfheContract as C;
    use fhevm_listener::contracts::TfheContract::TfheContractEvents as E;
    ...
    match S::try_from(op.operand).unwrap() {
        S::FheNot => E::FheNot(C::FheNot { caller, ct: input, result }),
        S::FheNeg => E::FheNeg(C::FheNeg { caller, ct: input, result }),
...
async fn test_fhe_unary_operands_events() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for op in &ops {
        ...
        let caller = "0x0000000000000000000000000000000000000000".parse().unwrap();
        listener_event_to_db.insert_tfhe_event(&tfhe_event(
            TfheContractEvents::TrivialEncryptBytes(TfheContract::TrivialEncryptBytes {
                caller,
                pt: inp_bytes,
...
async fn test_fhe_if_then_else_events() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for input_types in supported_types() {
        ...
        let (left_input, right_input) = if is_input_bool { (BigInt::from(0), BigInt::from(1)) } else { (BigInt::from(7), BigInt::from(12)) };
        ...
        for test_value in [false, true] {
            ...
            listener_event_to_db.insert_tfhe_event(&tfhe_event(
                TfheContractEvents::FheIfThenElse(TfheContract::FheIfThenElse {
                    caller,
                    control: input_handle.clone(),
                    ifTrue: left_handle.clone(),
                    ifFalse: right_handle.clone(),
            ...
            assert_eq!(
                decr_response.value.to_string(), expected_result,
                "operand output values not equal"
            );
        }
    }
