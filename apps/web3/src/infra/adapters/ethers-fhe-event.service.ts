...
export class EthersFheEventService implements FheEventService {
  #implementations = new Map<ChainId, EthersFheEventServiceImpl>()

  constructor(private readonly config: Map<ChainId, FheConfig>) {}
  fetchEvents(chainId: ChainId, fromBlock: number): Task<FheEvent[], AppError> {
    if (!this.#implementations.has(chainId)) {
      ...
      this.#implementations.set(chainId, new EthersFheEventServiceImpl(config))
    }
    return this.#implementations.get(chainId)!.fetchEvents(chainId, fromBlock)
  }
}
