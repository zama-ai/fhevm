...
import { EtherConfigFactory } from '#config/ether.config.js'
import { FheConfigFactory } from '#config/fhe.config.js'
...
@Module({
  ...
  providers: [
    ...
    SQSConsumer,
    {
      ...
      inject: [ConfigService],
      useFactory: (config: ConfigService) => {
        const map = EtherConfigFactory.getEtherConfig(config)
    ...
    },
    {
      ...
      inject: [ConfigService],
      useFactory: (config: ConfigService) => {
        const map = FheConfigFactory.getFheConfig(config)
