...
# Extract key request IDs - more precise pattern matching
...
# Update local.yaml - use temp file approach instead of sed -i
...
# Update .env.staging.coprocessor - use temp file approach instead of sed -i
