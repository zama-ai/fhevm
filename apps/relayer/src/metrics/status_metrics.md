...
| **`processing`**       | **Internally Queud before Transaction Broadcasting, and Transaction Submission**: ACL passed. request crafted and queued then, Tx sent to Mempool. Waiting for confirmation. <br> _Warning:_ If long too many transactions in the queue, gas price is too low or nonce is stuck. | Block Time (e.g., 50ms - 1s) and queue could be much more longer (see transactions metrics for more grannular details on the transaction only) |
