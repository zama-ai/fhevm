...
import type { Plan, Price } from "../../../hooks/usePlans";
import CommonTable, { CommonTableProps } from "../../common-table";
import { formatPrice, formatPeriod } from "../../../common/utils";

function formatNumberToHuman(input: "inf" | string | number) {
  // Handle "inf" case
  if (input === "inf") {
...
function TierTable(props: TierTableProps) {
  const { tiers } = props;
  ...
  let columns: CommonTableProps["columns"] = [
    {
      header: "Units",
      accessor: "up_to",
      cell: ({ index, value }) => {
