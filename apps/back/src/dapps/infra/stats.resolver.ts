...
export class StatsResolver {
  ...
  constructor(private readonly getDappRawStatsUC: uc.GetDappRawStatsUseCase) {}
  ...
  async cumulative(
    ...
    this.logger.log(`Resolving always for ${dappStat.id}`)
  ...
  async id(@Parent() dappStat: DappStatsType): Promise<string> {
    return Promise.resolve(dappStat.id + '123')
  }
}
