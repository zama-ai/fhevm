...
### 3. High — Some `module-type-marker` entries are not marker-only manifests

The package-kind definition and schema say a module-type marker contains only `type`. Some entries classified under
that kind also declare package entry-point and type metadata.

**Proposed resolution:** Add a general `nested-manifest` kind, optionally with a purpose or subtype. Reserve
`module-type-marker` for manifests that only establish module type and privacy.
