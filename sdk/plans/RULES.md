...
23. **`sdk/cleartext-config.json` is the only source for a cleartext-stack constant.** Every value the
    ...
    Every face is GENERATED by `fhevm-npm generate-cleartext-config` (a step of `make generate`):

    | Face       | Path                                                                                     |
    | ---------- | ---------------------------------------------------------------------------------------- |
    | TypeScript | `common-vendored/src/cleartext-config.ts`, synced to `<gen>/pkg/ts/` (rule 9)            |
    | Solidity   | `<gen>/create2-deploy/script/FhevmCleartextConfig.sol`, written per generation           |
    | Shell      | `<gen>/scripts/cleartext-config.sh`, written per generation and sourced by the launchers |

    The JSON sits **above** every generation, at the `sdk/` root, because the generations share it — every
    generation's copies are identical by construction. JSON rather than TypeScript because Solidity
    ...
    **A derived value records its formula, not just its result.** Three of these are keccak-derived from a
    ...
    every copy agrees with every other. So the JSON carries the `formula` alongside the value, and the
    check EVALUATES it — the preimage is already inside the expression, never recorded twice.
