...
contract CleartextArithmetic is ICleartextArithmetic, UUPSUpgradeableEmptyProxy, ACLOwnable {
    ...
    function recordNaryOp(
        ...
        uint256 needle = (op == FHEVMExecutor.Operators.fheIsIn) ? db.get(value) : 0;
    ...
    function _computeNaryOp(FHEVMExecutor.Operators op, uint256 needle, uint256[] memory values, FheType fheType)
        ...
        returns (uint256)
    {
        if (op == FHEVMExecutor.Operators.fheSum) return sum(values, FheTypeBitWidth.bitWidthForType(fheType));
