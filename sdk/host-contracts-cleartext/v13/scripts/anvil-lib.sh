...
SIG_INIT_KMS_VERIFIER="initializeFromEmptyProxy(address,uint64)"
...
SIG_INIT_PROTOCOL_CONFIG="initializeFromEmptyProxy((address,address,string,string)[],(uint256,uint256,uint256,uint256))"
...
load_generated_addresses_and_bootstrap() {
    ...
    PROTOCOL_CONFIG_ADDRESS="$(read_address PROTOCOL_CONFIG_ADDRESS)"
    KMS_GENERATION_ADDRESS="$(read_address KMS_GENERATION_ADDRESS)"
...
report_stack() {
    ...
    echo "   KMSGeneration       $KMS_GENERATION_ADDRESS"
...
assert_no_canonical_collision() {
    ...
    canonical="$ACL_ADDRESS $FHEVM_EXECUTOR_ADDRESS $KMS_VERIFIER_ADDRESS $INPUT_VERIFIER_ADDRESS
        $HCU_LIMIT_ADDRESS $PROTOCOL_CONFIG_ADDRESS $KMS_GENERATION_ADDRESS $PAUSER_SET_ADDRESS
    ...
    deployed="${EMPTY_ACL_IMPL:-} ${EMPTY_IMPL:-} ${IMPL_ACL:-} ${IMPL_EXECUTOR:-} ${IMPL_KMS_VERIFIER:-}
        ${IMPL_INPUT_VERIFIER:-} ${IMPL_HCU_LIMIT:-} ${IMPL_PROTOCOL_CONFIG:-} ${IMPL_KMS_GENERATION:-}
...
build_initializer_calldata() {
    ...
    INIT_KMS_VERIFIER="$(cast calldata "$SIG_INIT_KMS_VERIFIER" "$DECRYPTION_ADDRESS" "$GATEWAY_CHAIN_ID")"
    ...
    # KmsNode[] paired by index from the four parallel arrays, in the order LocalHostBootstrap pairs them.
    # read_bootstrap_array returns strings unquoted, so the ip and url fields are requoted per element.
    local nodes="" index tx_sender signer ip url
    for index in $(seq 1 "$KMS_NODE_COUNT"); do
        tx_sender="$(printf '%s' "$KMS_TX_SENDERS" | cut -d, -f"$index")"
        signer="$(printf '%s' "$KMS_SIGNERS" | cut -d, -f"$index")"
        ip="$(printf '%s' "$KMS_IPS" | cut -d, -f"$index")"
        url="$(printf '%s' "$KMS_URLS" | cut -d, -f"$index")"
        nodes="${nodes}${nodes:+,}($tx_sender,$signer,\"$ip\",\"$url\")"
    done
    # DEFAULT_KMS_THRESHOLDS sets every threshold to the node count.
    local n="$KMS_NODE_COUNT"
...
upgrade_stack_atomically() {
    ...
    ops="$ops,($PROTOCOL_CONFIG_ADDRESS,$IMPL_PROTOCOL_CONFIG,$INIT_PROTOCOL_CONFIG)"
    ops="$ops,($KMS_GENERATION_ADDRESS,$IMPL_KMS_GENERATION,$INIT_NOARGS)"
...
smoke_check() {
    ...
    _smoke_expect_array_len "ProtocolConfig.getKmsSigners()" "$PROTOCOL_CONFIG_ADDRESS" \
