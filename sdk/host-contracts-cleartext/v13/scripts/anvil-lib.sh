...
# The shell face of sdk/cleartext-config.json, generated by `fhevm-npm generate-cleartext-config`. It
# defines ZAMA_LOCAL_ACL / _COPROCESSOR / _KMS_VERIFIER — the three addresses ZamaConfig._getLocalConfig()
# compiles into consumer bytecode, which every launcher checks the stack actually landed on. They used to
# be literals in this file AND in anvil.sh, so the two launchers could disagree with each other as well as
# with the source of truth -- and nothing would have noticed: the TypeScript and Solidity checks cannot
# see a shell variable, and a wrong value here surfaces only as a dApp calling an address with no code.
