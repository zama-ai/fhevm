...
#
# Both defaults come from scripts/cleartext-config.sh — the generated shell face of
# sdk/cleartext-config.json — so they cannot drift from the pools the stack actually registers. Named
# rather than quoted in this help text for the same reason: a literal here would be a fourth copy.
#
# Examples:
...
source "${BASH_SOURCE[0]%/*}/cleartext-config.sh"
...
COUNT="${1:-25}"
MNEMONIC="${2:-$FHEVM_MNEMONIC}"
PATH_PREFIX="${3:-$CLEARTEXT_KMS_NODES_TX_SENDER_MNEMONIC_PATH}"
