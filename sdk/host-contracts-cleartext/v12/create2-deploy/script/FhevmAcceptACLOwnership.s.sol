 ...
 * forever (§6.1) — by the deployer before E, by the admin after. So the gates here do not protect
 ...
 * ---------------------------------------------------------------------------------------------
 *
 *   ACL.pendingOwner() == aclOwner    step B ran. Straight out of §8 — and note nothing but B can
 *                                     make it true, so this ordering never needed a gate invented
 *                                     for it.
 *   PauserSet.isPauser(aclOwner)      step A ran. The one check in this path that is NOT in §8; see
 *                                     above for what it buys. A' is deliberately not required — it
 *                                     is optional by design (§6.1), and requiring it would turn a
...
contract FhevmAcceptACLOwnership is FhevmCreate2Base {
    function run() external {
        ...
        // §11 R2, and the step where it matters most. Both of this step's ordering preconditions —
