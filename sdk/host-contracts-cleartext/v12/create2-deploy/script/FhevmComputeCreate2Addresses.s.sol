...
contract FhevmComputeCreate2Addresses is FhevmCreate2Base {
    ...
    /**
     * @dev the dependency graph: the dependency graph looks circular and is not. Only `aclAdd` feeds back, and it is
     ...
     *      ACLOwner initcode ← aclAdd. The step sequence steps B and C are structural, not ceremony.
     */
    ...
    function _pass2() private {
        ...
        // The salt scheme: the shared-impl proxies share ONE init-code hash — same implementation, same empty
    ...
    function _computeImplementations() private view returns (address[] memory impls) {
        ...
        for (uint256 i = 0; i < impls.length; i++) {
            ...
            // The initcode-size ceiling: a ~24 KB runtime plus constructor args approaches the EIP-3860 ceiling, and a
     ...
     *      code hash as observed on THIS chain, and the replay notice warning.
     */
