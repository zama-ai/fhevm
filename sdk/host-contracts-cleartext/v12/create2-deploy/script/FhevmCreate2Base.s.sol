 ...
 *
 * Implements the mechanics of the factory preflight (factory), the salt scheme (salts) and
 * The seal (seal). The scripts that inherit from it implement the three-pass pipeline, the step sequence, the terminal conditions and the precondition table.
 *
 * ---------------------------------------------------------------------------------------------
 ...
 *                           same chain (the deployment id: a redeploy always takes a fresh one).
 ...
 * that must never vary per operator or per chain, and the factory preflight's preflight pins its runtime code hash.
 */
abstract contract FhevmCreate2Base is Script {
    ...
    // ---------------------------------------------------------------------------------------
    // Roles (the salt scheme: `role` is the salt's last field, and the address name)
    // ---------------------------------------------------------------------------------------
    //
    ...
    function _implArtifact(uint256 i) internal pure returns (string memory) {
        ...
        if (i == 5) return "pkg/src/cleartext/CleartextArithmetic.sol:CleartextArithmetic";
        if (i == 6) return "pkg/src/cleartext/CleartextDB.sol:CleartextDB";
    ...
    function _loadConfig() internal {
        ...
        // The terminal conditions: E is required, not optional, and "admin == deployer" would make it a no-op that leaves
    ...
    ///      the three-pass pipeline expressible in one script run three times.
    ...
    /**
     * @dev All creates, in the order the step sequence's two hard edges require. Defined ONCE, here, because two
    ...
    function _aclProxyInitData(address deployer) internal pure returns (bytes memory) {
        // The deployer choice: the DEPLOYER, not the admin. `PauserSet.addPauser` is `onlyACLOwner`, so step A is
     ...
     *
     *      The return data is deliberately discarded — the factory preflight: "Nothing may parse the factory's return
     ...
     *      about a transaction some EARLIER step sent. reorg depth: Sepolia reorgs. A predicate evaluated
     ...
     *      as every other gate in this path: the shell is one orchestrator, the TS driver will be
