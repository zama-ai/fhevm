...
export const FHEVM_VERSION = '0.12';
 ...
 *
 * PROVENANCE, per the factory preflight's "read it off mainnet or Sepolia; do not transcribe it from memory or from a
 ...
 * which is exactly why the factory preflight says to read the value rather than recall it.
 */
export const FACTORY_CODEHASH = '0x2fa86add0aed31f33a762c9d88e807c475bd51d0f52bd0955754b2608f7e4989';

/**
 * The invariants: testnets only. This is the cleartext stack — FHE is replaced by plaintext and the KMS /
 ...
 *
 * Read the replay notice before trusting this list to do more than it does. It binds OUR tooling and nobody
 ...
 *
 * UNUSED IN THIS GENERATION. v12 is the oldest supported one, so it ships no upgrade
 * coordinator and nothing reads this. It is kept rather than deleted so `common.ts` stays byte-identical
 * to v13's apart from the two generation deltas — which is what keeps `diff -r v13 v12` short enough to
 * audit — and because when v11 lands, v12 gains an upgrade path and will need exactly this plumbing.
 *
 * The list itself is v13-shaped. Whichever generation first grows an upgrade must re-derive it for the
 * stack it upgrades FROM: a v11 stack has a different address set again.
 */
...
export function resolveOptions(flow: Flow, cli: CliArgs, cfg: ConfigFile, configPath: string | null): Options {
  ...
  // Before the value reaches `cast`, which would print it in its own error. See the key policy.
  ...
  // mnemonic that is public knowledge. Anywhere else it stays mandatory, and the key policy's keystone holds —
...
export function buildContext(flow: Flow, opt: Options): Ctx {
  ...
  // The key policy: the deployer key owns ACLOwner — root over the stack — until step F completes. Keystore
 ...
 * compute time — long before any admin key is involved, and in the multisig case the terminal conditions is written for,
...
/**
 * The factory preflight, hard gate. A different contract squatting 0x4e59… on some testnet is the one realistic way
 * The precondition table's "fatal mismatch" actually fires, and it would produce addresses nothing was compiled for.
 */
...
/**
 * The initcode-size ceiling, quantified before starting rather than discovered at send time.
 *
...
export async function broadcast(ctx: Ctx, target: string, signer?: Signer, sender?: string): Promise<void> {
  ...
  // The seal's gate. No-ops unless this is the deployment's first transaction; never reached by a dry run,
  ...
  // --slow: one transaction at a time, waiting for each receipt. The step sequence's two hard edges (impl₁ before
