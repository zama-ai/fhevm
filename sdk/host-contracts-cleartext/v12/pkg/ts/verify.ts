...
async function checkBootstrap(
  ...
  // This generation keeps the KMS signer set, the context id and a SINGLE threshold on `KMSVerifier`
  // itself. There is no `ProtocolConfig` to read, and no per-node metadata stored on chain at all — which
  // is exactly why a v12 -> v13 upgrade has to be handed the node details it cannot recover.
  if (expected.kmsSigners !== undefined) {
    const got = (await read(f.kmsVerifierAddress, kmsVerifierAbi, 'getKmsSigners')) as readonly string[];
    report.expect(
  ...
  if (expected.kmsContextId !== undefined) {
    report.expectEqual(
      ...
      await read(f.kmsVerifierAddress, kmsVerifierAbi, 'getCurrentKmsContextId'),
      expected.kmsContextId,
    );
  }
  if (expected.kmsThreshold !== undefined) {
    report.expectEqual(
      'bootstrap.kmsThreshold',
      await read(f.kmsVerifierAddress, kmsVerifierAbi, 'getThreshold'),
      expected.kmsThreshold,
    );
  }
}
