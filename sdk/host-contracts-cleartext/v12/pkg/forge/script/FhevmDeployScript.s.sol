 ...
 * `fhevm-config-0.12.0/addresses.sol` and `forge build` has been executed with
 ...
 * ---------------------------------------------------------------------------
 *
 * 1. Seven proxies, not five: the cleartext build adds CleartextArithmetic
 *    and CleartextDB.
 *
 ...
 *
 * 3. `KMSVerifier.initializeFromEmptyProxy` carries its own signer set and
 *    threshold, the same 4-argument shape as InputVerifier. (0.13 moves the KMS
 *    set out to a ProtocolConfig contract that does not exist in this
 *    generation.)
 *
...
contract FhevmDeployScript is Script {
    ...
    /// @dev This generation's KMSVerifier carries its own signer set and threshold.
