 ...
 *
 *   FOUNDRY_REMAPPINGS=fhevm-config-0.12.0/=internal/.deploy-config/ \
 ...
 * `fhevm-config-0.12.0/` addresses.sol this script was compiled against, which is what makes it a check
...
contract VerifyFhevmDeploy is Script {
    ...
    /// @dev This generation keeps the KMS signer set and threshold on `KMSVerifier` itself, so there is
    ///      no KMS *context* to inspect — no node metadata (tx-sender/ip/storage) is stored on chain at
    ///      all. 0.13 introduces ProtocolConfig and moves all of it there; that is the shape this check
    ///      has in the later generation, and the reason the v12->v13 migration has to be told the node
    ///      details rather than reading them off the running stack.
    function _checkKmsContext() private {
        console.log("[bootstrap: KMS signers]");
        _eqAddrArray(
            "KMSVerifier.getKmsSigners", KMSVerifier(kmsVerifierAdd).getKmsSigners(), LocalHostBootstrap.kmsSigners()
        );
    }

    /// @dev One threshold, not four: this generation's KMSVerifier carries a single value. 0.13 splits it
    ///      into publicDecryption/userDecryption/kmsGen/mpc on ProtocolConfig, which is why the migration
    ///      carries the one value into all four.
    function _checkThresholds() private {
        console.log("[bootstrap: thresholds]");
        _eqUint(
            "KMSVerifier.getThreshold", KMSVerifier(kmsVerifierAdd).getThreshold(), LocalHostBootstrap.KMS_NODE_COUNT
        );
    }
