 ...
 *   1. the empty proxies (nonces 0-8), each an ERC-1967 proxy over an empty UUPS implementation
...
abstract contract FhevmDeploy {
     ...
     */
    /// @dev In 0.12 the KMSVerifier carries its own signer set and threshold. 0.13 moved them to
    ///      ProtocolConfig, which is why the later generation's version of this returns only the domain.
    function _fhevmKmsVerifierConfig()
        private
        pure
...
////////////////////////////////////////////////////////////////////////////
    ...
    ////////////////////////////////////////////////////////////////////////////
    // Phase 1 — the empty proxies (nonces 0-8)
    ////////////////////////////////////////////////////////////////////////////
     ...
     *
     *      Slots 1 and 5 take the CLEARTEXT_FORGE_* blobs, which `DeployLocalStack.s.sol` deliberately
    ...
    function _deployImplementations() private returns (address[] memory implementations) {
        ...
        implementations[5] = _create(CLEARTEXT_FORGE_ARITHMETIC_CREATION_CODE, "CleartextArithmetic impl (forge)");
        implementations[6] = _create(CLEARTEXT_DB_CREATION_CODE, "CleartextDB impl");
    }
    ...
    function _materialize(address[] memory implementations) private {
        (address kmsVerifyingContract, uint64 kmsChainId, address[] memory kmsSigners, uint256 kmsThreshold) =
            _fhevmKmsVerifierConfig();
        ...
        ops[2] = ACLOwner.Op(
            ...
            abi.encodeCall(
                ...
                (kmsVerifyingContract, kmsChainId, kmsSigners, kmsThreshold)
            )
        );
        ops[3] = ACLOwner.Op(
            INPUT_VERIFIER_ADDRESS,
