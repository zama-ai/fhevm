...
Start anvil and deploy the cleartext v12 stack onto it with no Solidity
...
resolve_deployer() {
    ...
    # addresses are exactly CREATE(deployer, 0..9), so a raw deploy from the deployer at a low nonce
...
materialize_all_proxies() {
    ...
    echo "   ✅ every proxy placed at their canonical addresses (0 transactions)"
}
...
main() {
    ...
    echo ""
    echo "🚀 deploying the cleartext v12 stack (setCode + one atomic ACLOwner.upgrade, no solc)"
    deploy_empty_acl_implementation
    deploy_empty_implementation
    deploy_real_implementations
    deploy_acl_owner
    echo "   ✅ 2 empty + every real implementation + ACLOwner deployed"
    capture_proxy_runtime
