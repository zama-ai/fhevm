...
contract CleartextArithmetic is ICleartextArithmetic, UUPSUpgradeableEmptyProxy, ACLOwnable {
    ...
    function recordMulDiv(
        ...
        /// @dev `fheMulDiv` inverts the binary-op scalar convention: here `0x01` means `factor2` is
        ///      ENCRYPTED and `0x03` means it is a scalar, whereas `_computeBinaryOp` reads `0x01` as
        ///      "rhs is scalar". Do not route this through `_resolveBinaryOperands`.
        uint256 factor1Value = db.get(factor1);
        uint256 factor2Value = (scalarByte == FHE_MUL_DIV_FACTOR2_SCALAR)
            ? normalizeScalarToType(uint256(factor2), fheType)
            : db.get(factor2);
        uint256 divisorValue = normalizeScalarToType(uint256(divisor), fheType);

        db.set(result, mulDiv(factor1Value, factor2Value, divisorValue, FheTypeBitWidth.bitWidthForType(fheType)));
    }
    ...
    /// @dev `(a * b) / divisor` with intermediate widening, matching `FHE.mulDiv`. The product is
    ///      taken at full 256-bit precision before dividing, so it never truncates: `fheMulDiv` only
    ///      supports Uint8..Uint64, whose product is at most 2^128. Only the quotient is clamped.
    ///      `divisor` is non-zero — `fheMulDiv` reverts with `DivisionByZero` before we are called.
    function mulDiv(uint256 a, uint256 b, uint256 divisor, uint256 bitWidth) internal pure returns (uint256) {
        return clamp((a * b) / divisor, bitWidth);
    }
