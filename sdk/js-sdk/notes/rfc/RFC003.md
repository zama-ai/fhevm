...
- March 30, 2026: Updated naming (EncryptedValue, ClearValue, TransportKeyPair, signDecryptionPermit, etc.)
...
| `generateKeyPair()` returns raw key pair                      | `generateTransportKeyPair()` returns opaque `transportKeyPair`                                     |
| `createEIP712()` + wallet sign + `createSignedPermit()`       | `signDecryptionPermit({ signer, transportKeyPair, ... })` — all-in-one                             |
...
├── @fhevm/sdk/actions/decrypt   ← Decrypt actions (decrypt, generateTransportKeyPair)
...
createCoreFhevm() → base client (chain, runtime, trustedClient)
  ...
  │                      serializeTransportKeyPair, fetchFheEncryptionKeyBytes
...
| `generateTransportKeyPair()`               | async      | Generate a new E2E transport key pair                      |
...
Requires an `TransportKeyPair` and a signed decryption permit. The `signDecryptionPermit()` method handles EIP-712 construction and wallet signing in a single call:
...
The `transportKeyPair` is an opaque object — the TKMS private key is hidden behind `#privateFields` and never directly accessible.

| Method                                            | Description                         |
| ------------------------------------------------- | ----------------------------------- |
| `generateTransportKeyPair()`                      | Generate a fresh TKMS key pair      |
| `parseTransportKeyPair({ ... })`                  | Restore a key from serialized bytes |
| `serializeTransportKeyPair({ TransportKeyPair })` | Serialize for storage/persistence   |
| `TransportKeyPair.publicKey`                      | Get the public key hex              |
...
4. **Opaque E2E transport key pair** — `TransportKeyPair` hides the TKMS private key behind `#privateFields`. Only SDK internals can extract raw key material.
