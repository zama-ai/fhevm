...
- `TransportKeyPair` hides TKMS private key behind `#privateFields` — never exposed to application code
...
│   ├── kms/                       ← TransportKeyPair, SignedDecryptionPermit, KMS EIP-712
...
return c
  .extend(baseActions) // adds: publicDecrypt, signDecryptionPermit, parseTransportKeyPair, ...
  .extend(decryptActions) // adds: decrypt, generateTransportKeyPair, createUserDecryptEIP712, ...
...
| `TransportKeyPair` (`#privateFields`)                                 | TKMS private key never exposed as raw bytes to user code              |
...
| Key management   | `generateKeyPair()` → raw `{ publicKey, privateKey }` | `generateTransportKeyPair()` → opaque `TransportKeyPair`                            |
| Permit + signing | `createEIP712()` + wallet sign + manual bundling      | `signDecryptionPermit({ signer, TransportKeyPair, ... })` — all-in-one              |
| Decrypt          | 9 positional params                                   | Object: `decrypt({ TransportKeyPair, encryptedValues, signedPermit })`              |
