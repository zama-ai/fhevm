...
Options:
  ...
  --chain <name>        FHEVM chain (localstack_* | localstack | localcleartext | polygon_amoy).
...
# - polygon_amoy
case "$chain" in
    localcleartext|localstack|localstack_*|polygon_amoy) ;;
    *)
        echo "❌ fhetest-deploy.sh only supports local chains 'localcleartext | localstack | localstack_* | polygon_amoy'; got '$chain'" >&2
        exit 1
        ;;
esac
