...
#   4. Run only test/fheTest/cleartext-ethers.
...
TEST_TARGET="test/fheTest/cleartext-ethers"
...
################################################################################
#
# Run cleartext ethers tests
#
################################################################################

echo "🧪 Running cleartext ethers tests..."
(
    cd "$JS_SDK_DIR"
    CHAIN=localhost npx vitest run --config test/fheTest/vitest.config.ts "$TEST_TARGET"
)

echo "✅ cleartext-ethers tests passed."
