...
function resolveChainName(): FheTestChainName {
  ...
  if (
    ...
    throw new Error(
      `Invalid CHAIN env var: "${chain}". Expected "sepolia", "mainnet", "devnet" or "localhostFhevm".`,
    );
  }
  return chain;
}
...
const FHEVM_MNEMONIC =
  'test test test test test test test future home engine virtual motion';
...
export function getBaseEnv(): FheTestBaseEnv {
  ...
  if (!rpcUrl) {
    throw new Error(
      `RPC_URL is missing. Set it in test/.env.${chainName} or as an environment variable.`,
    );
  }
  ...
  if (chainName === 'localhostFhevm') {
    ...
    throw new Error(
      `Unsupported chain: "${chainName}". Expected "sepolia", "mainnet" or "localhostFhevm".`,
    );
  }
