 ...
 * @returns {TypedPlaintext[]}
 */
export function process_user_decryption_resp_solana_from_js(request, solana_user_pubkey, host_chain_id, agg_resp, enc_pk, enc_sk) {
    const ptr0 = passArray8ToWasm0(solana_user_pubkey, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    _assertClass(enc_pk, PublicEncKeyMlKem512);
    _assertClass(enc_sk, PrivateEncKeyMlKem512);
    const ret = wasm.process_user_decryption_resp_solana_from_js(request, ptr0, len0, host_chain_id, agg_resp, enc_pk.__wbg_ptr, enc_sk.__wbg_ptr);
    if (ret[3]) {
        throw takeFromExternrefTable0(ret[2]);
    }
