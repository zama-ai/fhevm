...
SDK term: `signLegacyDecryptionPermit()` (V1), `signUnifiedDecryptionPermit()` (V2), `canUseUnifiedDecryptionPermit()`, `serializeSignedDecryptionPermit()`, `parseSignedDecryptionPermit()`; type `SignedDecryptionPermit`
An EIP-712 permit signed by the user that authorizes a decryption request. `signLegacyDecryptionPermit()` (protocol v13 and below) and `signUnifiedDecryptionPermit()` (protocol v14 and above, gated by `canUseUnifiedDecryptionPermit()`) each construct and sign the permit in one step; both return the same `SignedDecryptionPermit` shape, which exposes `isDelegated: boolean` and `version: 1 | 2` directly rather than being a union of separate self/delegated types. Delegation is a `delegatorAddress` parameter, not a separate call. The signer may be a smart-contract wallet — the SDK verifies such a signature via ERC-1271 instead of `ecrecover`. The lower-level EIP-712 builders (`createUnsignedLegacyDecryptionPermitEip712()` / `createUnsignedUnifiedDecryptionPermitEip712()`) live in §3b.
*Source: SDKDeprecated terms (do not use): SignedPermit, signDecryptionPermit (still present but `@deprecated`, alias for `signLegacyDecryptionPermit`), SignedSelfDecryptionPermit, SignedDelegatedDecryptionPermit, SignSelfDecryptionPermitParameters, SignDelegatedDecryptionPermitParameters, onBehalfOf (use `delegatorAddress`)*
...
**KMS share flow** `[low]`
SDK term: `KmsSigncryptedShares` (internal only as of the RFC-016 unified permit rework; not exported from the public API)
In the private decryption flow, the signed-and-encrypted shares each KMS node returns: each is encrypted under the user's transport public key (only the user can read it) and signed by the node (the user can verify authenticity). The fetch/reconstruct steps are now internal, version-split (V1 in `SignedDecryptionPermitV1-p.ts`, V2 in `fetchKmsSigncryptedSharesV2-p.ts`), and wrapped by the high-level `decryptValue()`/`decryptValues()`/`decryptValuesFromPairs()`.
*Deprecated terms (do not use): fetchKmsSigncryptedShares(), decryptKmsSigncryptedShares() — removed from the public API in v1.1.0-alpha.7*

**EIP712** `[low]`
SDK term: `createUnsignedLegacyDecryptionPermitEip712()` (V1), `createUnsignedUnifiedDecryptionPermitEip712()` (V2), `verifyErc1271UserDecrypt()` (internal)
Build the decrypt-permit typed data without signing — for handing off to an external signing flow — via the standalone actions from `@fhevm/sdk/actions/base`. Signature verification is internal: `verifyErc1271UserDecrypt()` checks a permit's signature (EOA via `ecrecover`, smart-contract wallet via ERC-1271) before the SDK sends it to the KMS.
*Deprecated terms (do not use): createKmsUserDecryptEip712(), createKmsDelegatedUserDecryptEip712(), createKmsEip712Domain(), createCoprocessorEip712Domain(), verifyKmsUserDecryptEip712() — removed from the public API in v1.1.0-alpha.7*
