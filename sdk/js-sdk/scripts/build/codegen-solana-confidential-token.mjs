...
function renderZamaHostErrors(anchorIdl) {
  const errors = (anchorIdl.errors ?? []).map((error) => ({
    code: error.code,
    name: error.name,
    message: error.msg,
  }));
  return (
    ...
    `};\n\n` +
    `export const ZAMA_HOST_IDL_ERRORS: readonly ZamaHostIdlError[] = ${JSON.stringify(errors)};\n`
  );
}
...
for (const target of targets) {
  ...
  if (target.generatedPath.endsWith('src/solana/internal/generated/zamaHost')) {
    writeFileSync(
      `${temporaryGeneratedPath}/errors.ts`,
      await format(renderZamaHostErrors(anchorIdl), { ...prettierOptions, parser: 'typescript' }),
    );
  }
