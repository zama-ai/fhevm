...
const targets = [
  {
    ...
    keep: {
      // The two app-facing instructions plus the three the confidential-vault demo builds directly:
      // confidentialTransferFromValue and wrapUsdc (the on-chain from-value/wrap surface, deferred
      // in #1680 and now absorbed), and initializeTokenAccount (the vault `claim` flow needs the
      // user payout token account to pre-exist). redeemBurnedAmount stays pruned — it is a
      // batcher-internal CPI inside settle, never built by the SDK.
  ...
  },
  {
    ...
    // never leaks into the SDK's normal Solana paths. Keeps every batch-lifecycle instruction and
    // the three defined types their args reference; every account is a required input (all batcher
    // PDAs are pruned — the `batch` PDA seeds on `batcher.next_batch_index`, so a generated resolver
    // would have to fetch the batcher account; the vault module derives the PDAs explicitly instead).
    ...
    keep: {
      ...
      // directly into the instruction data, so the only referenced defined type is the
      // initializeBatcher direction enum; batchStatus/confidentialMint/vault are account-only.
      definedTypes: new Set(['batchDirection']),
