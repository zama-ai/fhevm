...
describe('User decryption', function () {
  ...
  it('test user decrypt ebool', async function () {
    ...
    const { publicKey, privateKey } = await this.instances.alice.generateKeypairAsync();
  ...
  describe('negative-acl', function () {
    it('should reject when user is not allowed for handle', async function () {
      const handle = await this.contract.xBool();
      const { publicKey: publicKeyBob, privateKey: privateKeyBob } = await this.instances.bob.generateKeypairAsync();
      ...
      try {
        ...
        expect((error as { message: string }).message).to.contains(
          `User ${this.signers.bob.address} is not authorized to decrypt handle ${handle}!`
        );
      }
    });
    ...
    it('should reject when request has expired', async function () {
      ...
      try {
        ...
        expect((error as { message: string }).message).to.equal('request has expired');
      }
    });
  });
});
