...
import { ADDRESSES } from "./engine/addresses";
import { getCleartextArtifact } from "./engine/artifacts";
import { type DeployedEngine, assertEngineReady, deployCleartextEngine } from "./engine/deploy";
import { formatDecrypted, generateKeypair, publicDecrypt, userDecrypt } from "./engine/decrypt";
import { type EncryptValue, encryptInput } from "./engine/encrypt";
import { FheType } from "./engine/fhetype";
import { type Eip1193Provider, FhevmNode } from "./engine/node";
import { defaultSigners } from "./engine/addresses";
import type {
...
export class FhevmApi implements HardhatFhevmRuntimeEnvironment {
  ...
  private async deployOrAdopt(): Promise<DeployedEngine> {
    ...
    const existing = await node.getCode(ADDRESSES.FHEVMExecutor);
    if (existing === getCleartextArtifact("FHEVMExecutor").deployedBytecode) {
