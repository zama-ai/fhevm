 ...
 * node's EIP-1193 `request`. Swapping this codec (e.g. for viem's pure encoders) would not touch transport.
 */
...
/** `eth_call` a view function on a cleartext contract and return the decoded result. */
...
/** Send a state-changing call from `from` (must be impersonated) to a cleartext contract. */
