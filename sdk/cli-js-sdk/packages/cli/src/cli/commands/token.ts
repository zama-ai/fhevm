...
export const registerTokenCommands = (program: Command): void => {
  ...
  tokenCommand
    ...
    .action(async (options, command) => {
      const { transferToken } = await import("@cli-fhevm-sdk/toolkit/flows/token/transfer");
  ...
  tokenCommand
    ...
    .action(async (options, command) => {
      const { balanceOfToken } = await import("@cli-fhevm-sdk/toolkit/flows/token/balance");
      const globals = getGlobalOptions(command);
