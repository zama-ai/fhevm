...
import { createFhevmClient, hasFhevmRuntimeConfig, setFhevmRuntimeConfig } from "@fhevm/sdk/viem";
...
export const resolveChain = (options: ClientOptions): FhevmChain => {
  if (options.network !== "testnet") {
    throw new Error(`Unsupported network "${options.network}". Only "testnet" is supported for now.`);
  }
  return withRelayerUrl(sepolia, options.relayerUrl);
};

export const resolveRpcUrl = (rpcUrl?: string): string => rpcUrl ?? Bun.env.SEPOLIA_RPC_URL ?? DEFAULT_TESTNET_RPC_URL;
...
export const loadAccount = (privateKey?: Hex, mnemonic?: string): Account => {
  ...
  const resolvedPrivateKey = privateKey ?? (Bun.env.PRIVATE_KEY as Hex | undefined);
  ...
  throw new Error("Provide --private-key, --mnemonic, PRIVATE_KEY, or MNEMONIC.");
};

export const createWallet = (options: ClientOptions & { privateKey?: Hex; mnemonic?: string }) => {
  const account = loadAccount(options.privateKey, options.mnemonic);
  const { transport, publicClient, fhevm, chain, rpcUrl } = createClients(options);
