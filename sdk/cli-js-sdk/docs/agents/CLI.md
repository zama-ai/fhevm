...
- Keep decrypt-family naming consistent: the `direct` subcommand does direct handle decryption, while `fresh` and `stored` are FHETest demo subcommands.
- The decrypt family roots (`user-decrypt`, `public-decrypt`, `delegated-user-decrypt`) are pure dispatchers with no options of their own; their `direct` subcommands decrypt existing `--handle` values from any contract, where `--contract` is the ACL pairing contract and defaults to FHETest. A bare root command with no subcommand prints help.
...
- `token transfer` returns `transferredHandle` because ERC-7984 does not revert on insufficient balance; the token ACL lets the recipient (not the sender) decrypt it with `user-decrypt direct --handle <transferredHandle> --contract <token address>`.
