...
import { deployConfidentialVestingWalletFixture, userDecryptReleased } from './ConfidentialVestingWallet.fixture.ts';
...
describe('ConfidentialVestingWallet', function () {
  ...
  let duration: bigint;
  let confidentialVestingWallet: TestConfidentialVestingWallet;
  ...
  it('can release', async function () {
    ...
    await expect(tx).to.emit(confidentialVestingWallet, 'ConfidentialERC20Released').withArgs(confidentialERC20Address);
    ...
    nextTimestamp = startTimestamp + duration / BigInt(4);
    await ethers.provider.send('evm_setNextBlockTimestamp', [nextTimestamp.toString()]);
