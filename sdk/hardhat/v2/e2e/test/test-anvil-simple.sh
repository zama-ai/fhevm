...
# Note: this script used to start a Hardhat Node, wait for it, and immediately kill it again before
# reaching Anvil — a copy-paste leftover from test-hardhat-node.sh that cost ~15s and did nothing.
# `hardhat test` compiles on its own, so nothing depended on it.
...
# The script name is `test:anvil:simple`. It used to be spelled `test:anvilsimple`, which does not
# exist in packages/hardhat-plugin/package.json — npm failed with "Missing script", `|| true`
# swallowed it, and the script exited 0 having run no tests at all.
#
...
# pattern goes stale (as it had: it pointed at TestAsyncDecrypt, which is entirely commented out),
# and mocha still exits 0 — so check explicitly.
