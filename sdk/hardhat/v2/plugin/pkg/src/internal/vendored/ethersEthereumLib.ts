...
export function createEthersEthereumHistory(rpcUrl: string): AbstractEthereumHistory {
  const provider = new EthersT.JsonRpcProvider(rpcUrl, undefined, { staticNetwork: true });
  return {
    ...
    getLogs: async ({ address, abi, eventNames, fromBlock, toBlock }) => {
      const itf = new EthersT.Interface(abi as EthersT.InterfaceAbi);
      const wanted = new Map(
        eventNames
          .map((name) => itf.getEvent(name))
          .filter((event): event is NonNullable<typeof event> => event !== null)
          .map((event) => [event.topicHash, event.name]),
      );
