...
export class FhevmProviderExtender extends ProviderWrapper {
  ...
  private async _handleEthEstimateGas(args: RequestArguments): Promise<unknown> {
    try {
      ...
      //
      // EIP-1474 says `eth_estimateGas` answers with a hex QUANTITY string, and that is what the wrapped
      // provider hands back — casting it to bigint was a lie the multiplication below then tripped over
      // with "Cannot mix BigInt and other types". `EthersT.toBigInt` accepts the string, a bigint, or a
      // number, so it is correct whatever a provider in the chain decided to return.
      const estimatedGasLimit = EthersT.toBigInt((await this._wrappedProvider.request(args)) as EthersT.BigNumberish);
