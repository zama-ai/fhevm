...
//!
//! A consensus consumer needs three things from `GatewayConfig`: the set of authorized
//! attestation signer addresses, the per-Coprocessor S3 bucket URLs (to fan attestation HEAD
//! requests at), and the majority threshold. Querying these on every request would trigger N+1
//! RPC calls, so the [`CoprocessorRegistry`] holds a whole snapshot behind a short TTL and
//! tolerates registration changes within one refresh window.
...
async fn get_copro_bucket<P: Provider<N>, N: Network>(
    contract: &GatewayConfigInstance<P, N>,
...
mod tests {
    ...
    fn mock_registry_load(asserter: &Asserter, tx_senders: &[Address], buckets: &[&str]) {
        asserter.push_success(&vec![addr(0x01)].abi_encode());
    ...
    async fn load_rejects_zero_threshold_as_critical() {
        let asserter = Asserter::new();
        asserter.push_success(&vec![addr(0x05)].abi_encode());
