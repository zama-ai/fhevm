...
//! back. They also check which error it is, since `Starved` is retried and `Split` is not, and
//! which buckets are named when the check does succeed.
//!
...
#[tokio::test]
async fn under_threshold_is_starved() {
    ...
    // `valid` is deliberately not pinned: the round is abandoned as soon as it becomes
    // unwinnable, which happens once both misses are in — whether or not the one good
    // attestation has arrived by then. What must hold is that it is retriable, not a split.
    assert!(
        matches!(err, ConsensusCheckError::Starved { required: 2, .. }),
        "expected Starved with required 2, got {err:?}"
    );
}
...
async fn cross_bucket_disagreement_is_no_consensus() {
    ...
    assert!(
        matches!(
            err,
            ConsensusCheckError::Split {
                valid: 3,
                largest: 1
            }
        ),
        "expected Split {{ valid: 3, largest: 1 }}, got {err:?}"
    );
}
...
#[tokio::test]
async fn unregistered_signers_are_starved() {
    ...
    assert!(
        matches!(
            err,
            ConsensusCheckError::Starved {
                valid: 0,
                required: 2
            }
        ),
        "expected Starved {{ valid: 0, required: 2 }}, got {err:?}"
    );
}

/// Buckets that answer too late count as not having answered.
#[tokio::test]
async fn head_timeout_is_starved() {
    let buckets = start_buckets(3).await;
    for bucket in &buckets {
...
#[tokio::test]
async fn cross_served_attestation_is_starved() {
    let buckets = start_buckets(2).await;
    buckets[1]
        .serve(&attestation(&buckets[0], HANDLE, CT_DIGEST).await)
        .await;
