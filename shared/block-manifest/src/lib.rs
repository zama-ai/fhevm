...
/// Maximum JSON wire size accepted for one signed manifest.
...
impl ManifestPayload {
    ...
    pub fn canonical_bytes(&self) -> Result<Vec<u8>, ManifestError> {
        ...
        push_u256(&mut out, U256::from(self.generation));
        ...
        for block in &self.detailed_range.blocks {
            push_u256(&mut out, U256::from(block.generation));
        ...
        for range in &self.historical_ranges {
            ...
            push_u256(&mut out, U256::from(range.end_block_generation));
...
fn validate_detailed_range(payload: &ManifestPayload) -> Result<(), ManifestError> {
    ...
    for (index, block) in blocks.iter().enumerate() {
        if block.generation != payload.generation {
            return Err(invalid(
                "detailed range block does not belong to the manifest generation",
            ));
        }
...
fn validate_history(payload: &ManifestPayload) -> Result<(), ManifestError> {
    ...
    for (index, range) in history.iter().enumerate() {
        ...
        if range.end_block_generation != payload.generation {
            return Err(invalid(
                "historical range does not belong to the manifest generation",
            ));
        }
...
mod tests {
    ...
    fn v1_generation_is_signed_but_not_ciphertext_consensus_material() {
        ...
        first.generation = 7;
        first.detailed_range.blocks[0].generation = 7;
        let content = block_content_digest(
            ManifestVersion::V1,
        ...
        second.generation = 8;
        second.detailed_range.blocks[0].generation = 8;
    ...
    #[test]
    fn v1_accepts_only_an_oldest_generation_truncated_history_range() {
        let mut payload = payload(Address::ZERO);
        payload.version = ManifestVersion::V1;
        payload.generation = 1;
        payload.detailed_range.blocks[0].generation = 1;
        let content = block_content_digest(
            ManifestVersion::V1,
            payload.coprocessor_context_id,
            payload.host_chain_id,
        ...
        .unwrap();
        payload.detailed_range.blocks[0].block_content_digest = content;
        ...
        payload.historical_ranges = vec![HistoricalRange {
            end_block_generation: 1,
            start_block_number: U256::from(41),
            end_block_number: U256::from(41),
            scale: 1,
            end_block_hash: payload.publication_parent_block_hash,
            digest: B256::repeat_byte(1),
        }];

        payload.validate().unwrap();

        payload.historical_ranges = vec![
            HistoricalRange {
                end_block_generation: 1,
                start_block_number: U256::from(41),
                end_block_number: U256::from(41),
            ...
            HistoricalRange {
                ...
                end_block_number: U256::from(40),
                scale: 2,
                end_block_hash: B256::repeat_byte(0xA8),
    ...
    fn manifest_digest_is_pinned() {
        ...
        assert_eq!(
            ...
            b256!("43a2dd8b7366a4a9e380e71bf1c0f0a8ec73551ebe6ab405753f578d7c80c3a5")
        );
    }
}
