//! Encrypted store resolution: presence, program ownership, type, address binding, and
//! the authority and application the account names.
//!
//! A handle entry names the object that authorizes it — the encrypted store, by address —
...
/// An encrypted store that passed presence, ownership, type and address binding.
///
/// No public constructor: [`resolve_encrypted_store`] is the only way in, which is what
...
impl ResolvedEncryptedStore {
    /// The encrypted store's address, which every leaf commitment binds.
    pub fn account_key(&self) -> SolanaPubkeyBytes {
        self.account_key
    }

    /// The authority this encrypted store belongs to, which the delegation rule is read
...
/// The address an encrypted store with these fields must live at: the PDA of its own
...
/// Resolves one entry's encrypted store against the snapshot.
///
/// In order: the account exists in the snapshot; it is owned by the deployment's program; its
/// data carries the encrypted store discriminator and borsh-decodes; its own fields
...
pub fn resolve_encrypted_store(
    ...
    // h3: an encrypted store and not another account type of the same program. Decoding
    ...
    let encrypted_store = decode_encrypted_store(&account.data).map_err(|error| match error {
        ...
        AclError::BadAccountData | AclError::MmrInconsistent => {
            EncryptedStoreFailure::Malformed { account_key }
        }
        ...
        | AclError::PublicDecryptProofInvalid => EncryptedStoreFailure::Malformed { account_key },
    })?;
...
/// Why an encrypted store could not be resolved.
...
pub enum EncryptedStoreFailure {
    ...
    #[error("encrypted store {account_key:?} is owned by {owner:?}, expected {expected:?}")]
    ForeignOwner {
        /// The address that was read.
        account_key: SolanaPubkeyBytes,
        /// Who owns it.
    ...
    /// The account is host-owned but is not an encrypted store — a different account type
    /// of the same program, caught by the discriminator.
    #[error("account {account_key:?} does not carry the encrypted store discriminator")]
    WrongAccountType {
        /// The address that was read.
        account_key: SolanaPubkeyBytes,
    },
    /// The discriminator matched but the body did not decode.
    #[error("encrypted store {account_key:?} body does not decode")]
    Malformed {
        /// The address that was read.
        account_key: SolanaPubkeyBytes,
    },
