...
//! per handle: the pair of *every* entry's encrypted store must be in the set. Checking
...
//! are unconditional.
//!
//! The pair being tested comes from the validated encrypted store. A request has no field
...
pub enum ScopeFailure {
    /// The encrypted store's `(program, scope)` is not in the signed set.
    #[error("application ({program:?}, {scope:?}) is outside the signed scope")]
    ScopeNotAllowed {
        /// The application program the encrypted store belongs to.
