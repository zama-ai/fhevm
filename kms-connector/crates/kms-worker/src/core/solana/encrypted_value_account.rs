...
//! its `valueKey` — and the chain of checks here is what turns that unsigned claim into a validated
//! account. Program ownership is the sole trust anchor: no party other than the host program can
//! produce data in an account the host program owns. Everything the later rules read (the app
//! account, the ACL domain, the current handle, the subject set, the MMR commitments) comes from
//! this validated account and from nowhere else.
//!
...
/// makes "read the app account from something that was never validated" unwritable.
...
impl ResolvedEncryptedValueAccount {
    ...
    /// The app account this encrypted value account belongs to — the app context of every later
    /// rule, read from the account and never from a request field.
...
/// The canonical encrypted value account address for a value key under this deployment.
...
/// encrypted value account a rejection rather than a redirection: an attacker naming a victim's
/// `valueKey` while supplying a different account fails it.
...
pub enum EncryptedValueAccountFailure {
    ...
    #[error("encrypted value account {account_key:?} fields derive a different value key")]
