...
pub struct DecryptionProcessor<GP: Provider, HP: Provider, C> {
    ...
    /// The host chains RPC clients used to check the decryption ACL and signature.
    ...
    C: ContextManager,
{
    ...
    pub async fn check_ciphertexts_allowed_for_public_decryption(
        ...
        for ct in sns_ciphertexts {
            ...
            if !host_client.is_allowed_for_decryption(ct.ctHandle).await? {
...
mod tests {
    ...
    async fn acl_errors_carry_host_chain_context() {
        ...
        let err = decryption_processor
            .check_ciphertexts_allowed_for_public_decryption(&[sns_ct])
            .await
            .map_err(RequestCheckError::record)
            .unwrap_err();
