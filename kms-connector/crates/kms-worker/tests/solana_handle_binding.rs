//! Handle binding: which handle of an encrypted value account a subject may decrypt, and on what evidence.
//!
...
//!
//! The file is dominated by substitutions of the leaf commitment: another encrypted value account, another
...
fn a_live_handle_authorizes_a_current_member() {
    ...
    let encrypted_value_account = EncryptedValueAccountFixture::new(live, &[subject]);
...
fn a_handle_that_is_no_longer_current_is_not_swapped_for_the_live_one() {
    ...
    encrypted_value_account.update(live);
...
fn a_non_member_is_rejected_in_current_mode() {
    ...
    let encrypted_value_account = EncryptedValueAccountFixture::new(live, &[member]);
...
fn a_sealed_leaf_authorizes_its_subject_after_an_update() {
    ...
    let mut encrypted_value_account = EncryptedValueAccountFixture::new(replaced, &[subject]);
    encrypted_value_account.update(handle(0x21, FHE_TYPE_UINT64));

    check_handle_binding(
        &resolved(&encrypted_value_account),
        replaced,
        subject,
        &historical(encrypted_value_account.proof(0)),
    )
...
fn historical_access_survives_a_membership_rotation() {
    ...
    let mut encrypted_value_account = EncryptedValueAccountFixture::new(replaced, &[subject]);
    encrypted_value_account.update(handle(0x23, FHE_TYPE_UINT64));
    encrypted_value_account.rotate_subjects(&[stranger]);
    let resolved = resolved(&encrypted_value_account);
...
/// An append merges only some peaks. A proof built before an append that left its peak alone still
/// verifies against the live peaks, and verifying is the whole test — age is not a failure.
/// Rejecting on age would fail valid proofs after every write to the encrypted value account.
#[test]
fn a_proof_whose_peak_no_later_append_merged_still_verifies() {
    let subject = Wallet::new(1).pubkey();
    let first = handle(0x24, FHE_TYPE_UINT64);
    let mut encrypted_value_account = EncryptedValueAccountFixture::new(first, &[subject]);
    encrypted_value_account.update(handle(0x25, FHE_TYPE_UINT64));
    encrypted_value_account.update(handle(0x26, FHE_TYPE_UINT64));
    assert_eq!(encrypted_value_account.encrypted_value.leaf_count, 2);
    let proof = encrypted_value_account.proof(0);
    encrypted_value_account.update(handle(0x27, FHE_TYPE_UINT64));
    assert_eq!(encrypted_value_account.encrypted_value.leaf_count, 3);
...
fn a_proof_invalidated_by_a_merging_append_no_longer_verifies() {
    ...
    let mut encrypted_value_account =
        EncryptedValueAccountFixture::new(handle(0x2e, FHE_TYPE_UINT64), &[subject]);
    encrypted_value_account.update(handle(0x2f, FHE_TYPE_UINT64));
    encrypted_value_account.update(third);
    encrypted_value_account.update(handle(0x31, FHE_TYPE_UINT64));
    assert_eq!(encrypted_value_account.encrypted_value.leaf_count, 3);
    let proof = encrypted_value_account.proof(2);
    encrypted_value_account.update(handle(0x32, FHE_TYPE_UINT64));
    assert_eq!(encrypted_value_account.encrypted_value.leaf_count, 4);
...
/// A leaf position the encrypted value account does not have is refused before any hashing: there
/// is nothing for the proof to be a proof of.
#[test]
fn a_leaf_index_at_or_above_the_observed_count_is_rejected() {
    let subject = Wallet::new(1).pubkey();
    let replaced = handle(0x33, FHE_TYPE_UINT64);
    let mut encrypted_value_account = EncryptedValueAccountFixture::new(replaced, &[subject]);
    encrypted_value_account.update(handle(0x34, FHE_TYPE_UINT64));
    let beyond = MmrProof {
        leaf_index: encrypted_value_account.encrypted_value.leaf_count,
        siblings: Vec::new(),
    };
    ...
    .expect_err("a position the encrypted value account does not have proves nothing");
...
fn a_proof_with_tampered_siblings_is_rejected() {
    ...
    let mut encrypted_value_account =
        EncryptedValueAccountFixture::new(replaced, &[subject, Wallet::new(2).pubkey()]);
    encrypted_value_account.update(handle(0x36, FHE_TYPE_UINT64));
    let mut proof = encrypted_value_account.proof(0);
    assert!(
...
fn a_leaf_commitment_for_another_encrypted_value_account_does_not_authorize() {
    ...
    let mut encrypted_value_account = EncryptedValueAccountFixture::new(target, &[subject]);
    let other_account: SolanaPubkeyBytes = [0x99; 32];
    // A leaf that would be valid if this encrypted value account lived at another address.
    encrypted_value_account.append(historical_access_leaf_commitment(
        other_account,
        0,
        target,
        subject,
    ...
    .expect_err("a leaf sealed for another encrypted value account authorizes nothing here");
...
fn a_leaf_commitment_for_another_subject_does_not_authorize() {
    ...
    let mut encrypted_value_account = EncryptedValueAccountFixture::new(target, &[subject]);
    let account = encrypted_value_account.account_key;
    encrypted_value_account.append(historical_access_leaf_commitment(
        account,
        0,
        target,
...
fn a_public_decrypt_leaf_does_not_authorize_historical_access() {
    ...
    encrypted_value_account.update(handle(0x46, FHE_TYPE_UINT64));
...
async fn a_verifying_proof_is_accepted_whatever_count_the_request_claims() {
    ...
    let mut encrypted_value_account =
        EncryptedValueAccountFixture::new(replaced, &[wallet.pubkey()]);
    encrypted_value_account.update(handle(0x51, FHE_TYPE_UINT64));
    let proof = encrypted_value_account.proof(0);
...
async fn a_failed_inclusion_is_classified_by_the_claimed_count() {
    ...
    let mut encrypted_value_account =
        EncryptedValueAccountFixture::new(handle(0x5e, FHE_TYPE_UINT64), &[wallet.pubkey()]);
    encrypted_value_account.update(handle(0x5f, FHE_TYPE_UINT64));
    encrypted_value_account.update(third);
    encrypted_value_account.update(handle(0x61, FHE_TYPE_UINT64));
    let stale_proof = encrypted_value_account.proof(2);
    let claimed_count = encrypted_value_account.encrypted_value.leaf_count;
    encrypted_value_account.update(handle(0x62, FHE_TYPE_UINT64));
...
fn an_access_proof_with_trailing_bytes_is_rejected() {
    ...
    let mut encrypted_value_account =
        EncryptedValueAccountFixture::new(replaced, &[wallet.pubkey()]);
    encrypted_value_account.update(handle(0x71, FHE_TYPE_UINT64));
    let mut bytes = borsh::to_vec(&encrypted_value_account.proof(0)).expect("a proof serializes");
...
fn an_access_proof_with_more_siblings_than_the_tree_can_produce_is_rejected() {
    ...
    let encrypted_value_account = EncryptedValueAccountFixture::new(target, &[wallet.pubkey()]);
