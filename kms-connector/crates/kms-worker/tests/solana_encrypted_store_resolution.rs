...
//! exist under this deployment's program, carry the encrypted store type, and live at
//! the address its own fields derive. Only then are its fields — the encrypted store
...
//! encrypted store whose MMR once held more peaks than it holds now has a tail.
...
#[test]
fn an_encrypted_store_named_by_its_address_resolves() {
    let encrypted_store =
...
/// An absent encrypted store is a rejection that may resolve itself: the account may simply
...
#[test]
fn an_encrypted_store_absent_at_the_observation_is_transient() {
    let encrypted_store =
...
#[test]
fn an_encrypted_store_owned_by_another_program_is_terminal() {
    let encrypted_store =
...
/// its bytes happen to mean when read as an encrypted store. Here the substitute is a real
/// delegation record — the account type most likely to be confused with an encrypted store,
...
#[test]
fn an_encrypted_store_whose_fields_derive_another_address_is_rejected() {
    let owner = Wallet::new(1).pubkey();
    let claimed = EncryptedStoreFixture::allowing(handle(0x14, FHE_TYPE_UINT64), owner);
...
#[test]
fn an_encrypted_store_with_an_altered_bump_is_rejected() {
    let owner = Wallet::new(1).pubkey();
    let mut altered = EncryptedStoreFixture::allowing(handle(0x15, FHE_TYPE_UINT64), owner);
    ...
    assert!(matches!(
        failure,
        EncryptedStoreFailure::AddressMismatch { account_key: key, derived }
            if key == account_key && derived != Some(account_key)
    ));
}

/// Trailing bytes are legal. An encrypted store is realloc-grown to its high-water mark and
/// never shrunk, so the tail is the normal state of any account whose MMR has held more peaks
/// than it holds now.
#[test]
fn trailing_bytes_after_the_encrypted_store_body_are_accepted() {
    let encrypted_store =
        EncryptedStoreFixture::allowing(handle(0x16, FHE_TYPE_UINT64), Wallet::new(1).pubkey());
    let mut grown = encrypted_store.account();
...
/// encrypted store that cannot be read, the second is an encrypted store with room
/// to spare.
#[test]
fn an_encrypted_store_with_a_truncated_body_is_rejected() {
    let encrypted_store =
        EncryptedStoreFixture::allowing(handle(0x17, FHE_TYPE_UINT64), Wallet::new(1).pubkey());
    let full = encrypted_store.account();
...
/// encrypted store in it. The type check and the decode are two checks because an account
/// can pass the first and fail the second.
#[test]
fn an_encrypted_store_holding_only_its_discriminator_is_rejected() {
    let encrypted_store =
        EncryptedStoreFixture::allowing(handle(0x18, FHE_TYPE_UINT64), Wallet::new(1).pubkey());
    let empty = SnapshotAccount {
        owner: PROGRAM_ID,
        data: encrypted_store_discriminator().to_vec(),
    };
...
#[test]
fn a_scoped_permit_admits_an_encrypted_store_of_a_signed_application() {
    let encrypted_store =
...
async fn a_permissive_permit_does_not_widen_the_allow_leaf() {
    ...
    let encrypted_store = EncryptedStoreFixture::allowing(live, stranger.pubkey());
    let request = RequestBuilder::new(&wallet)
