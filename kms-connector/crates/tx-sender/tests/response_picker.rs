...
use connector_utils::{tests::setup::TestInstanceBuilder, types::KmsResponseKind};
use rstest::rstest;
...
async fn test_pick_public_decryption() -> anyhow::Result<()> {
    ...
    assert_eq!(
        responses[0].kind,
        KmsResponseKind::PublicDecryption(inserted_response)
    );
    info!("Data OK!");
    Ok(())
}
...
async fn test_pick_user_decryption() -> anyhow::Result<()> {
    ...
    assert_eq!(
        responses[0].kind,
        KmsResponseKind::UserDecryption(inserted_response)
    );
    info!("Data OK!");
    Ok(())
}
...
async fn test_pick_prep_keygen() -> anyhow::Result<()> {
    ...
    assert_eq!(
        responses[0].kind,
        KmsResponseKind::PrepKeygen(inserted_response)
    );
    info!("Data OK!");
    Ok(())
}
...
async fn test_pick_keygen() -> anyhow::Result<()> {
    ...
    assert_eq!(
        responses[0].kind,
        KmsResponseKind::Keygen(inserted_response)
    );
    info!("Data OK!");
    Ok(())
}
...
async fn test_pick_crsgen() -> anyhow::Result<()> {
    ...
    assert_eq!(
        responses[0].kind,
        KmsResponseKind::Crsgen(inserted_response)
    );
    info!("Data OK!");
    Ok(())
}
