...
// Some cross-module Gateway binding payloads do not derive Debug or PartialEq.
#[derive(Clone)]
pub enum ProtocolEventKind {
    ...
    SolanaUserDecryptionV1(SolanaUserDecryptionRequestV1),
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::UserDecryptionV2(e) => f
                ...
                .finish(),
            Self::SolanaUserDecryptionV1(e) => {
                f.debug_tuple("SolanaUserDecryptionV1").field(e).finish()
            }
...
impl PartialEq for ProtocolEventKind {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (Self::SolanaUserDecryptionV1(a), Self::SolanaUserDecryptionV1(b)) => a == b,
...
pub fn from_user_decryption_row(row: &PgRow) -> anyhow::Result<ProtocolEvent> {
    ...
    let kind = match attestation_type.as_str() {
        ...
        other => anyhow::bail!("unsupported user-decrypt attestation type: {other}"),
    };
...
impl Display for ProtocolEventKind {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ProtocolEventKind::SolanaUserDecryptionV1(e) => {
                write!(f, "UserDecryptionRequest #{:#066x}", e.decryption_id)
            }
            ProtocolEventKind::PrepKeygen(e) => {
...
impl From<SolanaUserDecryptionRequestV1> for ProtocolEventKind {
    fn from(value: SolanaUserDecryptionRequestV1) -> Self {
        Self::SolanaUserDecryptionV1(value)
    }
}
...
impl TryFrom<DecryptionEvents> for ProtocolEventKind {
    ...
    fn try_from(value: DecryptionEvents) -> Result<Self, Self::Error> {
        match value {
            ...
            DecryptionEvents::UserDecryptionRequest_4(e) => {
                Ok(SolanaUserDecryptionRequestV1::try_from(e)?.into())
            }
...
mod decryption_event_tests {
    ...
    #[test]
    fn rejects_malformed_solana_event_before_persistence() {
        let event = UserDecryptionRequest_4 {
            decryptionId: U256::from(7),
            ctHandles: Vec::new(),
            requestValidity: RequestValiditySeconds::default(),
            publicKey: Default::default(),
            extraData: Default::default(),
            solanaRequest: vec![1, 2, 3].into(),
        };
        assert!(
            ProtocolEventKind::try_from(DecryptionEvents::UserDecryptionRequest_4(event)).is_err()
        );
    }
}
