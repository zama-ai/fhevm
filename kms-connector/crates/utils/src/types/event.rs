...
impl std::fmt::Debug for ProtocolEventKind {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::PrssInit(id) => f.debug_tuple("PrssInit").field(id).finish(),
            Self::KeyReshareSameSet(e) => f.debug_tuple("KeyReshareSameSet").field(e).finish(),
        }
    }
}
...
impl PartialEq for ProtocolEventKind {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (Self::PrssInit(a), Self::PrssInit(b)) => a == b,
            (Self::KeyReshareSameSet(a), Self::KeyReshareSameSet(b)) => a == b,
            _ => false,
        }
    }
}
