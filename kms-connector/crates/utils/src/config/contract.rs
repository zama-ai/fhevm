...
pub fn deserialize_protocol_config_contract_config<'de, D>(
    deserializer: D,
) -> std::result::Result<ContractConfig, D::Error>
where
    D: Deserializer<'de>,
{
    ContractConfig::parse("ProtocolConfig", Deserialize::deserialize(deserializer)?)
        .map_err(|e| serde::de::Error::custom(e.to_string()))
}
...
pub fn default_protocol_config_contract_config() -> ContractConfig {
    ContractConfig {
        contract_name: "ProtocolConfig".to_string(),
        address: address!("0x0000000000000000000000000000000000000000"),
        domain_name: "ProtocolConfig".to_string(),
        domain_version: "1".to_string(),
    }
}
