...
pub async fn insert_rand_response(
    ...
    let inserted_response = match response_str {
        "PublicDecryptionResponse" => KmsResponseKind::PublicDecryption(
            insert_rand_public_decrypt_response(db, id, status).await?,
        ),
        "UserDecryptionResponse" => KmsResponseKind::UserDecryption(
...
pub async fn insert_rand_prep_keygen_response(
    ...
    sqlx::query!(
        "INSERT INTO prep_keygen_responses(prep_keygen_id, signature, otlp_context, status) \
...
pub async fn insert_rand_keygen_response(
    ...
    sqlx::query!(
        "INSERT INTO keygen_responses(key_id, key_digests, signature, otlp_context, status) \
...
pub async fn insert_rand_crsgen_response(
    ...
    sqlx::query!(
        "INSERT INTO crsgen_responses(crs_id, crs_digest, signature, otlp_context, status) \
        VALUES ($1, $2, $3, $4, $5) ON CONFLICT DO NOTHING",
        crs_id.as_le_slice(),
        crs_digest.clone(),
        signature,
