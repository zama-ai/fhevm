...
import {EmptyUUPSProxyACL} from "../../src/host-contracts/contracts/emptyProxyACL/EmptyUUPSProxyACL.sol";
import {EmptyUUPSProxy} from "../../src/host-contracts/contracts/emptyProxy/EmptyUUPSProxy.sol";
import {ACL} from "../../src/host-contracts/contracts/ACL.sol";
import {FHEVMExecutor} from "../../src/host-contracts/contracts/FHEVMExecutor.sol";
import {InputVerifier} from "../../src/host-contracts/contracts/InputVerifier.sol";
import {KMSVerifier} from "../../src/host-contracts/contracts/KMSVerifier.sol";
import {HCULimit} from "../../src/host-contracts/contracts/HCULimit.sol";
import {PauserSet} from "../../src/host-contracts/contracts/immutable/PauserSet.sol";
import {ProtocolConfig} from "../../src/host-contracts/contracts/ProtocolConfig.sol";
import {KMSGeneration} from "../../src/host-contracts/contracts/KMSGeneration.sol";
import {KmsNode, KmsNodeParams, PcrValues} from "../../src/host-contracts/contracts/shared/Structs.sol";
import {IProtocolConfig} from "../../src/host-contracts/contracts/interfaces/IProtocolConfig.sol";
...
import {CleartextProtocolConfig} from "../../src/cleartext/CleartextProtocolConfig.sol";
import {CleartextKMSGeneration} from "../../src/cleartext/CleartextKMSGeneration.sol";
...
library DeployLib {
    ...
    function setProtocolConfigImplementation(VmSafe vm, address protocolConfigAddress, ProtocolConfig protocolConfigImplementation) internal {
        ...
        // v0.14: ProtocolConfig.initializeFromEmptyProxy takes KmsNodeParams[] (a superset of the
        // v0.13 KmsNode) plus a software version and PCR values. For the cleartext mock only the
        // signer/tx-sender pair matters (KMSVerifier reads the signer set back via getKmsSigners);
        // the remaining fields are cosmetic, so they are left empty and party ids are 1-based.
        ...
        UUPSUpgradeable(protocolConfigAddress)
            .upgradeToAndCall(
                ...
                abi.encodeCall(
                    ProtocolConfig.initializeFromEmptyProxy, (initialKmsNodeParams, initialThresholds, "1", pcrValues)
                )
            );
    }
    ...
    function writeAddressesFile(VmSafe vm, string memory path, FhevmAddresses memory a) internal {
        string memory content = string.concat(
            ...
            ");\n",
            "\n",
            "// ConfidentialBridge is not part of the cleartext mock; ACL only needs the constant (v0.14).\n",
            "address constant confidentialBridgeAdd = address(0x812b06e1CDCE800494b79fFE4f925A504a9A9810);\n",
            "\n",
