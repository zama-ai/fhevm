...
library FhevmConfigLib {
    function resolveDeployerFromEnv(VmSafe vm) internal view returns (Signer memory) {
        return SignerLib.resolvePrivateKeyFromEnv(vm, DEPLOYER_PRIVATE_KEY_ENV, DEPLOYER_MNEMONIC_ENV);
    }

    function canResolveEmptyUupsDeployerFromEnv(VmSafe vm) internal view returns (bool) {
        return SignerLib.canResolvePrivateKeyFromEnv(vm, EMPTY_UUPS_PRIVATE_KEY_ENV, EMPTY_UUPS_MNEMONIC_ENV);
    }

    function resolveEmptyUupsDeployerFromEnv(VmSafe vm) internal view returns (Signer memory) {
        return SignerLib.resolvePrivateKeyFromEnv(vm, EMPTY_UUPS_PRIVATE_KEY_ENV, EMPTY_UUPS_MNEMONIC_ENV);
    }

    function resolveDecryptionAddressFromEnv(VmSafe vm) internal view returns (address decryptionAddr) {
        decryptionAddr = _resolveAddressFromEnv(vm, DECRYPTION_ADDRESS_ENV);
    }

    function resolveInputVerificationAddressFromEnv(VmSafe vm) internal view returns (address inputVerificationAddr) {
        inputVerificationAddr = _resolveAddressFromEnv(vm, INPUT_VERIFICATION_ADDRESS_ENV);
    }

    function resolveGatewayChainIdFromEnv(VmSafe vm) internal view returns (uint64 gatewayChainId) {
        gatewayChainId = _resolveUint64FromEnv(vm, CHAIN_ID_GATEWAY_ENV);
    }

    function resolveKmsThresholdFromEnv(VmSafe vm) internal view returns (uint256 kmsThreshold) {
        AssertLib.assertEnvExists(vm, KMS_THRESHOLD_ENV);
        kmsThreshold = vm.envUint(KMS_THRESHOLD_ENV);
    }

    function resolveCoprocessorThresholdFromEnv(VmSafe vm) internal view returns (uint256 coprocessorThreshold) {
        AssertLib.assertEnvExists(vm, COPROCESSOR_THRESHOLD_ENV);
        coprocessorThreshold = vm.envUint(COPROCESSOR_THRESHOLD_ENV);
    }

    function resolveDeployers(VmSafe vm) internal view returns (Signer[] memory signers) {
    ...
    function resolveDeployersAsJson(VmSafe vm) internal view returns (string memory) {
    ...
    function _resolveUint48FromEnv(VmSafe vm, string memory envName) private view returns (uint48 value) {
    ...
    function _resolveUint64FromEnv(VmSafe vm, string memory envName) private view returns (uint64 value) {
        AssertLib.assertEnvExists(vm, envName);
    ...
    function _resolveAddressFromEnv(VmSafe vm, string memory envName) private view returns (address value) {
        AssertLib.assertEnvExists(vm, envName);
