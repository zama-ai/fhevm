...
contract BlindAuction is SepoliaConfig, ReentrancyGuard {
    ...
    function decryptWinningAddress() public onlyAfterEnd {
        ...
        _decryptionRequestId = FHE.requestDecryption(
            cts, 
...
describe("ConfidentialERC20", function () {
  ...
  beforeEach(async function () {
    ...
    this.encryptBid = async (
      targetContract: string,
      userAddress: string,
      amount: number
    ) => {
      const bidInput = hre.fhevm.createEncryptedInput(
        targetContract,
        userAddress
      );
    ...
    this.approve = async (signer: HardhatEthersSigner, amount: number) => {
      ...
      const approveTx = await this.USDCc
        .connect(signer)
    ...
    this.bid = async(signer: HardhatEthersSigner, amount: number) => {
      ...
      const bidTx = await this.blindAuction
        .connect(signer)
        .bid(
