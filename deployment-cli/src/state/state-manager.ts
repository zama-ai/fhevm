...
import { DeploymentStateData, StepResultData, StepState, StepStatus } from './types.js';
...
export class StateManager {
  ...
  private constructor(filePath: string, data: DeploymentStateData, logger: Logger) {
    this.filePath = filePath;
    this.data = data;
    this.logger = logger.child('state');
  }

  public static async create(options: StateManagerOptions, logger: Logger): Promise<StateManager> {
    const { deploymentName, configPath, configHash, steps } = options;
    const stateDir = path.resolve(process.cwd(), options.rootDir ?? DEFAULT_STATE_DIR);
  ...
  public setStepStatus(stepId: string, status: StepStatus): void {
    const step = this.data.steps[stepId] ?? { id: stepId, status: 'pending' };
