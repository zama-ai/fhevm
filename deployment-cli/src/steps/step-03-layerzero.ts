...
import { PackageName } from '../tasks/hardhat-runner.js';
...
export class Step03LayerzeroLink extends BaseStep {
  ...
  protected async validate(ctx: DeploymentContext): Promise<void> {
    ...
    if (!dao) {
      throw new ValidationError('DAO address missing. Complete Step 1 first.');
    }
    if (!safe) {
      throw new ValidationError('Safe address missing. Complete Step 2 first.');
    }
  }
  ...
  protected async execute(ctx: DeploymentContext): Promise<StepExecutionResult> {
    ...
    if (!daoAddress || !safeAddress) {
      throw new ValidationError('DAO and Safe addresses are required before executing Step 3.');
    }
    ...
    if (!senderAddress) {
      ...
      senderAddress = reader.readHardhatDeployment(this.pkgName, ethereumNetwork.name, 'GovernanceOAppSender');
      ...
      ctx.logger.info(`Using existing GovernanceOAppSender at ${senderAddress}`);
    }
    ...
    if (!receiverAddress) {
      ...
      receiverAddress = reader.readHardhatDeployment(this.pkgName, gatewayNetwork.name, 'GovernanceOAppReceiver');
      ctx.env.recordAddress('GOVERNANCE_OAPP_RECEIVER', receiverAddress, this.id);
      ctx.logger.success(`Deployed GovernanceOAppReceiver at ${receiverAddress}`);
    } else {
      ctx.logger.info(`Using existing GovernanceOAppReceiver at ${receiverAddress}`);
    }
