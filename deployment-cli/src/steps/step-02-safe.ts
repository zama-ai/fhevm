...
export class Step02Safe extends BaseStep {
  ...
  protected async execute(ctx: DeploymentContext): Promise<StepExecutionResult> {
    ...
    try {
      safeAddress = taskOutput.readHardhatDeployment(this.pkgName, gateway.name, 'SafeL2Proxy');
    } catch (error) {
      safeAddress = undefined;
    }

    if (safeAddress) {
      ctx.logger.info('Safe artifact found, reading existing deployment...');
      ...
      ctx.logger.info('Deploying Safe multisig wallet (SafeL2Proxy) on Gateway...');
      await ctx.hardhat.runTask({
        pkg: this.pkgName,
      ...
      safeAddress = reader.readHardhatDeployment(this.pkgName, gateway.name, 'SafeL2Proxy');
    ...
    try {
      adminModuleAddress = taskOutput.readHardhatDeployment(this.pkgName, gateway.name, 'AdminModule');
    } catch (error) {
      adminModuleAddress = undefined;
    }

    if (adminModuleAddress) {
      ctx.logger.info('AdminModule artifact found, reading existing deployment...');
      ctx.logger.success(`Using existing AdminModule at ${adminModuleAddress}`);
    } else {
      // Step 2: Deploy AdminModule (allows governance to execute Safe transactions)
      ctx.logger.info('Deploying AdminModule to enable governance control...');
      ...
      adminModuleAddress = reader.readHardhatDeployment(this.pkgName, gateway.name, 'AdminModule');
    ...
    if (ctx.config.options.auto_verify_contracts) {
      ...
      try {
        ...
        ctx.logger.warn('Safe verification failed (this may be acceptable if already verified)');
      }
      ...
      try {
        ...
        ctx.logger.warn('AdminModule verification failed (this may be acceptable if already verified)');
      }
    }
