...
export class Step06GatewayContracts extends BaseStep {
  ...
  protected async validate(ctx: DeploymentContext): Promise<void> {
    if (!ctx.env.getAddress('ZAMA_OFT')) {
      throw new ValidationError('Zama OFT must be deployed before Step 6.');
    }
    if (!ctx.env.getAddress('FEES_SENDER_TO_BURNER')) {
      throw new ValidationError('Fees sender to burner must be deployed before Step 6.');
    }
  }
  ...
  protected async execute(ctx: DeploymentContext): Promise<StepExecutionResult> {
    ...
    const baseEnvVars: Record<string, string> = {
      ...
      FEES_SENDER_TO_BURNER_ADDRESS: ctx.env.getAddress('FEES_SENDER_TO_BURNER')!,
    ...
    if (thresholds.user_decryption !== undefined) {
      baseEnvVars.USER_DECRYPTION_THRESHOLD = thresholds.user_decryption.toString();
    }
    if (thresholds.kms_generation !== undefined) {
      baseEnvVars.KMS_GENERATION_THRESHOLD = thresholds.kms_generation.toString();
    }
    ...
    kmsNodes.forEach((node, idx) => {
      baseEnvVars[`KMS_TX_SENDER_ADDRESS_${idx}`] = node.tx_sender_address;
    ...
    coprocessors.forEach((coprocessor, idx) => {
      baseEnvVars[`COPROCESSOR_TX_SENDER_ADDRESS_${idx}`] = coprocessor.tx_sender_address;
      baseEnvVars[`COPROCESSOR_SIGNER_ADDRESS_${idx}`] = coprocessor.signer_address;
      baseEnvVars[`COPROCESSOR_S3_BUCKET_URL_${idx}`] = coprocessor.s3_bucket_url;
    });
    ...
    custodians.forEach((custodian, idx) => {
      baseEnvVars[`CUSTODIAN_TX_SENDER_ADDRESS_${idx}`] = custodian.tx_sender_address;
      baseEnvVars[`CUSTODIAN_SIGNER_ADDRESS_${idx}`] = custodian.signer_address;
      baseEnvVars[`CUSTODIAN_ENCRYPTION_KEY_${idx}`] = custodian.encryption_key;
    });
    ...
    const gatewayEnvPath = path.join(projectRoot, this.pkgName, 'addresses', '.env.gateway');
    ...
    if (pauserAddresses.length === 0) {
      throw new ValidationError('At least one pauser address is required for Step 5.');
    }
    ...
    if (ctx.config.options.auto_verify_contracts) {
      ...
      try {
        await ctx.hardhat.runTask({
          ...
          args: ['--network', gateway.gatewayPkgName!, '--useInternalProxyAddress', 'true'],
          env: baseEnv
        });
        ctx.logger.success('Gateway contracts verified successfully');
      } catch (error) {
        ctx.logger.warn('Gateway contracts verification failed (this may be acceptable if already verified)');
      }
    }
