...
export class Step01AragonDao extends BaseStep {
  ...
  public readonly description = 'Creates the governance DAO and records its address.';
  ...
  protected async execute(ctx: DeploymentContext): Promise<StepExecutionResult> {
    ...
    const confirmed = await ctx.prompt.confirm('Have you completed the DAO deployment steps?', true);
    if (!confirmed) {
      throw new ValidationError('DAO deployment must be completed to continue.');
    }

    const daoAddress = await ctx.prompt.input('Enter the deployed DAO address');
