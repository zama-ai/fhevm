...
export class TaskOutputReader {
   ...
   */
  public readHardhatDeployment(pkgPath: string, network: string, contractName: string): string {
    ...
    try {
      ...
      if (!data.address) {
        throw new ValidationError(`No address field found in ${artifactPath}`);
      }
      ...
      if (error instanceof SyntaxError) {
        throw new ValidationError(`Invalid JSON in ${artifactPath}: ${error.message}`);
      }
  ...
  public readEnvFile(
    ...
    try {
      ...
      if (fieldMapping) {
        ...
        for (const [envKey, normalizedKey] of Object.entries(fieldMapping)) {
   ...
   */
  public resolveArtifactPath(pkgPath: string, network: string, contractName: string): string {
    return path.join(
      this.projectRoot,
      pkgPath,
      'deployments',
