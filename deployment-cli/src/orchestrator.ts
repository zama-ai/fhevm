...
import { loadDeploymentConfig, DeploymentConfig } from './config/schema.js';
import { EnvManager } from './config/env-manager.js';
import { NetworkRegistry } from './config/networks.js';
import { StateManager } from './state/state-manager.js';
import { StepStatus } from './state/types.js';
import { getDeploymentSteps } from './steps/index.js';
import { DeploymentStep, DeploymentContext } from './steps/base-step.js';
...
export class DeploymentOrchestrator {
  ...
  public static async create(
    ...
    const networks = new NetworkRegistry(config, options.networkEnvironment);
  ...
  public async deploy(options: DeployOptions = {}): Promise<void> {
    ...
    for (const step of stepIdsToRun) {
      ...
      if (options.resume && stepState?.status === 'completed') {
        this.logger.info(`Skipping ${step.id} (${step.name}) - already completed.`);
        continue;
      }

      const dependenciesSatisfied = step.dependencies.every((dependencyId) => {
        const dependencyState = this.state.getStepState(dependencyId);
