...
function parseWithSchema<T extends z.ZodTypeAny>(schema: T, opts: unknown): z.infer<T> {
...
export async function runCLI(): Promise<void> {
  ...
  program
    ...
    .requiredOption('-n, --network <name>', 'Network environment to use (e.g., testnet, mainnet)')
    .option('--resume', 'Resume from previous state by skipping completed steps', true)
    .option('--step <id>', 'Run a specific step by id')
    .action(async (opts: { network: string; resume?: boolean; step?: string }) => {
      ...
      const schema = z.object({
        ...
        step: z
          .string()
          .optional()
          .refine((v) => v === undefined || stepIds.includes(v), `step must be one of: ${stepIds.join(', ')}`)
      });
