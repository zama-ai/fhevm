...
export class EnvManager {
  ...
  public recordAddress(key: string, value: string, source = 'step'): void {
    ...
    this.addresses.set(normalizedKey, { key: normalizedKey, value, source });
  ...
  public getAddress(key: string): string | undefined {
    const normalizedKey = key.toUpperCase();
    return this.addresses.get(normalizedKey)?.value ?? this.state.getAddress(normalizedKey);
  }

  public exportAddresses(targetPath: string): void {
    const data = Object.fromEntries(
      Array.from(this.addresses.values()).map((entry) => [entry.key, entry.value])
    );
