...
# program IDs match each `declare_id!` (see scripts/e2e/test-keypairs/README.md). Always overwrite
# target artifacts: they may have been generated by an older branch with different program IDs.
...
if [ -n "$DEMO_LIFECYCLE_DIR" ]; then
  : "${SOLANA_LEDGER_DIR:?lifecycle mode requires a short, owned SOLANA_LEDGER_DIR}"
  LEDGER_PARENT="$(dirname "$SOLANA_LEDGER_DIR")"
  [[ "$SOLANA_LEDGER_DIR" =~ ^/tmp/fhevm-demo-$(id -u)/[0-9a-f]{24}\.ledger$ ]] \
    || { echo "[setup] invalid lifecycle-owned Solana ledger path" >&2; exit 1; }
  find "$LEDGER_PARENT" -prune -type d -user "$(id -u)" -print | grep -Fxq "$LEDGER_PARENT" \
    || { echo "[setup] Solana ledger parent is not a real owned directory" >&2; exit 1; }
  find "$SOLANA_LEDGER_DIR" -prune -type d -user "$(id -u)" -print | grep -Fxq "$SOLANA_LEDGER_DIR" \
    || { echo "[setup] lifecycle-owned Solana ledger is not a real owned directory" >&2; exit 1; }
...
# transaction instructions; created-public lifecycle outputs retain a narrow CPI event. This stands
# in for Yellowstone ingestion end-to-end. Handle-derivation params are auto-detected from the on-chain HostConfig PDA at
