...
describe('buildClaimInstructions', () => {
  ...
  it('derives every non-root account exactly as claim.rs validates them', async () => {
    ...
    const record = await pda(CONFIDENTIAL_BATCHER_PROGRAM_ADDRESS, [utf8('join-record'), base58.decode(batch), base58.decode(user)]);
  ...
  it('matches the golden derived addresses for the fixed fixture', async () => {
    ...
    expect(addresses[15]).toBe('Fc46oMpQnJjHqM1YNvc6TYgqRjTRyqu71rVKXAedUt4B'); // batchPayoutBalanceValue
