...
import { buildPublicLeafProof, bytesToHex, reconstructSolanaEncryptedValueAccount } from '@sdk-src/solana/proof.js';
...
function fixture() {
  ...
  const live = reconstructSolanaEncryptedValueAccount(base58.decode(state), events);
...
const service = { url: 'http://proof-service', apiKey: 'test' };
...
describe('publicProof', () => {
  it('verifies a burn leaf among balance updates in a shared history', async () => {
    ...
    const request = vi.fn().mockResolvedValue(new Response(JSON.stringify({ proofs: [{
      status: 'found', leafIndex: Number(proof.leafIndex), siblings: proof.siblings.map(bytesToHex),
    }] })));
    ...
    expect(JSON.parse(request.mock.calls[0]![1].body).leaves[0]).toEqual({
      encryptedState: bytesToHex(base58.decode(state)), handle: bytesToHex(handle), kind: 'public',
    });
  });
  ...
  it('rejects a proof for another handle rather than forwarding it to settlement', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(JSON.stringify({ proofs: [{
      status: 'found', leafIndex: Number(proof.leafIndex), siblings: invalidSiblings.map(bytesToHex),
    }] }))));
  ...
  it('fails immediately on incomplete retained history', async () => {
    ...
    const request = vi.fn().mockResolvedValue(new Response(JSON.stringify({ proofs: [{ status: 'historyIncomplete' }] })));
    vi.stubGlobal('fetch', request);
