...
const rpcWith = (account: unknown, status: unknown, blockHeight = 90n): Parameters<typeof reconcileSavedDeposit>[0] =>
  ({
...
describe('deposit join recovery', () => {
  ...
  test('preserves the journal when the join-record lookup fails transiently', async () => {
    ...
    await expect(reconcileSavedDeposit(rpc, session, storedDeposit())).rejects.toThrow('account lookup unavailable');
    expect(localStorage.getItem(activeDepositKey)).toBe(journal);
  });
});
