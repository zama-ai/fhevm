...
pub fn claim<'info>(ctx: Context<'info, Claim<'info>>) -> Result<()> {
    ...
    let joined = state.get::<zama_fhe::Uint<64>>(joined_amount_key()).map_err(fhe::invalid_execution)?;
    let payout_state = fhe::read_state(&ctx.accounts.batch_payout_balance_value)?;
    let output = zama_fhe::Output::state(state.result().allow(user).allow_transient(id, zama_fhe::State::new(&payout_state).id()));
    let execution = zama_fhe::FheExecution::build_returning(
        ...
        |builder| builder.mul_div(joined,
            zama_fhe::Scalar::<zama_fhe::Uint<64>>::u64(ctx.accounts.batch.payout_received),
            zama_fhe::Scalar::<zama_fhe::Uint<64>>::u64(ctx.accounts.batch.total_joined), output),
    ...
    }.invoke(execution, vec![ctx.accounts.join_state.to_account_info(), ctx.accounts.scratch.to_account_info(), ctx.accounts.batch_payout_balance_value.to_account_info()])?;
    ...
    ct::cpi::confidential_transfer_from_value(CpiContext::new_with_signer(
        ...
        &[&authority_seeds],
    ), ct::TransferInput::Grant { handle: claim_handle })?;
