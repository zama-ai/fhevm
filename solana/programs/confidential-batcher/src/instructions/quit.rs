...
pub struct Quit<'info> {
    ...
    /// CHECK: canonical ATA of `batch_authority` on `join_underlying_mint`. Absent = not frozen.
    pub batch_authority_underlying: UncheckedAccount<'info>,
    /// CHECK: canonical ATA of `user` on `join_underlying_mint`. Absent = not frozen.
...
pub fn quit<'info>(ctx: Context<'info, Quit<'info>>) -> Result<()> {
    ...
    ct::cpi::confidential_transfer_from_value(CpiContext::new_with_signer(
        ...
        ct::cpi::accounts::ConfidentialTransferFromValue {
            ...
            from_underlying: ctx.accounts.batch_authority_underlying.to_account_info(),
            to_underlying: ctx.accounts.user_underlying.to_account_info(),
