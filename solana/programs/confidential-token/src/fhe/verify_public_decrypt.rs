...
pub struct VerifyPublicDecrypt<'a, 'info> {
    ...
    /// MMR public-leaf inclusion proof for `expected_handle` against the encrypted store's current peaks.
    pub proof: zama_host::instructions::MmrInclusionProof,
    /// ValueAccount whose peaks the inclusion proof is checked against.
    pub encrypted_store: AccountInfo<'info>,
...
/// and the MMR proof against the encrypted store's peaks; this wrapper adds only the return-data integrity +
    ...
    cpi::verify_public_decrypt(
        CpiContext::new(
            ...
            cpi::accounts::VerifyPublicDecrypt {
                ...
                encrypted_store: request.encrypted_store,
            },
        ),
        expected_handle,
        request.cleartext,
