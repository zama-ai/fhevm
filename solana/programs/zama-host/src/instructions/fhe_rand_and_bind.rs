...
use super::common::*;
use crate::{
    events::{AclAllowedEvent, FheRandEvent},
    state::*,
};

/// Accounts for random ciphertext creation plus initial ACL record birth.
#[derive(Accounts)]
#[instruction(fhe_type: u8, output_nonce_key: [u8; 32], output_nonce_sequence: u64)]
#[event_cpi]
pub struct FheRandAndBind<'info> {
    /// Pays rent for the output ACL record.
    #[account(mut)]
    pub payer: Signer<'info>,
    /// Subject associated with the emitted random event.
...
/// Creates a random ciphertext handle and binds its first canonical ACL record.
pub fn fhe_rand_and_bind(
    ...
    let result = computed_rand_handle(seed, fhe_type, ctx.accounts.host_config.chain_id);
