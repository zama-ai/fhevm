...
fn operand_depth(operand: &FheEvalOperand, step_depths: &[u64]) -> u64 {
    ...
    match operand {
        FheEvalOperand::AllowedLocal { producer_index } => {
            step_depths.get(*producer_index as usize).copied().unwrap_or(0)
        }
    ...
    for step in steps {
        let (op_hcu, max_input_depth) = match step {
            FheEvalStep::Binary {
                ...
                let cost =
                    binary_op_hcu(*op, *output_fhe_type, matches!(rhs, FheEvalOperand::Scalar(_)))?;
        ...
        enforce_le(total, max_hcu_per_tx, ZamaHostError::HcuTransactionLimitExceeded)?;

        // Depth: critical path, capped independently of total (INV-11, INV-12, INV-23).
        let depth = step_depth(op_hcu, max_input_depth)?;
        enforce_le(depth, max_hcu_depth_per_tx, ZamaHostError::HcuTransactionDepthLimitExceeded)?;
...
mod tests {
    ...
    fn add_local(ty: u8, lhs_producer: u16, rhs_producer: u16) -> FheEvalStep {
        FheEvalStep::Binary {
            ...
            lhs: FheEvalOperand::AllowedLocal { producer_index: lhs_producer },
            rhs: FheEvalOperand::AllowedLocal { producer_index: rhs_producer },
            output_fhe_type: ty,
            output: FheEvalOutput::AllowedLocal,
        }
    }
    ...
    fn rand_hcu_unknown_fails_closed() {
        ...
        assert_eq!(rand_hcu(7).unwrap_err(), error!(ZamaHostError::HcuUnknownCost));
        assert_eq!(rand_hcu(200).unwrap_err(), error!(ZamaHostError::HcuUnknownCost));
    }
    ...
    fn cost_accessors_are_deterministic() {
        ...
        assert_eq!(trivial_encrypt_hcu(EU64).unwrap(), trivial_encrypt_hcu(EU64).unwrap());
    }
    ...
    fn meter_total_sums_all_steps_depth_le_total() {
        ...
        for d in &m.step_depths {
            assert!(*d <= m.total, "INV-23: per-value depth never exceeds frame total");
        }
    ...
    fn meter_unknown_cost_propagates() {
        ...
        let steps = vec![FheEvalStep::Rand { fhe_type: 7, output: FheEvalOutput::AllowedLocal }];
        assert_eq!(
