...
//!   act-once state (`confidential-token::disclose_secp`).
//! - Moving value or advancing a state machine on the certified cleartext is not. Guard it with a
//!   lane account that is *opened* when the value-moving effect is first produced and *closed*
//!   exactly once when it is finally consumed, so a second consumption attempt fails because the
//!   lane is gone (`confidential-token::redeem_burned_amount`, which closes the per-`burn_id`
//!   `PendingBurn` lane `confidential_burn` opened at seeds `["pending-burn", mint, token_account,
//!   burn_id]`; the alternate close path is `recover_pending_burn`, while the burned handle is still
//!   tip). Unlike a forever write-once marker, the lane's rent returns to the owner on close instead
//!   of being locked up permanently.
//!
//! The lane cannot ship from a shared crate: Anchor derives an account type's owner from the
//! program that declares it, so state declared here would be owned by this program and unusable as
//! an app's own state. What is shared is this rule, the reference implementation named above, and
