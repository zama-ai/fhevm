...
pub struct RemoveEncryptedValueSubject<'info> {
    ...
    /// `fhe_execute` persistent create/update). Subjects alone cannot remove peers —
    /// apps that store a PDA in `encrypted_value_account_authority` rotate auditors by
    /// CPI + `invoke_signed` as that PDA.
