...
/// - INV-6 / INV-15: preserves `max_hcu_per_tx >= max_hcu_depth_per_tx`, `0` = unlimited
///   (`check_hcu_ordering`).
...
pub fn set_max_hcu_per_tx(ctx: Context<HostAdmin>, value: u64) -> Result<()> {
    ...
    // INV-6: the new total must not fall below the current depth limit (0 = unlimited on either side).
