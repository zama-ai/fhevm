...
This section explains **what survived the cleanup**, **why it deserves to stay**, and **why the layout is shaped this way**. The PoC is not a minimal demo — it is a vertical slice that must stay aligned with EVM fhEVM invariants while remaining editable by app developers (OpenZeppelin) without touching protocol internals.

### Principles the layout encodes

```text
1. Protocol vs app split
   zama-host = generic FHE host (like fhEVM host contracts on EVM).
   confidential-token = one app built on that host (like an ERC-7984-style token).

2. One execution surface on-chain
   All FHE semantics go through execute_frame CPI batches — no per-op host instructions.
   Apps build frames via confidential-token/src/fhe.rs, not by assembling raw CPI by hand.

3. Events are the coprocessor boundary
   ZamaHost emits protocol events via emit_cpi! (inner instructions).
   host-listener decodes those into the same DB shape the EVM path uses.
   App events (BalanceHandleUpdatedEvent, ConfidentialRandCreatedEvent) are indexer-only.

4. Two test speeds, one fixture library
   Fast path: CleartextBackend in solana/tests (LiteSVM, ~44 tests, seconds).
   Slow path: tfhe-worker solana_poc (Postgres + real TFHE, #[ignore], CI job).
   Both import zama-solana-tests so scenarios are not duplicated.

5. Spec lives outside this repo
   RFC 024 in tech-spec PR #448 is normative. This branch implements and validates it.
   No parallel RFC-sync markdown in fhevm — validated text goes to the tech-spec PR.
```

### Top-level map

| Path | Role | Why keep it |
|------|------|-------------|
| `solana/programs/zama-host` | On-chain protocol host | Without it there is no ACL enforcement, handle birth, or protocol events. This is the Solana analogue of the fhEVM host contract surface. |
| `solana/programs/confidential-token` | Reference app | Proves an SPL-like token can drive the host correctly. Safe edit zone for OpenZeppelin — token semantics, not host semantics. |
| `solana/tests` | Test crate + shared fixtures | Single place for LiteSVM tests and worker imports. Avoids a separate `runtime-tests/` crate and a nested `tests/` directory. |
| `solana/crates/zama-host-events` | IDL → Rust decoders | host-listener and solana/tests must decode CPI bytes identically. One codegen crate prevents decoder drift. |
| `solana/scripts/` | IDL sync/check | After zama-host changes, listener IDL must match. Small scripts enforce that in CI. |
| `coprocessor/.../host-listener/.../solana_adapter.rs` | EVM-parity ingestion | Solana txs must land in the same event DB the worker already understands. |
| `coprocessor/.../tfhe-worker/.../solana_poc.rs` | Real-ciphertext E2E | Cleartext tests prove authorization and event shape; worker tests prove TFHE compute + decrypt on Solana-originated handles. |

### `zama-host` — what each piece does

| Module / surface | Purpose |
|------------------|---------|
| `execute_frame` | Single entry for FHE steps + durable `Allow` actions. Matches RFC 024 “frame” model. |
| `allow_acl_subjects` | Append subjects on the canonical ACL PDA (no second record per handle). |
| `allow_for_decryption` | Public-decrypt flag on an ACL record. |
| `assert_acl_record` | Debug/test helper to read back ACL state on-chain. |
| `frame.rs` | Step dispatch: Add, Sub, TrivialEncrypt, Rand. Transient allow inside frame; durable allow only via explicit actions. |
| `acl.rs` | Keyed-nonce PDAs (`nonce_key` + `nonce_sequence`), compute-time ACL checks, subject lists. |
| `handles.rs` | Handle/seed derivation (bank hash, chain id, op metadata). Fails closed without previous slot hash. |
| `FheRandEvent` + global rand counter PDA | Protocol rand semantics aligned with EVM fheRand (not bounded rand — out of PoC scope). |

**Removed and not coming back:** legacy per-op instructions (`fhe_binary_op`, `test_emit_*`, etc.). They duplicated `execute_frame` and confused the host boundary.

### `confidential-token` — what each piece does

| Surface | Purpose |
|---------|---------|
| `initialize_mint` / `initialize_token_account` | SPL-shaped setup with `acl_domain_key`, compute signer PDA, balance ACL slot. |
| `wrap_usdc` | Deposit public USDC → encrypted balance handle via host trivial encrypt. |
| `confidential_transfer` | Sub/add frame, balance handle rotation, output ACL records, `BalanceHandleUpdatedEvent`. |
| `poc_authorize_transfer_amount` | **PoC shortcut:** stand-in for external input verification (trivial encrypt + allow). Keeps transfer E2E unblocked until a real input path exists. |
| `poc_demo_confidential_rand` | End-to-end rand demo: `fhe.rand_u64()` + durable allow + `ConfidentialRandCreatedEvent` for app indexers. |
| `fhe.rs` | Dev-facing `fhe::execute` builder (`encrypted`, `add`, `sub`, `rand_u64`, `allow`). The pattern production apps should follow. |

**Not implemented (intentionally):** `fheRandBounded`, real ciphertext input / transciphering, KMS connector wiring.

### `solana/tests` (`zama-solana-tests`) — module map

One crate, flat `src/`. Integration tests in `host_events.rs` are a `#[cfg(test)]` module so Cargo does not need a second `tests/` tree.

| Module | Why it exists |
|--------|---------------|
| `host_events.rs` | **44 integration tests** — host CPI, token flows, ACL negatives, wrap/transfer/decrypt E2E, rand E2E, KMS-shaped decrypt requests. |
| `programs.rs` | Load compiled `.so` into LiteSVM (`svm_with_program`). |
| `transaction.rs` | Send txs, capture `TransactionMetadata` for inner-instruction event walks. |
| `host_ix.rs` | Build raw ZamaHost instructions (`execute_frame_ix`, labels). |
| `instructions.rs` | Build confidential-token instruction bytes (transfer variants, wrap, rand demo). |
| `fixture.rs` | Token mint/account/SPL setup shared across scenarios. |
| `acl.rs` | PDA addresses, read ACL records, assertion helpers. |
| `events.rs` | Collect/decode CPI events (same path as host-listener). |
| `scenarios.rs` | High-level `run_wrap_scenario`, `run_transfer_scenario`, `run_rand_demo_scenario`. |
| `cleartext.rs` | `CleartextBackend` — local add/sub/trivial/rand for fast semantic checks without Postgres. |
| `semantic.rs` | Compare on-chain results against backend expectations (cleartext or worker). |
| `invariants.rs` | EVM-derived checks (event counts on failure, wrap/transfer output shape). |
| `kms.rs` | RFC-016-shaped user decrypt request builders and test-local verification (not wired to real KMS). |
| `util.rs` | Bank hash seeding, plaintext encoding, handle expectations. |

**Why tfhe-worker depends on this crate:** `solana_poc.rs` runs the same wrap/transfer/rand scenarios against Postgres + real TFHE. Duplicating fixture setup in the coprocessor workspace would guarantee drift.

### Coprocessor touchpoints — why they stay in fhevm

| Component | Justification |
|-----------|---------------|
| `host-listener/solana_adapter.rs` | Normalizes Solana CPI events into existing `TfheContractEvents` / DB rows. Without it, Solana is a dead end — no worker pickup. |
| `host-listener/idl/zama_host.json` | Checked-in IDL snapshot for listener builds and CI without Anchor in every job. Synced from `solana/target/idl` via script. |
| `tfhe-worker/solana_poc.rs` | Proves Solana-originated events compute and decrypt with real ciphertexts. CI job runs transfer paths; rand worker test exists locally. |
| `zama-host-events` shared crate | Single source of truth for event discriminators and decode logic between listener and tests. |

### What was removed in cleanup (and why)

| Removed | Reason |
|---------|--------|
| `solana/runtime-tests/` | Merged into `solana/tests`. One crate, no `runtime-tests/tests/` nesting. |
| `solana/litesvm-harness/` | Renamed to `solana/tests`. Name reflected implementation detail, not purpose. |
| `solana/rfc-024-poc-sync.md` | RFC belongs in tech-spec PR #448, not a second copy in fhevm. |
| Golden CPI fixture + capture test | Binary fixtures go stale on any IDL/host change; inline unit tests in `solana_adapter.rs` cover decode logic. |
| `collect_zama_host_cpi_payloads` | Only served golden capture; typed collectors remain. |

Nothing in that list removed on-chain behavior or integration test coverage.

### What a reader should *not* expect from this PoC

```text
- Production-ready input verification (poc_authorize_transfer_amount is explicit stand-in)
- fheRandBounded / bounded random range
- KMS connector integration (decrypt shape is tested locally)
- TypeScript frontend (Rust helpers in tests/src/kms.rs are the contract for that shape)
- Final account sizing, rent, or mainnet deployment hardening
```

Those gaps are documented as open work, not missing files.
