...
fn the_tally_never_crosses_the_budget_even_transiently() {
    ...
    let _ = FheExecution::build(
        ...
        |builder| {
            ...
            let oversized = builder.sum((0..60).map(|_| value));
            probe(builder, &probes);
            assert_eq!(
            ...
            for tag in 0..MAX_FHE_EXECUTION_STEPS {
                ...
                let result = match attested {
                    Ok(attested_input) => builder
                        .add(attested_input, Scalar::<Uint<64>>::u64(1))
                        .map(|_| ()),
                    Err(error) => Err(error),
                };
                probe(builder, &probes);
...
fn the_shapes_past_each_ceiling_are_rejected_with_their_own_error() {
    ...
    // gate rejects it at the step that crosses. (Wider audiences run out of dictionary slots
    // first: `TooManyDictionaryEntries`.)
    assert_eq!(
        build(Box::new(persist_shape(
            PersistKind::Update,
            MAX_FHE_EXECUTION_STEPS,
...
fn the_invoke_model_matches_a_counting_allocator_for_every_admitted_shape() {
    ...
    for (name, build) in frontier_shapes() {
        ...
        let dynamic_keys: Vec<Pubkey> = execution
            ...
            .collect();
        let authority_keys: Vec<Pubkey> = execution.value_authorities().collect();
        ...
        assert_eq!(authority_keys.len(), cost.value_authorities, "{name}");
        ...
        // Every optional witness present — the maximum the model charges; an absent one only
        // shrinks the real cost. A host account-list change breaks this literal, which is the
