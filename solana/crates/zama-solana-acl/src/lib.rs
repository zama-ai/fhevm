...
//! coprocessor's leaf indexer, and the off-chain KMS connector — for the `EncryptedState`
...
//! crate provides the exact seed list through [`EncryptedState::seeds`].
//!
...
//! third-party verifier reconstruct and check `EncryptedState` state through these exports, so a
...
pub use history::{
    ...
    ReconstructedStateHistory, StateHistoryError, StateHistoryEvent,
};
...
/// Commitment for `HistoricalAccessLeaf { encrypted_state_account, leaf_index, handle, key }`:
...
/// Commitment for `PublicDecryptLeaf { encrypted_state_account, leaf_index, handle }`.
...
mod tests {
    ...
    fn resource_bounds_match_liveness_doc() {
        ...
        assert_eq!(EncryptedState::account_size(0, 0), 121);
        assert_eq!(
            EncryptedState::account_size(MAX_STATE_SLOTS, MAX_MMR_PEAKS),
            4217,
            "max account must stay « Solana's 10240-byte realloc cap"
        );
    }

    /// A test encrypted State that maintains its own leaf list so it can build proofs,
    ...
    fn a_leaf_authorizes_its_key_on_its_handle_only() {
        let owner = h(1);
        let mut l = StateHistory::new(h(10));
        l.update(h(10), &[owner]);
        l.update(h(11), &[owner]);
        let old = l.proof(0);
        let new = l.proof(1);
    ...
    fn on_chain_account_decoder_reads_layout() {
        ...
        assert_eq!(
            data.len(),
            EncryptedState::account_size(l.value.slots.len(), l.value.peaks.len())
        );

        let decoded = decode_encrypted_state(&data).unwrap();
