...
mod tests {
    ...
    fn matches_on_chain_append_and_authorizes() {
        ...
        assert!(authorize_state_historical(acct, &value, h(10), owner, &proof).is_ok());
        assert!(authorize_state_historical(acct, &value, h(10), h(2), &proof).is_err());
        assert!(authorize_state_historical(acct, &value, h(11), owner, &proof).is_err());

        let other = reconstruct(h(0xBB), &events);
        assert_ne!(other.peaks, account.peaks);
        let other_proof = other.build_proof(0).unwrap();
        assert!(authorize_state_historical(acct, &value, h(10), owner, &other_proof).is_err());
        ...
        assert!(authorize_state_public(acct, &pub_value, h(10), &pub_proof).is_ok());
        assert!(authorize_state_public(acct, &pub_value, h(11), &pub_proof).is_err());
    }
}
