...
use zama_solana_acl::{mmr_verify, reconstruct, MmrProof, StoreHistoryEvent};
...
fn to_event(event: &LeafEvent) -> StoreHistoryEvent {
    match event {
        LeafEvent::Allowed { handle, key } => StoreHistoryEvent::Allowed {
            handle: from_hex32(handle),
            key: from_hex32(key),
        },
        LeafEvent::MarkedPublic { handle } => StoreHistoryEvent::MarkedPublic {
            handle: from_hex32(handle),
        },
    }
}
...
fn vector(
    ...
    LeafVector {
        ...
        encrypted_store_account: hex32(&acct),
        events,
...
fn every_committed_vector_recomputes_and_verifies() {
    ...
    for v in &file.vectors {
        let acct = from_hex32(&v.encrypted_store_account);
