...
//! one final close. Scratch account contents are always created by the real host.
...
pub fn fhe_transaction(
    ...
    let scratch = zama_host::transient_address(payer).0;
    let open = anchor_ix(
        zama_host::ID,
        zama_host::accounts::OpenScratch {
            payer,
            scratch,
            instructions: Instructions::id(),
            system_program: anchor_lang::system_program::ID,
        },
        zama_host::instruction::OpenScratch {},
    );
    let close = anchor_ix(
        zama_host::ID,
        zama_host::accounts::CloseScratch {
            scratch,
            refund: payer,
            instructions: Instructions::id(),
        },
        zama_host::instruction::CloseScratch {},
    );
    std::iter::once(open)
        .chain(body)
...
fn body_result(result: TransactionResult) -> InstructionResult {
    let raw_result = result.raw_result.map_err(|error| match error {
        TransactionError::InstructionError(index, error) => {
            assert_eq!(
                index, 1,
                "the scratch envelope failed before/after the tested instruction: {error:?}"
            );
            error
        }
...
pub fn process_fhe_instruction(
    ...
    ensure_system_accounts(context, &[zama_host::transient_address(payer).0]);
...
pub fn check_fhe_instruction(
    ...
    let scratch = zama_host::transient_address(payer).0;
    if !accounts.iter().any(|(address, _)| *address == scratch) {
        accounts.push((scratch, empty_system_account()));
    }
