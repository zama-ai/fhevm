...
leaf-proof servers and the KMS connectors inside the namespace read it. Program keypairs are read only
...
Build scripts pre-install the platform tools Anchor requests (`scripts/install-sbf-tools.sh`).
This avoids `cargo-build-sbf` selecting a different compiler from GitHub's mutable latest
release. If an earlier build mixed tool versions, run `cargo clean --target sbpfv3-solana-solana`
