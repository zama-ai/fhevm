...
export const registerSolanaCoprocessorSql = (programId: string, sourceChainId: string, chainId: bigint = SOLANA_HOST_CHAIN_ID): string => {
  ...
  return `BEGIN;
    ...
    DO $$ BEGIN
      IF EXISTS (SELECT 1 FROM host_chains WHERE chain_id = ${chainId}
    ...
    INSERT INTO host_chains (chain_id,name,acl_contract_address)
      VALUES (${chainId},'solana','${program}') ON CONFLICT DO NOTHING;
    INSERT INTO keys (key_id_gw,key_id,pks_key,sks_key,cks_key,sns_pk,compressed_xof_keyset,chain_id,block_hash)
      SELECT k.key_id_gw,k.key_id,k.pks_key,k.sks_key,k.cks_key,k.sns_pk,k.compressed_xof_keyset,${chainId},k.block_hash
      FROM keys k WHERE k.chain_id=${source}
        ...
        AND NOT EXISTS (SELECT 1 FROM keys s WHERE s.chain_id=${chainId}
