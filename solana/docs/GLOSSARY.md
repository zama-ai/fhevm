...
| **program** | The application program in a State identity. Its control of the authority PDA is proved when the State is created. | — | application contract |
...
| **authority** | The PDA controlling an encrypted state. Its signature authorizes State reads for computation, State outputs and current-slot publication. Token state uses the token-account PDA; contribution state uses each participant’s JoinRecord PDA. `ExecutionAuthority` names an execution signer, not a second identity. | encrypted value account authority, `ExecutionEncryptedValueAccountAuthority` | application contract |
| **slot key** | Application-defined 32-byte key locating a current handle inside State; never a PDA seed. Examples: balance, total supply, burned amount, contribution. | encrypted value label | storage slot |
| **StateId** | Builder identity containing `(program, authority, scope)` and its canonical State address. A slot is separately identified by its key. | `EncryptedValueId` | — |
| **allow** | A historical decrypt permission binding State, handle and allowed key. Fresh State outputs may append allows without occupying a slot. Later permission changes for history-only handles are deferred to #2007. An allow does not itself admit compute in the current Solana model. | subject, subject list, `allow_subjects`, `remove_subject`, `persistAllowed` entry | `FHE.allow`, with re-sharing deferred on Solana |
...
| **transient** | An execution-local result referenced by `EarlierStep`. Cross-invocation use requires an explicit scratch result grant; returning its bytes supplies no permission. | — | local FHE result |
...
| **State slot** | `FheExecuteOperand::StateSlot` reads a key and checks the expected handle; `FheExecuteOutput::State` optionally writes a slot and appends permissions. Builder `State.get`/`State.set` describe these operations; `FheHandle<T>` carries a validated typed handle. | `StoredValue`, `AllowedPersistent` | application storage |
...
| **persistent** | State or decrypt history that outlives the execution. A historical result can remain decryptable without a current slot. | durable | persistent ACL permission |
| **create / created-public** | Create allocates empty encrypted state separately from computation. A fresh execution result can be made public with `make_public`, with or without a slot write. | birth, born-public | — |
| **update** | Replace a State slot using an expected previous handle and previous shared leaf count. Failure reverts the transaction. Previously sealed permissions remain valid. | supersede, rotation | storage update |
...
| **result grant** | Transaction-local compute permission for an exact produced handle and consumer State. Held in host-owned scratch, derived from an initiating State; the consumer authority must sign. Never a decrypt leaf. | — | `allowTransient` composition |
| **scratch** | Host-owned PDA `["transient", initiating_state]`, opened with that State authority’s signature and closed by the exact final top-level instruction. Closure refunds the payer recorded at opening. Failed finalization rolls back all writes. | — | transient ACL storage |
...
| **MMR** | Append-only Merkle mountain range shared by all decrypt permissions under a State. Only peaks and leaf count are stored on-chain; proofs establish exact historical leaves. | — | — |
| **leaf** | Historical-access commitment `(state, leaf_index, handle, allowed_key)` or public commitment `(state, leaf_index, handle)`, each domain-separated. Fresh State outputs append allows in declaration order, then a public leaf if requested. | — | — |
...
| **disclosed value kind** | `DisclosedValueKind` selects the current token slot when requesting publication. The generic `HandleDisclosedEvent` certifies State, handle and cleartext, not the slot kind; original operation events identify the result. | — | generic amount disclosure |
...
- **"subject"**, **"compute subject"**, **"compute signer"**: an encrypted State keeps no list of who may decrypt it, and reading a value is
  admitted by its authority's signature. Say *allow* / *viewer* / *allowed
  key*, or *State authority*.
...
- **`value_key`**, or the retired `EncryptedValue` account: State identity is `(program, authority, scope)`;
  a slot key selects a handle within that State.
