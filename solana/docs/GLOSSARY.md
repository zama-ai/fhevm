...
|---|---|---|---|
| **handle** | 32-byte opaque identifier of one ciphertext, derived on-chain during execution. Bytes 22–29 are the host `chain_id` (DD-052): high byte `0x01` for Solana, `0x00` for EVM. | — | `bytes32` handle |
...
| **application** | The pair `(program, scope)`: the program a value belongs to and the scope, an account of that program, it named. `AppScope` in code. It is the identity HCU metering charges, the deny list names, and a permit scopes to. Trustworthy only as the pair: the host verifies `program` on every write (below), so another program cannot forge that half, and `scope` is an account `program` owns, checked when the store is created. | domain, `acl_domain_key`, compute subject (as the metered identity) | ACL contract instance / `msg.sender` of the dapp |
...
| **scope** | The instance partition, paired with `program`: an account `program` owns, such as a confidential mint for token state or a batch for contribution state. `create_encrypted_store` refuses any other address. A permit can restrict `(program, scope)` pairs. Scope is not a signing credential. | domain, `domainKey`, `allowedAclDomainKeys` | application instance |
...
| **allowed key** | The key an allow leaf binds a handle to: the account whose permission the leaf records. A user-decrypt entry names it as the entry's owner address. | subject | the `account` of `persistAllowed` |
