...
| **transient** | A result recorded in the transaction’s transient store. Its producing State may use it across calls; another State needs an explicit grant. `EarlierStep` references a current-call occurrence. Returning bytes grants no permission. | — | local FHE result |
...
| **result grant** | Transaction-local compute permission for an exact produced handle and consumer State. Held in shared host-owned transient store; the consumer authority must sign consumption, not grant creation. Never a decrypt leaf. | — | `allowTransient` composition |
