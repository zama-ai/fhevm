...
| **brand** | Compile-time identity of one builder, and SDK-only: `'brand` is a fresh invariant lifetime [`FheExecution::build`] hands each invocation, carried by every transient value that builder returns, so feeding one builder's value to another does not compile (`compile_fail` doctest on `FheExecution::build`). Nothing on the wire and never read at runtime — the field is a `PhantomData`, so the type is the whole mechanism. It stands in for a runtime scope tag, which SBF cannot make unique because writable statics are forbidden on-chain. | runtime scope tag | — |
