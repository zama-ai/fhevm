...
| **transient** | A result recorded in transaction scratch. Its producing State may use it across calls; another State needs an explicit grant. `EarlierStep` references a current-call occurrence. Returning bytes grants no permission. | — | local FHE result |
