...
| **encrypted value account authority** | Second component of an encrypted value ID: the account that controls the value. It must sign to create it, update its handle, or replace its subjects — the host program checks exactly that (`require_keys_eq!(authority, value.encrypted_value_account_authority)` in `assert_output_acl_metadata`). For a token balance it is the token account itself. Not called just "account": that named the attachment and hid the authority. | `app_account`, account | the contract holding the value |
| **encrypted value label** | Third component of an encrypted value ID: the app-chosen discriminator naming *which* encrypted value of the authority this is — `balance`, `total_supply`, `burned_amount`. Bare "label" says only that it is 32 bytes, which is true of all three components. The struct field stays `label` where the enclosing type already says `EncryptedValue`; it carries the full name wherever it stands alone. | SDK `namespace` | the storage slot's name |
| **subject** | A pubkey in an encrypted value account's subject list: it may decrypt that value. Membership is the whole ACL — there are no role flags. | — | ACL `account` address |
