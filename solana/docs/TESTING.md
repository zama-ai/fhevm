...
| --- | --- | --- | --- | --- |
| Clear-text evaluator model | `cargo test -p zama-solana-runtime-tests --test cleartext_fhe_eval` | The test-owned evaluator agrees with the intended type and operator truth tables, including rejected shapes. | Host instruction admission, SBF execution, TFHE evaluation, randomness, ACL/attestation checks, or any production path. | None beyond a Rust toolchain. Warm: very fast. |
...
| Real SBF token runtime | `bash scripts/check-zama-host-idl.sh && cargo test -p zama-solana-runtime-tests --test token_mollusk -- --nocapture` | `confidential-token` plus real host/token/SPL CPIs, state transitions, events, settlement, and rejection paths under Mollusk. | A validator, relayer/coprocessor/KMS wiring, or real TFHE. | Same SBF prerequisite and cold-build cost as the host suite. |
| Host-listener reconstruction | `cd ../coprocessor/fhevm-engine && SQLX_OFFLINE=true cargo test -p host-listener --features solana-reconstruct solana_reconstruct::` | Emitless Solana instruction/account decoding and deterministic host-event reconstruction. | Yellowstone transport, a live validator, database insertion, worker compute, or decrypt completion. | Coprocessor workspace dependencies and offline SQLx metadata. Warm: focused; cold compilation can take minutes. |
...
`cleartext_fhe_eval` is a test-owned model/mock. It is deliberately independent evidence for
operator intent; it is not an implementation of host admission, TFHE, cryptographic randomness,
ACL or attestation enforcement, and it is not an authority or example for production code quality.
Passing it is useful only for behavior that does not depend on those omitted boundaries.
...
The `host_mollusk` and `token_mollusk` suites execute real SBF under Mollusk. Mollusk surfaces
resulting **account state**, **inner instructions (CPIs)**, and **return data**, which are the stable
artifacts these suites assert on. Plain `emit!` program-data logs are intentionally not part of the
runtime-test contract; tests should assert the state transition, emitted Anchor CPI event, return
data, or CPI shape that makes the behavior observable.
