...
**Requirement:** keep the PoC ↔ RFC-021 mirror in sync as the gateway evolves. The Solana listener
currently consumes finalized Yellowstone updates and inserts directly, so it does not use the EVM
block-status reorg substrate (`host_chain_blocks_valid` + `cmd/block_history.rs`). If ingestion moves
to a lower commitment, define rollback handling before that change. Decide the decrypt-release
commitment level (`finalized` vs `confirmed`).
