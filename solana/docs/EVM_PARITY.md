...
| `ACL.isAllowedForDecryption(handle)` | public-decrypt flag | exact-handle `PublicDecryptLeaf` MMR inclusion proof (no live flag — DD-032); exposed to any app (not token-only) as the stateless host `verify_public_decrypt`, which checks that proof + a KMS cert against the current context and returns `(handle, cleartext)` via `return_data` (DD-040) | **MET** (DIVERGENCE: proof, not a stored flag) |
