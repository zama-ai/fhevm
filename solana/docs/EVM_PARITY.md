...
|---|---|---|---|
| `ACL.allow(handle, account)` | persistent grant; caller must be allowed | `allow_subjects` — authority `Signer` must equal `EncryptedValue.account` (same gate as persistent create/update; decrypt subjects are not co-admins) + canonical PDA + deny-list + pause; append-only and idempotent, capped at `MAX_ENCRYPTED_VALUE_SUBJECTS=8` (DD-032; fhevm-internal#1862 #13). Token-scoped values use confidential-token `allow_token_account_subjects` (CPI + `invoke_signed` as the token-account PDA) | **MET** (DIVERGENCE: account-gated, not subject-admin) |
