...
| `ACL.invalidateDecryptionSignaturesBefore(timestamp)` | raise the caller's watermark so older decrypt signatures die | `revoke_permits` — the same per-user watermark (`PermitInvalidation` at `["permit-invalidation", user]`, absent = 0), monotonic, enforced by the connector's `check_not_invalidated` against the permit's signed `start_timestamp`. Takes **no argument**: it always writes the current clock, so "invalidate everything signed before an earlier moment" is not expressible. Deliberately config-free — a lever the operator can pause is not the user's lever. Neither side reaches a permit pre-signed to open later (INVARIANTS #63; EVM reverts `InvalidationTimestampInTheFuture`) | **PARTIAL** — watermark and enforcement match; the caller-chosen past timestamp is missing |
