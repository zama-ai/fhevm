...
| ---------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Contract storage holds encrypted handles | Host-owned `EncryptedStore` PDA `(program, authority, scope)` holds bounded keyed slots and a shared MMR                                                                                            | Apps read accounts; slot keys do not create separate PDAs.                                                                                                             |
...
| Previously allowed encrypted operand     | Store-slot input requires exact current handle and Store authority signature; a transient input requires its producing Store or an exact transient store grant, plus that Store authority signature | Compute permission is separate from an MMR decrypt permission.                                                                                                         |
| `FHE.allow` / `allowThis`                | A fresh Store output appends exact-handle/key leaves, optionally without storing a slot                                                                                                             | Existing viewers cannot directly re-share a historical handle. New history-only grants are deferred to [#2007](https://github.com/zama-ai/fhevm-internal/issues/2007). |
| `FHE.allowTransient`                     | All results are implicitly usable by their producing Store across calls; explicit grants share them with other Stores                                                                               | The transient store must be opened and closed in that transaction. It authorizes exact handle/consumer-Store use, and its expiry does not restrict derived outputs.    |
...
are outside the port today. Coprocessor and KMS signature checks use the same EIP-712 trust roots
...
| Transfer an existing handle                        | `confidential_transfer_from_value` uses a current slot or transient store grant through `TransferInput`, with the required authority signatures.                                                                                                                                       |
| Receiver contract composes with transferred result | Receiver app exposes its own entrypoint. Token returns the transferred handle and grants the consuming Store through transient store. Batcher updates its own contribution; each JoinRecord owns that Store. No transfer receipt accumulator or permanent transferred-amount register. |
...
| Balance / total supply                             | Read the corresponding slot in the token-account or total-supply-authority Store.                                                                                                                                                                                                      |
