...
| `ACL.allowForDecryption(handles[])` | mark publicly decryptable | `make_handle_public` — any allowed subject can seal an exact-handle `PublicDecryptLeaf` into the MMR so publicness never survives a later `update_encrypted_value` (DD-005 superseded, DD-032) | **MET** (DIVERGENCE: per-handle) |
