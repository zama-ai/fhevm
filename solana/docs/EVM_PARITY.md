...
| `_burn(from, amount)` | decrease balance + total supply | `confidential_burn` (supersede balance + total-supply encrypted value accounts) + `redeem_burned_amount` consuming the stateless host `verify_public_decrypt` (release underlying from vault — KMS-cert gated against the live context the cert names, fhevm-internal#1765; public-decrypt gated, permanent replay marker) | **MET** + Solana-only redeem leg (no ERC7984 analogue; wrapper needs underlying release) |
