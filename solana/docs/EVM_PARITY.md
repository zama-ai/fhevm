...
| Previously allowed encrypted operand | State-slot input requires exact current handle and State authority signature; a transient input requires its producing State or an exact transient store grant, plus that State authority signature | Compute permission is separate from an MMR decrypt permission. |
...
| `FHE.allowTransient` | All results are implicitly usable by their producing State across calls; explicit grants share them with other States | The transient store must be opened and closed in that transaction. It authorizes exact handle/consumer-State use, and its expiry does not restrict derived outputs. |
...
| Receiver contract composes with transferred result | Receiver app exposes its own entrypoint. Token returns the transferred handle and grants the consuming State through transient store. Batcher updates its own contribution; each JoinRecord owns that State. No transfer receipt accumulator or permanent transferred-amount register. |
