...
| `FHEVMExecutor` batched/expression compute | — | `fhe_execute` (bounded mixed-step eval; `MAX_FHE_BATCH_OPS=32`) | **MET** (Solana-native batching to limit CPI depth; DD-008; mixed binary/ternary/unary/trivial-encrypt/rand/rand-bounded/sum/isIn/mulDiv steps with `AllowedLocal` transients, and `VerifiedInput` operands) |
