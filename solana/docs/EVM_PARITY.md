...
| Previously allowed encrypted operand | State-slot input requires exact current handle and State authority signature; a transient input requires an exact scratch grant and consumer authority signature | Compute permission is separate from an MMR decrypt permission. |
...
| `FHE.allowTransient` | Earlier-step operands inside an execution; canonical scratch grants across invocations in the same transaction | Scratch must be opened and closed in that transaction. It authorizes exact handle/consumer-State use, and its expiry does not restrict derived outputs. |
