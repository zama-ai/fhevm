...
| `confidentialTotalSupply()` view | encrypted total supply handle | one stable total-supply `EncryptedValue` encrypted value account (born in `initialize_mint`; replaced by `confidential_burn`, DD-032) — read off-chain | **MET** (DIVERGENCE: account read, not a view call) |
...
| `FHE.allowTransient(transferred, sender)` | transaction-local grant | instruction-local `AllowedLocal` value within one `fhe_execute`, plus CPI signer propagation within one instruction | **DIVERGENCE** (no tstore; DD-008) |
...
| `ACL.allowTransient(handle, account)` | tx-local grant | no persistent analog; instruction-local `AllowedLocal` within one `fhe_execute`, plus CPI signer propagation within one instruction | **DIVERGENCE** (no tstore; DD-008) |
...
| `ACL.cleanTransientStorage()` | wipe tx transient (AA bundling) | nothing to reclaim — `AllowedLocal` values are instruction-scoped and never persisted (the one-shot `TransientSession` account tier was removed, DD-008) | **DIVERGENCE** (no persistent transient state) |
...
| `FHEVMExecutor.verifyCiphertext` (input) | verify signed/proved input, allowTransient | `fhe_execute` `FheExecuteOperand::VerifiedInput` operand — in-batch secp256k1 recover + threshold-check of the coprocessor EIP-712 `CiphertextVerification` attestation, asserts `contract_chain_id == host chain id`; transient-allowed, no persistent ACL | **MET** (DD-007: verify ≠ allow; same coprocessor EIP-712 trust root as EVM) |
| `FHEVMExecutor` batched/expression compute | — | `fhe_execute` (bounded mixed-step eval; `MAX_FHE_BATCH_OPS=16`) | **MET** (Solana-native batching to limit CPI depth; DD-008; mixed binary/ternary/unary/trivial-encrypt/rand/rand-bounded/sum/isIn/mulDiv steps with `AllowedLocal` transients, and `VerifiedInput` operands) |
...
| `HCULimit` (per-op/tx/block/depth homomorphic-compute caps) | gas-like metering | `HostConfig::max_hcu_per_tx` / `max_hcu_depth_per_tx` summed over an `fhe_execute` batch (`u64::MAX` = off), plus per-app per-slot block cap (`hcu_block_cap_per_app`, DD-039), plus Solana compute-budget + op-count/collection caps | **DIVERGENCE** (per-batch + per-app-per-slot cap vs global per-block metering) — see fragility #3 |
...
stateless host `verify_public_decrypt` verifier (DD-040, binds the disclosed encrypted value account to the mint ACL
...
3. **No *global* per-block HCU plane.** The host enforces a per-app, per-slot HCU block cap
   (`HostConfig::hcu_block_cap_per_app`, DD-039) plus per-batch total and critical-path caps
...
- **Constraint-driven divergences** are all intentional and documented (no tstore, no `msg.sender`,
  shallow CPI, per-batch HCU cap vs per-block metering, account-witnesses vs storage reads). Input and
