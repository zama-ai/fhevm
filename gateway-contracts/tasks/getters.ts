...
async function loadCoprocessorContextsContract(
  ...
  const coprocessorContextsAddress = customCoprocessorContextsAddress
    ...
    : dotenv.parse(fs.readFileSync("addresses/.env.gateway")).COPROCESSOR_CONTEXTS_ADDRESS;
...
task("task:getCoprocessorSigners")
  .addOptionalParam(
    "customCoprocessorContextsAddress",
    "Use a custom address for the CoprocessorContexts contract instead of the default one - ie stored inside .env.gateway",
  )
  .setAction(async function (taskArguments: TaskArguments, { ethers }) {
    ...
    const listCurrentCoprocessorSigners = await coprocessorContexts.getCoprocessorSigners(activeContextId);
    console.log(
