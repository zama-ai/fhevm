...
describe("MultichainACL", function () {
  ...
  describe("Delegate account", async function () {
    ...
    beforeEach(async function () {
      ...
      for (let i = 0; i < coprocessorTxSenders.length; i++) {
        await MultichainACL.connect(coprocessorTxSenders[i]).delegateAccount(
          hostChainId,
          delegationAccounts,
          allowedContracts,
        );
    ...
    it("Should delegate account with 2 valid responses", async function () {
      ...
      await MultichainACL.connect(coprocessorTxSenders[0]).delegateAccount(
        hostChainId,
        newDelegationAccounts,
        allowedContracts,
      );
      const txResponse = MultichainACL.connect(coprocessorTxSenders[1]).delegateAccount(
        hostChainId,
        newDelegationAccounts,
        allowedContracts,
      );
    ...
    it("Should delegate account with 2 valid responses and ignore the other valid one", async function () {
      ...
      const txResponse1 = await MultichainACL.connect(coprocessorTxSenders[0]).delegateAccount(
        hostChainId,
        newDelegationAccounts,
        allowedContracts,
      );
      ...
      const txResponse3 = await MultichainACL.connect(coprocessorTxSenders[2]).delegateAccount(
        hostChainId,
        newDelegationAccounts,
        allowedContracts,
      );
    ...
    it("Should revert because coprocessor tries to delegate account twice", async function () {
      await expect(
        MultichainACL.connect(coprocessorTxSenders[0]).delegateAccount(
          hostChainId,
          delegationAccounts,
          allowedContracts,
        ),
