    ...
    Pausable
{
    ...
    string private constant EIP712_KEYGEN_TYPE =
        "KeygenVerification(uint256 prepKeygenId,uint256 keyId,bytes serverKeyDigest,bytes publicKeyDigest)";
    ...
    struct KmsManagementStorage {
        ...
        uint256 keyCounter;
        /// @notice The mapping of prepKeygenId to keyId
        mapping(uint256 prepKeygenId => uint256 keyId) prepKeygenIdToKeyId;
    ...
    function keygen(ParamsType paramsType) external virtual onlyGatewayOwner {
        ...
        // represents a preprocessing keygen request, with format: [0000 0001 | counter_1..31]
        $.prepKeygenCounter++;
    ...
    function prepKeygenResponse(uint256 prepKeygenId, bytes calldata signature) external virtual onlyKmsTxSender {
        ...
        if (!$.isRequestDone[prepKeygenId] && _isKmsConsensusReached(consensusTxSenders.length)) {
            ...
            // represents a keygen request, with format: [0000 0010 | counter_1..31]
            $.keyCounter++;
            uint256 keyId = $.keyCounter;

            // Store the keyId associated to the prepKeygenId
            $.prepKeygenIdToKeyId[prepKeygenId] = keyId;
    ...
    function keygenResponse(
        ...
        uint256 prepKeygenId = $.prepKeygenIdToKeyId[keyId];

        // Compute the digest of the KeygenVerification struct.
        bytes32 digest = _hashKeygenVerification(prepKeygenId, keyId, serverKeyDigest, publicKeyDigest);
        ...
        if (!$.isRequestDone[keyId] && _isKmsConsensusReached(consensusUrls.length)) {
            ...
            emit ActivateKey(keyId, consensusUrls);
        }
    }
    ...
    function crsgenRequest(uint256 maxBitLength, ParamsType paramsType) external virtual onlyGatewayOwner {
        ...
        // represents a crsgen request, with format: [0000 0011 | counter_1..31]
        $.crsCounter++;
    ...
    function crsgenResponse(
        ...
        if (!$.isRequestDone[crsId] && _isKmsConsensusReached(consensusUrls.length)) {
            ...
            emit ActivateCrs(crsId, consensusUrls);
        }
    }
     ...
     * @param serverKeyDigest The digest of the generated server key.
     * @param publicKeyDigest The digest of the generated public key.
            ...
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        ...
                        prepKeygenId,
                        keyId,
                        keccak256(serverKeyDigest),
                        keccak256(publicKeyDigest)
                    )
                )
            );
    }
