...
// `check` and `update` must produce byte-identical mocks, so both run the SAME Prettier pass: `check`
// formats its freshly generated mocks before comparing, validating exactly what `update` writes. The
// comparison stays whitespace-insensitive only to ignore trailing-newline / line-ending noise.
const normalizeForComparison = (content) => content.replace(/\s+/g, '');

// Format a directory of .sol mocks with the project's Prettier config. Invokes the local Prettier
// binary directly (not `npx`, which can print install notices to stderr and false-fail) and rejects
// only on a non-zero exit, so callers fail loudly on a real Prettier error.
const PRETTIER_BIN = path.join(__dirname, '..', 'node_modules', '.bin', 'prettier');
function formatWithPrettier(targetPath) {
  return new Promise((resolve, reject) => {
    execFile(PRETTIER_BIN, [targetPath, '--write', '--log-level', 'silent'], (error) => {
...
program
  ...
  .action(() =>
    generateAllMockContracts().catch((e) => {
      logError(e.message);
      exit(1);
    }),
  );
 ...
 */
async function checkMocksUpToDate() {
  logInfo('Checking if mock contracts are up-to-date...');

  // Generate into a project-local temp dir so Prettier resolves the gateway-contracts config
  // (solidity plugin, single quotes) exactly as `update` does — a dir under the OS tmpdir would
  // fall back to Prettier defaults and never match what `update` writes.
  const tempDir = fs.mkdtempSync(path.join(__dirname, '..', '.mock-check-'));
  ...
  try {
    ...
    // Generate every mock into the temp dir, then format it with the same Prettier pass `update`
    // runs, so the comparison below is against exactly what `update` would write.
    for (const mockFile of existingMockFiles) {
      ...
      if (!fs.existsSync(contractFilePath) || !fs.existsSync(interfaceFilePath)) {
        // Throw (not exit) so the `finally` below still removes the temp dir.
        throw new Error(`Contract or interface file not found for mock: ${mockFile}`);
      }
    ...
    for (const mockFile of existingMockFiles) {
      ...
      const existingContent = normalizeForComparison(fs.readFileSync(mockFile, 'utf-8'));
      const generatedContent = normalizeForComparison(fs.readFileSync(tempMockFile, 'utf-8'));
    ...
    if (outdated) {
      ...
      // Throw (not exit) so the `finally` below still removes the temp dir; the CLI action handler
      // logs the message and exits non-zero.
      throw new Error('Some mock contracts are outdated. Run `update` to regenerate them.');
    }
 ...
 */
async function generateAllMockContracts() {
  ...
  // Format the generated mocks with the same Prettier pass `check` runs (awaited, so a failure
  // surfaces instead of being swallowed in a fire-and-forget callback).
  await formatWithPrettier(MOCKS_DIR);
