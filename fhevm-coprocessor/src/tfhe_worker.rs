...
use std::{cell::Cell, collections::{BTreeSet, HashMap}, num::NonZeroUsize};
use crate::types::{TfheTenantKeys, SupportedFheCiphertexts};
use crate::tfhe_ops::{current_ciphertext_version, deserialize_fhe_ciphertext, perform_fhe_operation};

pub async fn run_tfhe_worker(args: crate::cli::Args) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    loop {
        // here we log the errors and make sure we retry
        if let Err(cycle_error) = tfhe_worker_cycle(&args).await {
            eprintln!("Error in background worker, retrying shortly: {:?}", cycle_error);
        }
        tokio::time::sleep(tokio::time::Duration::from_millis(5000)).await;
    }
}

async fn tfhe_worker_cycle(args: &crate::cli::Args) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    loop {
        ...
        if !keys_to_query.is_empty() {
            ...
            ", &keys_to_query).fetch_all(trx.as_mut()).await?;

            assert!(keys.len() > 0, "We should have keys here, otherwise our database is corrupt");
        ...
        for w in the_work {
            ...
            tfhe_work_set.spawn_blocking(move || -> Result<_, (Box<(dyn std::error::Error + Send + Sync)>, i32, String)> {
                ...
                if w.tenant_id != TFHE_TENANT_ID.get() {
                    ...
                    let keys = rk.get(&w.tenant_id).expect("Can't get tenant key from cache");
            ...
            match finished_work_unit {
                ...
                Err((err, tenant_id, output_handle)) => {
                    let _ = query!("
                        ...
                        WHERE tenant_id = $2
                        AND output_handle = $3
                    ", err.to_string(), tenant_id, output_handle)
                    .execute(trx.as_mut())
                    .await?;
                }
            }
